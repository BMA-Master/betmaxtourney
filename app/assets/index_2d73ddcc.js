...
/*  Neodigm JuiceBar End  */ 
    ...
    `,this.setupEventListeners()}}customElements.define("bma-bet-entry",ef);class tf extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get observedAttributes(){return["data-match-id","data-match-guid","data-match-title","data-match-short-title","data-scheduled-at","data-home-team","data-away-team","data-odds-markets","data-is-disabled","data-sync-theme","data-scoreboard","data-sport-key","data-tournament-tags"]}get matchId(){return this.getAttribute("data-match-id")||""}get matchGuid(){return this.getAttribute("data-match-guid")||""}get matchTitle(){return this.getAttribute("data-match-title")||""}get shortTitle(){return this.getAttribute("data-match-short-title")||this.matchTitle}get scheduledAt(){return this.getAttribute("data-scheduled-at")||""}get homeTeam(){return this.getAttribute("data-home-team")||"Home"}get awayTeam(){return this.getAttribute("data-away-team")||"Away"}get oddsMarkets(){const t=this.getAttribute("data-odds-markets");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing odds_markets:",s),null}}get isDisabled(){return this.getAttribute("data-is-disabled")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}get scoreboard(){const t=this.getAttribute("data-scoreboard");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing scoreboard:",s),null}}get sportKey(){return this.getAttribute("data-sport-key")||""}get sportGroup(){const t=this.sportKey;if(!t)return"";const s=qt.find(n=>n.key===t);return s?s.group:""}get tournamentTags(){const t=this.getAttribute("data-tournament-tags");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing tournament tags:",s),[]}}isMatchInProgressLocked(){if(!this.tournamentTags.some(o=>o.match_inprogress_lock===!0))return!1;const n=this.scoreboard;if(!n||!n.time_remaining)return!1;const a=n.time_remaining.toLowerCase();return a==="in progress"||a.includes("q")||a.includes("half")||a.includes("period")}connectedCallback(){this.render(),this.startCountdownTimer()}disconnectedCallback(){this.stopCountdownTimer()}startCountdownTimer(){this.stopCountdownTimer(),this.countdownInterval=setInterval(()=>{this.updateCountdown()},6e4)}stopCountdownTimer(){this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null)}updateCountdown(){const t=this.shadowRoot?.querySelector(".countdown");t&&(t.textContent=this.getCountdownText())}attributeChangedCallback(t,s,n){s!==n&&this.render()}formatDate(t){if(!t)return"TBD";let s=new Date(t);return!t.includes("Z")&&!t.match(/[+-]\d{2}:\d{2}$/)&&(s=new Date(t+"Z")),s.toLocaleString(void 0,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}getCountdownText(){if(!this.scheduledAt)return"";const t=new Date,s=new Date(this.scheduledAt);!this.scheduledAt.includes("Z")&&!this.scheduledAt.match(/[+-]\d{2}:\d{2}$/)&&s.setTime(new Date(this.scheduledAt+"Z").getTime());const n=s-t;if(n<=0)return"";const a=Math.floor(n/(1e3*60*60)),r=Math.floor(n%(1e3*60*60)/(1e3*60));return a>24?`${Math.floor(a/24)}d ${a%24}h`:a>0?`${a}h ${r}m`:r>5?`${r} min`:"Starting Soon"}getGameStatus(){const t=this.getCountdownText();if(t)return{type:"upcoming",text:t,color:"#F7C60D"};const s=this.scoreboard;if(s&&s.time_remaining){const n=s.time_remaining.toLowerCase();if(n==="final")return{type:"final",text:"Final",color:"#969696"};if(n==="in progress"||n.includes("q")||n.includes("half")||n.includes("period")){const a=s.home_score!==void 0&&s.home_score!==null||s.away_score!==void 0&&s.away_score!==null,r=s.period!==void 0&&s.period!==null&&s.period!==0;if(a||r)return{type:"live",text:s.time_remaining,color:"#00E676"}}}return{type:"scheduled",text:this.formatDate(this.scheduledAt),color:"#969696"}}handleButtonClick(t,s,n,a,r){t.stopPropagation();const o=t.currentTarget;if(o.classList.contains("btn--disabled")||o.dataset.disabled==="true")return;const i=o.classList.contains("btn--active"),l={matchId:this.matchId,matchGuid:this.matchGuid,matchTitle:this.matchTitle,abbreviatedTitle:this.shortTitle,homeTeam:this.homeTeam,awayTeam:this.awayTeam,scheduledAt:this.scheduledAt,type:s,team:n,price:a,point:r,timestamp:Date.now()};this.dispatchEvent(new CustomEvent("bet-button-click",{bubbles:!0,composed:!0,detail:{betData:l,isActive:i,button:o}}))}renderButton(t,s,n,a,r=!1){if(!n)return'<button class="btn btn--empty" disabled></button>';const o=n.price,i=n.point;let l="";return t==="spread"?l=`${i>0?`+${i}`:i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`:t==="money"?l=o:t==="total"&&(l=`${a}${i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`),`<button class="btn ${r?"btn--disabled":""}"
            ...
            ${l}
        </button>`}render(){const t=this.theme==="dark",s=this.oddsMarkets,n=s&&(s.h2h||s.totals||s.spreads),a=this.isMatchInProgressLocked(),r=this.isDisabled||a,o=n,i=this.getGameStatus(),l=this.scoreboard,c=l?.home_score||0,d=l?.away_score||0,u=i?.type==="live"||i?.type==="final",p=s?.spreads?.outcomes?.find(m=>m.name===this.homeTeam),f=s?.spreads?.outcomes?.find(m=>m.name===this.awayTeam),b=s?.h2h?.outcomes?.find(m=>m.name===this.homeTeam),_=s?.h2h?.outcomes?.find(m=>m.name===this.awayTeam),y=s?.totals?.outcomes?.find(m=>m.name==="Over"),h=s?.totals?.outcomes?.find(m=>m.name==="Under");this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: block;
                }
                    ...
                    <div class="row row--home">
                        ...
                        ${o?`
                            <div class="buttons">
                                ...
                                ${this.renderButton("money","home",b,null,r)}
    ...
    `}}customElements.define("bma-match-status",sf);class Yt extends HTMLElement{static currentlyExpanded=null;constructor(){super(),this.attachShadow({mode:"open"}),this.expanded=!1}static get observedAttributes(){return["data-rank","data-username","data-user-guid","data-tournament-dollars","data-total-betslips","data-total-payout","data-combined-betslips","data-user-matches","data-badge-class","data-is-current-user","data-sync-theme"]}get rank(){return this.getAttribute("data-rank")||""}get username(){return this.getAttribute("data-username")||"Unknown"}get userGuid(){return this.getAttribute("data-user-guid")||""}get tournamentDollars(){return this.getAttribute("data-tournament-dollars")||"0"}get totalBetslips(){return this.getAttribute("data-total-betslips")||"0"}get totalPayout(){return this.getAttribute("data-total-payout")||"0"}get combinedBetslips(){const t=this.getAttribute("data-combined-betslips");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing combined_betslips:",s),[]}}get userMatches(){const t=this.getAttribute("data-user-matches");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing user_matches:",s),[]}}get badgeClass(){return this.getAttribute("data-badge-class")||""}get isCurrentUser(){return this.getAttribute("data-is-current-user")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}connectedCallback(){this.render(),this.attachEventListeners()}attributeChangedCallback(t,s,n){s!==n&&(this.render(),this.attachEventListeners())}toggleExpanded(){Yt.currentlyExpanded&&Yt.currentlyExpanded!==this&&Yt.currentlyExpanded.collapse(),this.expanded=!this.expanded;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");if(this.expanded){t.style.display="block",s.textContent="▼",Yt.currentlyExpanded=this;const n=document.querySelector("body > neodigm-sodapop");n&&n.scrollTo(0,0)}else t.style.display="none",s.textContent="▶",Yt.currentlyExpanded===this&&(Yt.currentlyExpanded=null)}collapse(){this.expanded=!1;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");t&&(t.style.display="none"),s&&(s.textContent="▶")}attachEventListeners(){const t=this.shadowRoot.querySelector(".leaderboard-card");t&&t.addEventListener("click",()=>this.toggleExpanded())}renderBetsTable(){const t=this.combinedBetslips,s=this.userMatches;if(!t||t.length===0)return'<p class="no-bets">No bets available</p>';const n=t.filter(i=>{const l=i.bet||[];if(l.length===0)return!1;const c=l[0],d=Object.keys(c).filter(u=>u!=="short_title")[0];return c[d]?.reconciled===!0});if(n.length===0)return'<p class="no-bets">No reconciled bets</p>';const a={};s.forEach(i=>{const l=i.guid||i.id||i.odds_id;l&&(a[l]=i)});const r={};n.forEach(i=>{const l=i.coreMatches__guid||i.match_guid||i.odds_id||"unknown";r[l]||(r[l]={bets:[],matchTitle:"Unknown Match",sportKey:"default"});const c=a[l];if(c){r[l].matchTitle=c.short_title||c.title||"Unknown Match";const d=c.sport_id,u=qt.find(p=>p.key===d);r[l].sportKey=u?.group||"default"}r[l].bets.push(i)});let o="";return Object.keys(r).forEach(i=>{const l=r[i],c=l.sportKey||"default",d=l.matchTitle||"Unknown Match";o+=`
            ...
            `,o+=`
                <table class="bets-table">
                    ...
                    <tbody>
            `,l.bets.forEach(u=>{const p=u.bet||[];if(p.length===0)return;const f=p[0],b=Object.keys(f).filter(h=>h!=="short_title")[0],_=f[b],y=parseFloat(_.payout||0)>0;o+=`
                    <tr class="${y?"bet-win":"bet-loss"}">
                        <td class="bet-selection">${b}</td>
                        <td class="bet-type">${_.type||"-"}</td>
                        <td class="bet-odds">${_.odds>0?"+":""}${_.odds||"-"}</td>
                        <td class="bet-stake">$${parseFloat(_.stake||0).toFixed(2)}</td>
                        <td class="bet-payout ${y?"payout-win":"payout-loss"}">$${parseFloat(_.payout||0).toFixed(2)}</td>
                    </tr>
                `}),o+=`
                    </tbody>
                </table>
  ...
  background-image: var(--badge__icon--bronze);
        ...
        `;const c=document.getElementById("pop-play__caro-info-summary2--id");c&&(c.innerHTML="");const d=document.getElementById("pop-play__caro-info-list--id");d&&a.matches_expanded&&Array.isArray(a.matches_expanded)&&(d.innerHTML=`
            ...
            `+a.matches_expanded.map(u=>{const p=u.scoreboard_data?typeof u.scoreboard_data=="string"?u.scoreboard_data:JSON.stringify(u.scoreboard_data):"";return`
                    ...
                    ></bma-match-status>
                `}).join(""))},async populateLeaderboard(){const e=me(),s=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,n=e.appSession?.session_user?.guid;if(!s){console.warn("[appEvents] Cannot populate leaderboard: no tournament GUID");return}const a=document.getElementById("pop-play__caro-leaderboard--id");if(!a)return;let r=document.getElementById("leaderboard-loading-banner");r||(r=document.createElement("neodigm-juicebar"),r.id="leaderboard-loading-banner",r.setAttribute("role","progressbar"),r.setAttribute("data-n55-theme","info"),r.setAttribute("data-n55-size","small"),r.innerHTML="<div></div>",r.style.cssText=`
                width: 100%;
                margin: 6px;
            `,a.parentNode.insertBefore(r,a)),r.style.visibility="visible";try{const o=await Te.fetchLeaderboard(s),i=o?.data||o?.rows;if(i&&i.length>0){const c=e.coreTourn[e.coreTourn.length-1]?.data.find(b=>b.guid===s);a.innerHTML=i.map((b,_)=>{let y="";if(c?.tags&&Array.isArray(c.tags)){const g=c.tags.find(v=>!!(typeof v=="object"&&v!==null&&(v.entity_guid===b.user_guid&&v.badge&&v.badge.startsWith("--badge__ribbon--")||v[b.user_guid]&&v[b.user_guid].startsWith("--badge__ribbon--"))));if(g){const v=g.badge||g[b.user_guid];v==="--badge__ribbon--gold"?y="badge-trophy--gold":v==="--badge__ribbon--silver"?y="badge-trophy--silver":v==="--badge__ribbon--bronze"&&(y="badge-trophy--bronze")}}const h=b.combined_betslips?typeof b.combined_betslips=="string"?b.combined_betslips:JSON.stringify(b.combined_betslips):"[]";let m="[]";if(b.combined_betslips&&c?.matches_expanded){const g=typeof b.combined_betslips=="string"?JSON.parse(b.combined_betslips):b.combined_betslips,v=[...new Set(g.map(A=>A.coreMatches__guid||A.match_guid||A.odds_id).filter(Boolean))],S=c.matches_expanded.filter(A=>{const k=A.guid||A.id||A.odds_id;return v.includes(k)});m=JSON.stringify(S)}return`
            ...
            `,setTimeout(()=>{r&&(r.style.visibility="hidden")},1800)}},hydratePlayPopup(){const e=me(),s=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(!s||e.coreTourn.length===0)return;const a=e.coreTourn[e.coreTourn.length-1].data.find(r=>r.guid===s);a&&(console.log("[appEvents] Hydrating play popup for tournament:",s),this.populateInfoPage(),this.renderMatchCards(a),this.updateTournamentDashboard(a),this.updatePlayButton())},bindAppListeners(e){W.subscribe("APP",(y,h)=>{console.warn("~~ sub all APP | "+y+"|"+h)}),W.subscribe("WC",(y,h)=>{console.warn("~~ sub all WC  | "+y+"|"+h)}),W.subscribe("ROUTE",(y,h)=>{console.warn("~~ sub all ROUTE  | "+y+"|"+h)});let t="lobby",s="all";const n=()=>{const y=document.querySelector(".featured-carousel-section");y&&(t==="lobby"?y.style.display="block":y.style.display="none")},a=()=>{if(e.coreTourn.length===0)return;const h=e.coreTourn[e.coreTourn.length-1].data,m=e.appSession?.session_user?.guid;document.querySelectorAll(".tournament-item").forEach(S=>S.classList.remove("h-ds__none"));let v=0;if(h.forEach(S=>{const A=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${S.guid}"]`);if(!A)return;const k=A.parentElement;let R=!0;if(t==="my"?(S.entities?.guids||[]).includes(m)||(R=!1):t==="completed"&&(S.status||S.class)!=="COMPLETED"&&(R=!1),s!=="all"&&R){const E=S.sports_allowed||[];s==="multi"?E.length<=1&&(R=!1):E.some(L=>L.key===s)||(R=!1)}R?v++:k&&k.classList.add("h-ds__none")}),console.log(`[app_events] Filter applied - ${t}/${s} - showing ${v} of ${h.length} tournaments`),v===0){let S="No tournaments match the current filters";t==="my"&&(S="You haven't joined any tournaments yet"),t==="completed"&&(S="No completed tournaments yet"),typeof neodigmToast<"u"&&neodigmToast.q(S,"night")}};W.subscribe(e.hierTopics.COREBETSLIP,(y,h)=>{JSON.parse(h);let m=16;switch(y){case e.hierTopics.COREBETSLIP__BET:m=5,qs.shootConfetti(),console.log("~~~  |  "+y+" | ",h);const g=document.querySelector("neodigm-sodapop");g&&g.setAttribute("data-wait","true");const v=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry");if(v.length===0){console.warn("[app_events] No bets to submit"),g&&g.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("No bets to submit","warning");break}const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,k=e.appSession?.session_user?.guid;if(!A||!k){console.error("[app_events] Missing tournament or user GUID"),g&&g.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Missing tournament or user data","danger");break}const R=Array.from(v).map(H=>{const G=H.getAttribute("data-content-team-points"),$=H.getAttribute("data-content-odds"),se=H.getAttribute("data-stake")||"0",ne=H.getAttribute("data-content-type"),ce=H.getAttribute("data-payout")||"0",oe=H.getAttribute("data-match-guid")||"",ve={};return ve[G]={type:ne,stake:parseFloat(se),odds:parseFloat($),payout:parseFloat(ce),reconciled:!1},{acctEntity__guid:k,coreTournaments__guid:A,coreMatches__guid:oe,bet:[ve],status:"PENDING"}});console.log("[app_events] Submitting bet slips:",R),Te.postBetSlips(R).then(H=>{console.log("[app_events] Bet slips posted successfully:",H);const G=R.reduce(($,se)=>{const ne=se.bet[0],ce=Object.keys(ne)[0];return $+(ne[ce]?.stake||0)},0);typeof neodigmToast<"u"&&neodigmToast.q(`Bets placed successfully!|Total Stake: TD$ ${G.toFixed(2)}`,"success"),Te.fetchBetSlips(k,A).then($=>{if($?.rows){const se={timestamp:Date.now(),source:"API",data:$.rows};e.pushcoreBetSlip(se),console.log("[app_events] Refreshed bet slips in store:",$.rows.length,"items")}ke.refreshPlayPopupUI(A,$),g&&g.setAttribute("data-wait","false")}).catch($=>{console.error("[app_events] Error fetching fresh bet slips:",$),g&&g.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Bets placed but failed to refresh|Please close and reopen","warning")})}).catch(H=>{console.error("[app_events] Error posting bet slips:",H),typeof neodigmToast<"u"&&neodigmToast.q("Failed to place bets|Please try again","danger"),g&&g.setAttribute("data-wait","false")});break;case e.hierTopics.COREBETSLIP__SPREAD:break;case e.hierTopics.COREBETSLIP__MONEY:break;case e.hierTopics.COREBETSLIP__TOTAL:break;case e.hierTopics.COREBETSLIP__CLOSE:m=24;break;case e.hierTopics.COREBETSLIP__TDSYNC:if(document.querySelector("#summary-balance")&&h){h=JSON.parse(h);const G=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(e.coreBetSlip[0]?.data&&G){const $=e.coreBetSlip[0].data.filter(I=>I.coreTournaments__guid===G),se=[],ne=[],ce=[];$.forEach(I=>{const V=I.bet||[];if(V.length===0)return;const j=V[0];Object.keys(j).filter(le=>le!=="short_title").forEach(le=>{const T=j[le],w=T?.reconciled!==!1,O=parseFloat(T?.payout||0);w?O===0?ne.push(I):ce.push(I):se.push(I)})});const oe=nt.search(se,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,ve=nt.search(ne,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,$e=oe+ve,xe=nt.search(ce,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,Ae=h.tournament_dollars-$e+xe;ke.updateTDBalance("#summary-balance",Ae)}else ke.updateTDBalance("#summary-balance",h.tournament_dollars)}break;case e.hierTopics.COREBETSLIP__INTERSYNC:let x=0,L=0,P=0;if(e.coreBetSlip.length>0){const G=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(G){const se=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(xe=>xe.coreTournaments__guid===G),ne=[],ce=[],oe=[];se.forEach(xe=>{const Ae=xe.bet||[];if(Ae.length===0)return;const I=Ae[0];Object.keys(I).filter(j=>j!=="short_title").forEach(j=>{const X=I[j],le=X?.reconciled!==!1,T=parseFloat(X?.payout||0);le?T===0?ce.push(xe):oe.push(xe):ne.push(xe)})}),x=nt.search(ne,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,L=nt.search(oe,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0;const ve=nt.search(ce,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,$e=nt.search(oe,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;P=ve+$e}}let z=!0,Q=0;const ie=document.querySelector("[data-bets-valid]");if(ie&&h){h=JSON.parse(h),h.pending_stake_sum=0,h.pending_payout_sum=0,h.bets.length||(z=!1),h.bets.forEach(oe=>{oe.stake=Number(oe.stake),oe.stake?h.pending_stake_sum+=oe.stake:z=!1}),document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry").forEach(oe=>{const ve=parseFloat(oe.getAttribute("data-payout")||"0");h.pending_payout_sum+=ve});const G=x+h.pending_stake_sum,$=L+h.pending_payout_sum;Q=h.tournament_dollars-G-P+L;const ne=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let ce=null;ne&&e.coreTourn.length>0&&(ce=e.coreTourn[e.coreTourn.length-1]?.data?.find(ve=>ve.guid===ne)),ke.updateSummaryAndDashboard(Q,G,$,ce),(h.tournament_dollars<0||Q<0)&&(z=!1),ie.dataset.betsValid=z}break}m&&neodigmWired4Sound&&neodigmWired4Sound.sound(m,"QUITE").vibrate()});const r=()=>{const h=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let m=0;h&&e.coreTourn.length>0&&(m=e.coreTourn[e.coreTourn.length-1].data.find(S=>S.guid===h)?.tournament_dollars||0),W.publish(e.hierTopics.COREBETSLIP__TDSYNC,JSON.stringify({tournament_dollars:m,timestamp:Date.now()}))},o=()=>{const h=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let m=0;h&&e.coreTourn.length>0&&(m=e.coreTourn[e.coreTourn.length-1].data.find(x=>x.guid===h)?.tournament_dollars||0);let g=0,v=0,S=0;if(e.coreBetSlip.length>0&&h){const E=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(H=>H.coreTournaments__guid===h);console.log("[app_events] Filtered bets for tournament:",h,"found:",E.length),console.log("[app_events] Using latest bet slip data index:",e.coreBetSlip.length-1),console.log("[app_events] First bet sample:",E[0]);const x=[],L=[],P=[];E.forEach(H=>{const G=H.bet||[];if(G.length===0)return;const $=G[0];Object.keys($).filter(ne=>ne!=="short_title").forEach(ne=>{const ce=$[ne],oe=ce?.reconciled!==!1,ve=parseFloat(ce?.payout||0);oe?ve===0?L.push(H):P.push(H):x.push(H)})}),console.log("[app_events] Bet categories:",{unreconciled:x.length,reconciledZero:L.length,reconciledNonZero:P.length});const z=nt.search(x,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Q=nt.search(L,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,ie=nt.search(P,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;S=Q+ie,v=nt.search(P,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,g=z,console.log("[app_events] Balance calculation:",{unreconciledStakes:z,lostBetStakes:Q,wonBetStakes:ie,allReconciledStakes:S,displayedStake:g,reconciledPayouts:v,tournamentDollars:m,calculatedBalance:m-g-S+v})}else console.warn("[app_events] No bet data found in Pinia store or missing tournamentGuid");const A=m-g-S+v;let k=null;h&&e.coreTourn.length>0&&(k=e.coreTourn[e.coreTourn.length-1].data.find(E=>E.guid===h)),ke.updateSummaryAndDashboard(A,g,v,k)};W.subscribe(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,(y,h)=>{const g=JSON.parse(h)?.rows||[],v=document.querySelector(".bet-grid__slip-MYBETS");if(!v)return;if(v.innerHTML="",g.length===0){v.innerHTML='<p style="color: var(--app-core-color--gray-6__dark--brand, #969696); text-align: center; padding: 20px;">No active bets</p>';const R=document.querySelector(".bet-grid__slip");R&&R.setAttribute("data-active-bet-tab","BETSLIP"),r();return}g.forEach(R=>{if((R.bet||[]).length===0)return;const x=document.createElement("bma-bet-existing");x.setAttribute("data-corebetslip",JSON.stringify(R)),v.appendChild(x)});const S=document.querySelector(".bet-grid__slip");S&&S.setAttribute("data-active-bet-tab","MYBETS"),me().appCLIFeatures?.features?.disable_existing_bet_btn?.state===!0&&ke.disableExistingBetButtons(),setTimeout(()=>{console.log("[app_events] Calling updateSummaryFromExistingBets after HYDRATE, bet count:",g.length),console.log("[app_events] Pinia store bet count:",e.coreBetSlip[0]?.data?.length||0),o()},200)}),document.addEventListener("click",y=>{const h=y.target;if(h&&h.classList.contains("bet-slip-tab")){const m=h.dataset.betTab,g=document.querySelector(".bet-grid__slip");m&&g&&(g.setAttribute("data-active-bet-tab",m),neodigmWired4Sound&&neodigmWired4Sound.sound(3),console.log("[app_events] Switched to bet tab:",m))}if(h&&h.dataset.publishBetslip){if(document.querySelector("[data-bets-valid]")?.dataset?.betsValid!=="true"){console.warn("[app_events] Cannot submit - bets invalid or button disabled");return}if(h.dataset.processing==="true"){console.warn("[app_events] Bet submission already in progress");return}h.dataset.processing="true";const g=h.dataset.publishBetslip;W.publish(g,JSON.stringify({timestamp:Date.now()})),setTimeout(()=>{delete h.dataset.processing},2e3)}});const i=()=>{const h=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let m=0;h&&e.coreTourn.length>0&&(m=e.coreTourn[e.coreTourn.length-1].data.find(k=>k.guid===h)?.tournament_dollars||0);const g=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry"),v=Array.from(g).map(S=>({teamPoints:S.getAttribute("data-content-team-points"),odds:S.getAttribute("data-content-odds"),stake:S.getAttribute("data-stake")||"0",type:S.getAttribute("data-content-type"),abbreviatedTitle:S.getAttribute("data-abbreviated-title"),scheduledAt:S.getAttribute("data-scheduled-at")}));W.publish(e.hierTopics.COREBETSLIP__INTERSYNC,JSON.stringify({tournament_dollars:m,bets:v,timestamp:Date.now()}))},l=(y,h)=>{const m=JSON.parse(h),g=document.querySelector(".bet-grid__slip-BETSLIP-content");if(!g){console.error("[app_events] Bet slip container not found");return}const v=g.querySelector(".bet-slip__empty");v&&v.remove();let S="";m.type==="total"?S=m.team==="over"?"Over":"Under":m.team==="home"?S=m.homeTeam:m.team==="away"&&(S=m.awayTeam);let A=S;if(m.point)if(m.type==="spread"){const P=parseFloat(m.point)>0?`+${m.point}`:m.point;A=`${S} ${P}`}else m.type==="total"&&(A=`${S} ${m.point}`);const k=document.createElement("bma-bet-entry");k.setAttribute("data-content-team-points",A),k.setAttribute("data-content-odds",m.price||"0"),k.setAttribute("data-content-stake-text","0"),k.setAttribute("data-content-type",m.type||""),k.setAttribute("data-abbreviated-title",m.abbreviatedTitle||`${m.homeTeam} vs ${m.awayTeam}`),k.setAttribute("data-scheduled-at",m.scheduledAt||""),k.setAttribute("data-match-guid",m.matchGuid||""),k.setAttribute("data-home-team",m.homeTeam||""),k.setAttribute("data-away-team",m.awayTeam||""),k.setAttribute("data-bet-team",S||"");const E=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(E&&e.coreTourn.length>0){const z=e.coreTourn[e.coreTourn.length-1].data.find(Q=>Q.guid===E);if(z?.sports_allowed&&z.sports_allowed.length>0){const Q=z.sports_allowed[0];k.setAttribute("data-sport-key",Q.key||"");const ie=qt.find(H=>H.key===Q.key);k.setAttribute("data-sport-group",ie?.group||"")}}g.appendChild(k),console.log("[app_events] Bet entry appended to container:",k,"Container children:",g.children.length),setTimeout(()=>{d(k)},0);const x=document.querySelector(".bet-grid__slip");x&&(x.setAttribute("data-active-bet-tab","BETSLIP"),console.log("[app_events] Switched to BETSLIP tab"));const L=g.querySelectorAll("bma-bet-entry").length;L>=3?requestAnimationFrame(()=>{setTimeout(()=>{const P=document.querySelector(".bet-grid__slip");P&&(console.log("[app_events] Scrolling parent to bottom - bet count:",L,"scrollHeight:",P.scrollHeight,"current scrollTop:",P.scrollTop),P.scrollTo({top:P.scrollHeight,behavior:"smooth"}),setTimeout(()=>{console.log("[app_events] After scroll - scrollTop:",P.scrollTop)},500))},150)}):console.log("[app_events] Skipping scroll - only",L,"bet cards (need 3+)"),setTimeout(()=>{i()},200),console.log("[app_events] Created bet entry:",{teamPoints:A,odds:m.price,type:m.type})};W.subscribe(e.hierTopics.COREBETSLIP__SPREAD,(y,h)=>{l(y,h),c()}),W.subscribe(e.hierTopics.COREBETSLIP__MONEY,(y,h)=>{l(y,h),c()}),W.subscribe(e.hierTopics.COREBETSLIP__TOTAL,(y,h)=>{l(y,h),c()});function c(){const y=window.matchMedia("(orientation: portrait)").matches,h=window.matchMedia("(max-width: 768px)").matches;if(y||h){const m=document.querySelector(".bet-grid__select"),g=document.querySelector(".bet-grid__slip"),v=document.querySelector(".bet-grid__toggle-text");m&&g&&v&&(m.classList.add("collapsed"),g.classList.remove("collapsed"),v.textContent="Close Bet Slip",console.log("[app_events] Bet slip expanded on portrait mode"))}}const d=y=>{const h={sport:{key:y.getAttribute("data-sport-key")||"",group:y.getAttribute("data-sport-group")||""},Match:{scheduled_at:y.getAttribute("data-scheduled-at")||"",home_team_id:y.getAttribute("data-home-team")||"",home_team_score:"",away_team_id:y.getAttribute("data-away-team")||"",away_team_score:""},Bet:{scope:{team_id:y.getAttribute("data-bet-team")||"",over:"",under:""},type:y.getAttribute("data-content-type")||"",odds:y.getAttribute("data-content-odds")||"0",stake:y.getAttribute("data-stake")||"0"}},m=ml.calcPayout(h);y.setAttribute("data-payout",m.toString()),console.log("[app_events] Payout calculated:",{stake:h.Bet.stake,odds:h.Bet.odds,payout:m})};new MutationObserver(y=>{y.forEach(h=>{h.type==="attributes"&&h.attributeName==="data-stake"&&h.target.tagName==="BMA-BET-ENTRY"&&(console.log("[app_events] Stake changed:",h.target.getAttribute("data-stake")),d(h.target),clearTimeout(window.betSlipStakeTimeout),window.betSlipStakeTimeout=setTimeout(()=>{i()},300))})}).observe(document.body,{attributes:!0,attributeFilter:["data-stake"],subtree:!0}),console.log("[app_events] MutationObserver for stake changes initialized"),document.addEventListener("bma-bet-entry-remove",y=>{console.log("[app_events] Removing bet entry:",y.detail);const h=y.target,m=y.detail;h.remove(),setTimeout(()=>{const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(A){const k=me(),R=k.coreTourn[k.coreTourn.length-1];if(R&&R.data){const E=R.data.find(x=>x.guid===A);E&&ke.updateTournamentDashboard(E)}}},100),document.querySelectorAll("bma-bet-match-card").forEach(S=>{(S.shadowRoot?.querySelectorAll(".btn")||[]).forEach(k=>{const R=k.dataset.betType,E=k.dataset.price,x=R===m.type,L=E===m.odds;x&&L&&k.classList.contains("btn--active")&&(k.classList.remove("btn--active"),console.log("[app_events] Deactivated button via X removal:",{btnType:R,btnPrice:E}))})});const v=document.querySelector(".bet-grid__slip-BETSLIP-content");v&&v.children.length,setTimeout(()=>{i()},100)}),W.subscribe(e.hierTopics.CANVAS__HYDRATE__STATUS,(y,h)=>{const m=JSON.parse(h);m&&m.guid&&neodigmUtils&&neodigmUtils.shake(`[data-bma-tourn-guid="${m.guid}"]`)}),W.subscribe(e.hierTopics.WC__APP__FOOT,(y,h)=>{JSON.parse(h);let m=16;switch(y){case e.hierTopics.WC__APP__FOOT__ALL_SPORTS:W.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LOBBY,JSON.stringify({tab:"ALL_SPORTS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_TOURNEYS:W.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_MY,JSON.stringify({tab:"MY_TOURNEYS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__LEADERBOARD:m=24,W.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LEADERBOARD,JSON.stringify({tab:"LEADERBOARD",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_PROFILE:W.publish(e.hierTopics.WC__APP__HEAD_TOP__USER_PROFILE,JSON.stringify({tab:"USER_PROFILE",timestamp:Date.now()}));break}m&&neodigmWired4Sound&&neodigmWired4Sound.sound(m).vibrate()}),W.subscribe(e.hierTopics.WC__APP__HEAD_SPORTS,(y,h)=>{const m=JSON.parse(h);let g=16;y=="WC.APP.HEAD_SPORTS.PREV"||y=="WC.APP.HEAD_SPORTS.NEXT"||y=="WC.APP.HEAD_SPORTS.HEAD_SPORTS_DRAG_START"?g=3:(s=m.key||"all",console.log(`[app_events] Sports filter changed to: ${s} (${m.group})`),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SPORT_FILTER_CHANGE"}))),g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),W.subscribe(e.hierTopics.WC__APP__HEAD_MID,(y,h)=>{JSON.parse(h);let m=16;const g=()=>{s="all";const v=document.querySelector("bma-app-head-sports");v&&v.setAttribute("data-selected-chip","all")};switch(y){case"WC.APP.HEAD_MID.HEAD_MID_LOBBY":t="lobby",g(),n(),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_MY":if(!e.appSession?.session_user?.guid){console.warn("[app_events] No user GUID found, cannot filter tournaments"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to view your tournaments","warning");return}t="my",g(),n(),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_COMPLETED":t="completed",g(),n(),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_LEADERBOARD":t="leaderboard",g(),n(),a(),m=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break;case"WC.APP.HEAD_MID.HEAD_MID_SQUAD":t="squad",g(),n(),a(),m=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break}m&&neodigmWired4Sound&&neodigmWired4Sound.sound(m).vibrate()}),W.subscribe(e.hierTopics.ROUTE__HOME__USER_PROFILE,async(y,h)=>{JSON.parse(h);let m=16;switch(y){case"ROUTE.HOME.USER_PROFILE.INSTALL_PWA":const g=window.deferredPWAPrompt;if(!g){console.log("No install prompt available"),typeof neodigmToast<"u"&&neodigmToast.q("PWA install not available","warning");return}try{g.prompt();const{outcome:S}=await g.userChoice;S==="accepted"?(console.log("PWA installed successfully"),window.deferredPWAPrompt=null,typeof neodigmToast<"u"&&neodigmToast.q("App installed successfully!","success"),m=8,neodigmSodaPop&&neodigmSodaPop.close()):(console.log("PWA install declined"),typeof neodigmToast<"u"&&neodigmToast.q("Install cancelled","night"))}catch(S){console.error("PWA install error:",S),typeof neodigmToast<"u"&&neodigmToast.q("Install failed","danger")}break;case"ROUTE.HOME.USER_PROFILE.TOGGLE_THEME":(()=>{const A=e.appCLIFeatures.features.theme.state==="dark"?"light":"dark";e.setFeaturePersistPub("theme",{state:A});const k=document.getElementById("app");k&&k.setAttribute("data-sync-theme",A),document.body.setAttribute("data-sync-theme",A)})(),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.RESET_PASSWORD":Fa.push({name:"resethash_route"}),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.SIGNOUT":Fa.push({name:"signout_route"}),neodigmSodaPop&&neodigmSodaPop.close();break}m&&neodigmWired4Sound&&neodigmWired4Sound.sound(m).vibrate()});const p=(y,h)=>{const m=y.split(".").pop();console.log("[app_events] SODAPOP_PLAY subscriber - action:",m,"topic:",y),setTimeout(()=>{const g=document.querySelectorAll(".play-cntr .btn-info-sm"),S=document.querySelector(".play-cntr")?.closest("neodigm-sodapop");g.forEach(A=>{const k=A.dataset.publishRouteHome?.split(".").pop()||"";if(k===m){if(console.log("[app_events] Activating button:",k),A.classList.add("btn-info-sm--active"),A.classList.remove("btn-info-sm--inactive"),neodigmCarousel.nav({id:"caro-play",nav:k}),neodigmWired4Sound.sound(3),S){const R=m==="PLAY"?"hidden":"auto";S.style.overflow=R,console.log("[app_events] Set sodapop overflow to:",R)}}else A.classList.add("btn-info-sm--inactive"),A.classList.remove("btn-info-sm--active")})},100)};W.subscribe("ROUTE.HOME.SODAPOP_PLAY.INFO",p);let f=null;W.subscribe("ROUTE.HOME.SODAPOP_PLAY.LEADERBOARD",(y,h)=>{if(p(y),f){console.log("[appEvents] Leaderboard refresh debounced (cooling down)");return}f=setTimeout(()=>{f=null},2e3),console.log("[appEvents] Refreshing leaderboard data"),ke.populateLeaderboard()}),W.subscribe("ROUTE.HOME.SODAPOP_PLAY.PLAY",(y,h)=>{JSON.parse(h);const m=document.getElementById("btn-join__play--id");if(!m){console.warn("[app_events] PLAY button not found");return}const g=m.dataset.requiresJoin==="true",v=m.textContent.trim().toLowerCase();if(console.log("[app_events] PLAY button clicked - text:",v,"requiresJoin:",g),g&&v==="join"){const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,k=me();if(!A){console.error("[app_events] No tournament GUID found");return}console.log("[app_events] Executing JOIN for tournament:",A),W.publish(k.hierTopics.WC__TOURN_ACTION,JSON.stringify({action:"JOIN",tournamentGuid:A,timestamp:Date.now()})),m.textContent="Play",m.dataset.requiresJoin="false",setTimeout(()=>{p(y),console.log("[app_events] JOIN triggered, advancing carousel to PLAY page")},500)}else p(y),console.log("[app_events] Advancing carousel to PLAY page")}),W.subscribe(e.hierTopics.WC__APP__HEAD_TOP,(y,h)=>{JSON.parse(h);let m=10;switch(y){case"WC.APP.HEAD_TOP.USER_PROFILE":m=10,neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_my_profile"),setTimeout(()=>{neodigmCarousel.nav({id:"caro-user_profile",nav:"caro-page__badges"})},1200);break;case"WC.APP.HEAD_TOP.LOGO":const g=e.appSession.session_app.version,v=new Date().getFullYear();g&&neodigmToast&&neodigmToast.q(`${g} 3/15/2026, 10:20:20 PM|© ${v} Bet Max Action`,"night");break}m&&neodigmWired4Sound&&neodigmWired4Sound.sound(m).vibrate()});let b=null,_=null;W.subscribe(e.hierTopics.WC__TOURN_ACTION,(y,h)=>{const m=JSON.parse(h);let g=0;switch(m?.action){case"JOIN":g=512;break;case"PLAY":g=8;break;case"INFO":g=8;break}if(g){b=m?.tournamentGuid,_=m?.action,console.log("[app_events] Storing pending tourn data:",b,_);const v=e.appSession?.session_user?.guid,S=m?.tournamentGuid;v&&S?Te.fetchBetSlips(v,S).then(A=>{if(A?.rows){const k={timestamp:Date.now(),source:"API",data:A.rows};e.pushcoreBetSlip(k),console.log("[app_events] Bet slips fetched:",A.rows.length,"items")}setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play"),setTimeout(()=>{W.publish(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,JSON.stringify(A))},3e3)},g)}).catch(A=>{console.error("[app_events] Error fetching bet slips:",A),setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}):setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}}),W.subscribe(e.hierTopics.WC__TOURN_ACTION,(y,h)=>{const m=JSON.parse(h);let g=16;switch(m?.action){case"FOCUS":g=3;break;case"SPORT_ICON":g=10;break;case"JOIN":g=5;break;case"PLAY":g=16;break}g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),W.subscribe(e.hierTopics.WC__TOURN_ACTION,(y,h)=>{const m=JSON.parse(h);let g=0,v="night";switch(neodigmOpt&&neodigmOpt.EVENT_SOUNDS&&(neodigmOpt.EVENT_SOUNDS=!1),m?.tournamentStatus){case"UPCOMING":v="success";break;case"LOCKED":v="warning";break;case"COMPLETED":v="danger";break}switch(m?.action){case"SPORT_ICON":m?.sportTitle==m?.sportDescription?g=m?.sportTitle:g=m?.sportTitle+"|"+m?.sportDescription;break}g&&neodigmToast&&neodigmToast.q(g,v)}),W.subscribe(e.hierTopics.WC__TOURN_ACTION,async(y,h)=>{const m=JSON.parse(h);if(m?.action==="JOIN"){const v=me().appSession?.session_user?.guid;if(!v){console.error("No entity guid found in session"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to join tournaments","warning");return}const S=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${m.tournamentGuid}"]`);S&&(S.setAttribute("data-bma-tourn-wait","true"),qs.shootConfetti());const A={acctEntityGuid:v,tournamentGuid:m.tournamentGuid};try{const k={method:"POST",body:JSON.stringify(A),headers:Te.genHeaders()};console.log("Posting to:",Te.API_baseURI+"/m5t/"+Te.API_ver+"/coreTournaments/join",A);const E=await(await fetch(Te.API_baseURI+"/m5t/"+Te.API_ver+"/coreTournaments/join",k)).json();if(console.log("Join response:",E),E.ok)typeof neodigmToast<"u"&&neodigmToast.q("Successfully joined tournament!","success"),setTimeout(()=>{const x=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${m.tournamentGuid}"]`);if(x){x.setAttribute("data-bma-tourn-wait","false");const L=x.getAttribute("data-bma-tourn-entities");if(L)try{const P=JSON.parse(L);P.guids.includes(v)||(P.guids.push(v),x.setAttribute("data-bma-tourn-entities",JSON.stringify(P)))}catch(P){console.error("Failed to parse entities:",P)}}},3e3);else{const x=E?.message||"Failed to join tournament";typeof neodigmToast<"u"&&neodigmToast.q(x,"danger");const L=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${m.tournamentGuid}"]`);L&&L.setAttribute("data-bma-tourn-wait","false")}}catch(k){console.error("Join tournament error:",k),typeof neodigmToast<"u"&&neodigmToast.q("Network error joining tournament","danger");const R=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${m.tournamentGuid}"]`);R&&R.setAttribute("data-bma-tourn-wait","false")}}}),W.subscribe(e.hierTopics.ROUTE__HOME_ONMOUNT,async(y,h)=>{console.log("[app_events] ROUTE__HOME_ONMOUNT event received");const m=me();try{const g=await Te.fetchTournaments();console.log("[app_events] Tournaments fetched:",g),g?.rows&&Array.isArray(g.rows)?(m.pushCoreTourn({timestamp:Date.now(),source:"API",data:g.rows}),console.log("[app_events] Pushed to coreTourn, length:",m.coreTourn.length),W.publish(m.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now()}))):console.warn("[app_events] Invalid API response format:",g)}catch(g){console.error("[app_events] Error fetching tournaments:",g),typeof neodigmToast<"u"&&neodigmToast.q("Please Sign In","success")}}),W.subscribe(e.hierTopics.PROMOTION__LOAD,async(y,h)=>{console.log("[app_events] PROMOTION__LOAD event received");try{const m=await Te.fetchPromotions();if(console.log("[app_events] Promotions fetched:",m),m?.ok&&m?.data&&Array.isArray(m.data)){const g=m.data.filter(S=>S.class==="banner"&&S.status==="LIVE");console.log("[app_events] Banner promotions:",g);const v=document.querySelector(".featured-swiper .swiper-wrapper");if(!v){console.warn("[app_events] Swiper wrapper not found");return}v.innerHTML="",g.forEach((S,A)=>{const k=document.createElement("div");k.className="swiper-slide";const R=document.createElement("div");R.className=`featured-card featured-card--${A+1}`;const E=S.hero_img?encodeURI(S.hero_img):"";R.style.backgroundImage=`url("${E}")`,R.setAttribute("data-promotion-hero-img",S.hero_img||""),R.setAttribute("data-promotion-caption",S.caption||""),R.setAttribute("data-promotion-tagline",S.tagline||""),R.setAttribute("data-promotion-toast",S.toast||""),R.setAttribute("data-promotion-topic",S.topic?.topic||""),R.setAttribute("data-promotion-topic-token",S.topic?.token||""),R.setAttribute("data-promotion-marquee",S.maquee||""),k.appendChild(R),v.appendChild(k)}),console.log("[app_events] Created",g.length,"promotion slides"),window.featuredSwiper&&window.featuredSwiper.update()}}catch(m){console.error("[app_events] Error fetching promotions:",m)}}),W.subscribe(e.hierTopics.PROMOTION__CLICK,(y,h)=>{console.log("[app_events] PROMOTION__CLICK event received");try{const m=JSON.parse(h);console.log("[app_events] Promotion clicked:",m),m.toast&&typeof neodigmToast<"u"&&neodigmToast.q(m.toast,"success"),typeof neodigmSodaPop<"u"&&(neodigmSodaPop.setOnAfterOpen(()=>{const g=document.querySelector(".l-promotion #promoHero");g&&m.heroImg&&(g.src=m.heroImg);const v=document.querySelector(".l-promotion #promCaption");v&&m.caption&&(v.textContent=m.caption);const S=document.querySelector(".l-promotion #promoTagline");return S&&m.tagline&&(/<[^>]+>/.test(m.tagline)?S.innerHTML=m.tagline:S.textContent=m.tagline),!0},"sodapop_promotion"),neodigmSodaPop.autoOpen("sodapop_promotion"))}catch(m){console.error("[app_events] Error handling promotion click:",m)}}),W.subscribe(e.hierTopics.ROUTE__HOME_HYDRATE,(y,h)=>{console.log("[app_events] ROUTE__HOME_HYDRATE event received");const m=me();if(m.coreTourn.length===0){console.warn("[app_events] No tournament data in coreTourn array");return}const g=m.coreTourn[m.coreTourn.length-1],v=m.coreTourn.length>1?m.coreTourn[m.coreTourn.length-2]:null;console.log("[app_events] Hydrating with latest data:",g);const S=[];if(v){const E=g.data,x=v.data;E.forEach(L=>{const P=x.find(z=>z.guid===L.guid);P&&P.status!==L.status&&(console.log(`[app_events] Status changed for tournament ${L.guid}: ${P.status} -> ${L.status}`),S.push(L.guid))})}const A=m.appSession?.session_user?.guid,k=[...g.data].sort((E,x)=>{const L=E.status||E.class,P=x.status||x.class,z=E.entities?.guids||[],Q=x.entities?.guids||[],ie=z.includes(A),H=Q.includes(A),G=(Ae,I)=>{const V=new Date(Ae.status_time||0).getTime();return new Date(I.status_time||0).getTime()-V},$=L==="LOCKED"&&ie,se=P==="LOCKED"&&H;if($&&!se)return-1;if(!$&&se)return 1;if($&&se)return G(E,x);const ne=L==="UPCOMING",ce=P==="UPCOMING";if(ne&&!ce)return-1;if(!ne&&ce)return 1;if(ne&&ce)return G(E,x);const oe=L==="COMPLETED"&&ie,ve=P==="COMPLETED"&&H;if(oe&&!ve)return-1;if(!oe&&ve)return 1;if(oe&&ve)return G(E,x);const $e=L==="COMPLETED"&&!ie,xe=P==="COMPLETED"&&!H;return $e&&!xe?1:!$e&&xe?-1:G(E,x)}),R=ke.categorizeTournaments(k,t,s,A);if(console.log(`[app_events] Categorized into ${R.length} categories for filter: ${t}/${s}`),window._homeRouteRenderCards&&typeof window._homeRouteRenderCards=="function"){if(window._homeRouteRenderCards(R,!0),setTimeout(()=>{a(),console.log("[app_events] Re-applied filter after hydration:",t)},100),S.length>0){const E=g.data;S.forEach(x=>{const L=E.find(P=>P.guid===x);if(L){const P=Math.floor(Math.random()*2e3)+1e3;setTimeout(()=>{W.publish(m.hierTopics.CANVAS__HYDRATE__STATUS,JSON.stringify({guid:x,status:L.status,timestamp:Date.now()}))},P)}})}}else console.warn("[app_events] window._homeRouteRenderCards not available")}),W.subscribe(e.hierTopics.SSE__CORE__TOURN_SYNC,(y,h)=>{console.log("[app_events] SSE__CORE__TOURN_SYNC event received:",h);const m=me();try{const g=JSON.parse(h);console.log("[app_events] SSE tournament sync data:",g);const v=JSON.parse(g.msg);if(console.log("[app_events] Parsed tournaments array:",v),!Array.isArray(v)){console.warn("[app_events] SSE msg is not an array:",v);return}m.pushCoreTourn({timestamp:Date.now(),source:"SSE",data:v}),console.log("[app_events] Pushed SSE data to coreTourn, length:",m.coreTourn.length),W.publish(m.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SSE"})),typeof neodigmSodaPop<"u"&&neodigmSodaPop.isOpen()==="sodapop_play"&&(console.log("[SSE] Play popup is open, triggering real-time update"),ke.hydratePlayPopup())}catch(g){console.error("[app_events] Error processing SSE tournament sync:",g)}}),setTimeout(function(){neodigmSodaPop&&neodigmMarquee&&neodigmEnchantedCTA&&(neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{},1e3),neodigmMetronome.init().pause(800),neodigmMarquee.init().pause(1200),neodigmEnchantedCTA.init(),neodigmCarousel.init()}),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{NeodigmClaire.showCanv("neodigm-sodapop",.5).setTheme(["warning","success"]).initCanvOn("neodigm-sodapop").waxOn("neodigm-sodapop");const y=document.querySelector("#caption__my-profile");y&&(y.textContent="My Profile");const h=e.appSession?.session_user?.guid;if(console.log("[app_events] User GUID:",h),console.log("[app_events] coreTourn length:",e.coreTourn.length),!h||e.coreTourn.length===0){console.warn("[app_events] Missing user GUID or tournament data");return}const g=e.coreTourn[e.coreTourn.length-1].data;console.log("[app_events] Processing tournaments:",g.length);let v=0,S=0,A=0;g.forEach((k,R)=>{if(!k.tags||!Array.isArray(k.tags)){console.log(`[app_events] Tournament ${R} has no tags or tags not an array`);return}k.tags.forEach((E,x)=>{if(typeof E=="object"&&E!==null){let L=null;E.entity_guid===h&&E.badge?L=E.badge:E[h]&&(L=E[h]),L==="--badge__ribbon--gold"?v++:L==="--badge__ribbon--silver"?S++:L==="--badge__ribbon--bronze"&&A++}})}),setTimeout(()=>{let k=document.querySelectorAll(".badge-counter");if(k.length===0){const R=document.querySelector("neodigm-sodapop");R&&(k=R.querySelectorAll(".badge-counter"))}k.length>=3?(k[0].textContent=v,k[1].textContent=S,k[2].textContent=A,console.log("[app_events] Updated UI counters successfully")):console.warn("[app_events] Not enough counter elements found")},500)},1e3)},"sodapop_my_profile"),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{const y=me(),h=document.querySelector(".play-cntr");b&&(h.dataset.currentTournGuid=b),_&&(h.dataset.currentTournAction=_);const m=h?.dataset?.currentTournGuid,g=h?.dataset?.currentTournAction||"INFO";console.log("[app_events] sodapop_play opened - GUID:",m,"action:",g),window.initBetGridToggle&&window.initBetGridToggle(),ke.updatePlayButton(),ke.populateInfoPage(),setTimeout(()=>{ke.populateLeaderboard()},600);let v=g==="PLAY"||g==="JOIN"?"PLAY":g;if(!m||y.coreTourn.length===0||y.coreTourn[y.coreTourn.length-1].data.find(z=>z.guid===m)?.status==="COMPLETED"&&(v="LEADERBOARD",console.log("[app_events] Tournament is COMPLETED, defaulting to LEADERBOARD")),v){const L=`ROUTE.HOME.SODAPOP_PLAY.${v}`;console.log("[app_events] Publishing initial topic:",L),W.publish(L,JSON.stringify({tournamentGuid:m,timestamp:Date.now()}))}if(!m||y.coreTourn.length===0)return;const A=y.coreTourn[y.coreTourn.length-1].data.find(L=>L.guid===m);ke.renderMatchCards(A);const k=document.querySelector("#pop-play__tourn-caption--id"),R=document.querySelector("#pop-play__tourn-tagline--id");k&&(k.textContent=A.caption),R&&(R.textContent=A.tagline),console.log("[Modal Debug] Updating dashboard for tournament:",A?.caption);const E=document.querySelector(".head-caption");console.log("[Modal Debug] Modal header found:",!!E),E&&console.log("[Modal Debug] Modal classes:",E.className),ke.updateTournamentDashboard(A);const x=document.getElementById("dashboard-rank");if(x){const L=A.entities?.guids?.length||0;x.innerHTML=`
