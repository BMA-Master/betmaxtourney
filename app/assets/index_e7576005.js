...
/*  Neodigm JuiceBar End  */ 
    ...
    `,this.setupEventListeners()}}customElements.define("bma-bet-entry",ef);class tf extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get observedAttributes(){return["data-match-id","data-match-guid","data-match-title","data-match-short-title","data-scheduled-at","data-home-team","data-away-team","data-odds-markets","data-is-disabled","data-sync-theme","data-scoreboard","data-sport-key","data-tournament-tags"]}get matchId(){return this.getAttribute("data-match-id")||""}get matchGuid(){return this.getAttribute("data-match-guid")||""}get matchTitle(){return this.getAttribute("data-match-title")||""}get shortTitle(){return this.getAttribute("data-match-short-title")||this.matchTitle}get scheduledAt(){return this.getAttribute("data-scheduled-at")||""}get homeTeam(){return this.getAttribute("data-home-team")||"Home"}get awayTeam(){return this.getAttribute("data-away-team")||"Away"}get oddsMarkets(){const t=this.getAttribute("data-odds-markets");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing odds_markets:",s),null}}get isDisabled(){return this.getAttribute("data-is-disabled")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}get scoreboard(){const t=this.getAttribute("data-scoreboard");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing scoreboard:",s),null}}get sportKey(){return this.getAttribute("data-sport-key")||""}get sportGroup(){const t=this.sportKey;if(!t)return"";const s=qt.find(n=>n.key===t);return s?s.group:""}get tournamentTags(){const t=this.getAttribute("data-tournament-tags");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing tournament tags:",s),[]}}isMatchInProgressLocked(){if(!this.tournamentTags.some(o=>o.match_inprogress_lock===!0))return!1;const n=this.scoreboard;if(!n||!n.time_remaining)return!1;const a=n.time_remaining.toLowerCase();return a==="in progress"||a.includes("q")||a.includes("half")||a.includes("period")}connectedCallback(){this.render(),this.startCountdownTimer()}disconnectedCallback(){this.stopCountdownTimer()}startCountdownTimer(){this.stopCountdownTimer(),this.countdownInterval=setInterval(()=>{this.updateCountdown()},6e4)}stopCountdownTimer(){this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null)}updateCountdown(){const t=this.shadowRoot?.querySelector(".countdown");t&&(t.textContent=this.getCountdownText())}attributeChangedCallback(t,s,n){s!==n&&this.render()}formatDate(t){if(!t)return"TBD";let s=new Date(t);return!t.includes("Z")&&!t.match(/[+-]\d{2}:\d{2}$/)&&(s=new Date(t+"Z")),s.toLocaleString(void 0,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}getCountdownText(){if(!this.scheduledAt)return"";const t=new Date,s=new Date(this.scheduledAt);!this.scheduledAt.includes("Z")&&!this.scheduledAt.match(/[+-]\d{2}:\d{2}$/)&&s.setTime(new Date(this.scheduledAt+"Z").getTime());const n=s-t;if(n<=0)return"";const a=Math.floor(n/(1e3*60*60)),r=Math.floor(n%(1e3*60*60)/(1e3*60));return a>24?`${Math.floor(a/24)}d ${a%24}h`:a>0?`${a}h ${r}m`:r>5?`${r} min`:"Starting Soon"}getGameStatus(){const t=this.getCountdownText();if(t)return{type:"upcoming",text:t,color:"#F7C60D"};const s=this.scoreboard;if(s&&s.time_remaining){const n=s.time_remaining.toLowerCase();if(n==="final")return{type:"final",text:"Final",color:"#969696"};if(n==="in progress"||n.includes("q")||n.includes("half")||n.includes("period")){const a=s.home_score!==void 0&&s.home_score!==null||s.away_score!==void 0&&s.away_score!==null,r=s.period!==void 0&&s.period!==null&&s.period!==0;if(a||r)return{type:"live",text:s.time_remaining,color:"#00E676"}}}return{type:"scheduled",text:this.formatDate(this.scheduledAt),color:"#969696"}}handleButtonClick(t,s,n,a,r){t.stopPropagation();const o=t.currentTarget;if(o.classList.contains("btn--disabled")||o.dataset.disabled==="true")return;const i=o.classList.contains("btn--active"),l={matchId:this.matchId,matchGuid:this.matchGuid,matchTitle:this.matchTitle,abbreviatedTitle:this.shortTitle,homeTeam:this.homeTeam,awayTeam:this.awayTeam,scheduledAt:this.scheduledAt,type:s,team:n,price:a,point:r,timestamp:Date.now()};this.dispatchEvent(new CustomEvent("bet-button-click",{bubbles:!0,composed:!0,detail:{betData:l,isActive:i,button:o}}))}renderButton(t,s,n,a,r=!1){if(!n)return'<button class="btn btn--empty" disabled></button>';const o=n.price,i=n.point;let l="";return t==="spread"?l=`${i>0?`+${i}`:i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`:t==="money"?l=o:t==="total"&&(l=`${a}${i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`),`<button class="btn ${r?"btn--disabled":""}"
            ...
            ${l}
        </button>`}render(){const t=this.theme==="dark",s=this.oddsMarkets,n=s&&(s.h2h||s.totals||s.spreads),a=this.isMatchInProgressLocked(),r=this.isDisabled||a,o=n,i=this.getGameStatus(),l=this.scoreboard,c=l?.home_score||0,d=l?.away_score||0,u=i?.type==="live"||i?.type==="final",p=s?.spreads?.outcomes?.find(h=>h.name===this.homeTeam),f=s?.spreads?.outcomes?.find(h=>h.name===this.awayTeam),b=s?.h2h?.outcomes?.find(h=>h.name===this.homeTeam),_=s?.h2h?.outcomes?.find(h=>h.name===this.awayTeam),y=s?.totals?.outcomes?.find(h=>h.name==="Over"),m=s?.totals?.outcomes?.find(h=>h.name==="Under");this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: block;
                }
    ...
    `}}customElements.define("bma-match-status",sf);class Yt extends HTMLElement{static currentlyExpanded=null;constructor(){super(),this.attachShadow({mode:"open"}),this.expanded=!1}static get observedAttributes(){return["data-rank","data-username","data-user-guid","data-tournament-dollars","data-total-betslips","data-total-payout","data-combined-betslips","data-user-matches","data-badge-class","data-is-current-user","data-sync-theme"]}get rank(){return this.getAttribute("data-rank")||""}get username(){return this.getAttribute("data-username")||"Unknown"}get userGuid(){return this.getAttribute("data-user-guid")||""}get tournamentDollars(){return this.getAttribute("data-tournament-dollars")||"0"}get totalBetslips(){return this.getAttribute("data-total-betslips")||"0"}get totalPayout(){return this.getAttribute("data-total-payout")||"0"}get combinedBetslips(){const t=this.getAttribute("data-combined-betslips");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing combined_betslips:",s),[]}}get userMatches(){const t=this.getAttribute("data-user-matches");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing user_matches:",s),[]}}get badgeClass(){return this.getAttribute("data-badge-class")||""}get isCurrentUser(){return this.getAttribute("data-is-current-user")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}connectedCallback(){this.render(),this.attachEventListeners()}attributeChangedCallback(t,s,n){s!==n&&(this.render(),this.attachEventListeners())}toggleExpanded(){Yt.currentlyExpanded&&Yt.currentlyExpanded!==this&&Yt.currentlyExpanded.collapse(),this.expanded=!this.expanded;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");if(this.expanded){t.style.display="block",s.textContent="▼",Yt.currentlyExpanded=this;const n=document.querySelector("body > neodigm-sodapop");n&&n.scrollTo(0,0)}else t.style.display="none",s.textContent="▶",Yt.currentlyExpanded===this&&(Yt.currentlyExpanded=null)}collapse(){this.expanded=!1;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");t&&(t.style.display="none"),s&&(s.textContent="▶")}attachEventListeners(){const t=this.shadowRoot.querySelector(".leaderboard-card");t&&t.addEventListener("click",()=>this.toggleExpanded())}renderBetsTable(){const t=this.combinedBetslips,s=this.userMatches;if(!t||t.length===0)return'<p class="no-bets">No bets available</p>';const n=t.filter(i=>{const l=i.bet||[];if(l.length===0)return!1;const c=l[0],d=Object.keys(c).filter(u=>u!=="short_title")[0];return c[d]?.reconciled===!0});if(n.length===0)return'<p class="no-bets">No reconciled bets</p>';const a={};s.forEach(i=>{const l=i.guid||i.id||i.odds_id;l&&(a[l]=i)});const r={};n.forEach(i=>{const l=i.coreMatches__guid||i.match_guid||i.odds_id||"unknown";r[l]||(r[l]={bets:[],matchTitle:"Unknown Match",sportKey:"default"});const c=a[l];if(c){r[l].matchTitle=c.short_title||c.title||"Unknown Match";const d=c.sport_id,u=qt.find(p=>p.key===d);r[l].sportKey=u?.group||"default"}r[l].bets.push(i)});let o="";return Object.keys(r).forEach(i=>{const l=r[i],c=l.sportKey||"default",d=l.matchTitle||"Unknown Match";o+=`
            ...
            `,o+=`
                <table class="bets-table">
                    ...
                    <tbody>
            `,l.bets.forEach(u=>{const p=u.bet||[];if(p.length===0)return;const f=p[0],b=Object.keys(f).filter(m=>m!=="short_title")[0],_=f[b],y=parseFloat(_.payout||0)>0;o+=`
  ...
  background-image: var(--badge__icon--bronze);
        ...
        `;const c=document.getElementById("pop-play__caro-info-summary2--id");c&&(c.innerHTML="");const d=document.getElementById("pop-play__caro-info-list--id");d&&a.matches_expanded&&Array.isArray(a.matches_expanded)&&(d.innerHTML=`
            ...
            `+a.matches_expanded.map(u=>{const p=u.scoreboard_data?typeof u.scoreboard_data=="string"?u.scoreboard_data:JSON.stringify(u.scoreboard_data):"";return`
                ...
                width: 100%;
                margin: 6px;
            `,a.parentNode.insertBefore(r,a)),r.style.visibility="visible";try{const o=await Se.fetchLeaderboard(s),i=o?.data||o?.rows;if(i&&i.length>0){const c=e.coreTourn[e.coreTourn.length-1]?.data.find(b=>b.guid===s);a.innerHTML=i.map((b,_)=>{let y="";if(c?.tags&&Array.isArray(c.tags)){const g=c.tags.find(v=>!!(typeof v=="object"&&v!==null&&(v.entity_guid===b.user_guid&&v.badge&&v.badge.startsWith("--badge__ribbon--")||v[b.user_guid]&&v[b.user_guid].startsWith("--badge__ribbon--"))));if(g){const v=g.badge||g[b.user_guid];v==="--badge__ribbon--gold"?y="badge-trophy--gold":v==="--badge__ribbon--silver"?y="badge-trophy--silver":v==="--badge__ribbon--bronze"&&(y="badge-trophy--bronze")}}const m=b.combined_betslips?typeof b.combined_betslips=="string"?b.combined_betslips:JSON.stringify(b.combined_betslips):"[]";let h="[]";if(b.combined_betslips&&c?.matches_expanded){const g=typeof b.combined_betslips=="string"?JSON.parse(b.combined_betslips):b.combined_betslips,v=[...new Set(g.map(A=>A.coreMatches__guid||A.match_guid||A.odds_id).filter(Boolean))],S=c.matches_expanded.filter(A=>{const k=A.guid||A.id||A.odds_id;return v.includes(k)});h=JSON.stringify(S)}return`
                        <bma-leaderboard-card
                            ...
                            data-combined-betslips="${m.replace(/"/g,"&quot;")}"
                            data-user-matches="${h.replace(/"/g,"&quot;")}"
                            data-badge-class="${y}"
            ...
            `,setTimeout(()=>{r&&(r.style.visibility="hidden")},1800)}},hydratePlayPopup(){const e=me(),s=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(!s||e.coreTourn.length===0)return;const a=e.coreTourn[e.coreTourn.length-1].data.find(r=>r.guid===s);a&&(console.log("[appEvents] Hydrating play popup for tournament:",s),this.populateInfoPage(),this.renderMatchCards(a),this.updateTournamentDashboard(a),this.updatePlayButton())},bindAppListeners(e){V.subscribe("APP",(y,m)=>{console.warn("~~ sub all APP | "+y+"|"+m)}),V.subscribe("WC",(y,m)=>{console.warn("~~ sub all WC  | "+y+"|"+m)}),V.subscribe("ROUTE",(y,m)=>{console.warn("~~ sub all ROUTE  | "+y+"|"+m)});let t="lobby",s="all";const n=()=>{const y=document.querySelector(".featured-carousel-section");y&&(t==="lobby"?y.style.display="block":y.style.display="none")},a=()=>{if(e.coreTourn.length===0)return;const m=e.coreTourn[e.coreTourn.length-1].data,h=e.appSession?.session_user?.guid;document.querySelectorAll(".tournament-item").forEach(S=>S.classList.remove("h-ds__none"));let v=0;if(m.forEach(S=>{const A=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${S.guid}"]`);if(!A)return;const k=A.parentElement;let R=!0;if(t==="my"?(S.entities?.guids||[]).includes(h)||(R=!1):t==="completed"&&(S.status||S.class)!=="COMPLETED"&&(R=!1),s!=="all"&&R){const E=S.sports_allowed||[];s==="multi"?E.length<=1&&(R=!1):E.some(L=>L.key===s)||(R=!1)}R?v++:k&&k.classList.add("h-ds__none")}),console.log(`[app_events] Filter applied - ${t}/${s} - showing ${v} of ${m.length} tournaments`),v===0){let S="No tournaments match the current filters";t==="my"&&(S="You haven't joined any tournaments yet"),t==="completed"&&(S="No completed tournaments yet"),typeof neodigmToast<"u"&&neodigmToast.q(S,"night")}};V.subscribe(e.hierTopics.COREBETSLIP,(y,m)=>{JSON.parse(m);let h=16;switch(y){case e.hierTopics.COREBETSLIP__BET:h=5,qs.shootConfetti(),console.log("~~~  |  "+y+" | ",m);const g=document.querySelector("[data-bets-valid]");g&&(g.dataset.betsValid="false");const v=document.querySelector("neodigm-sodapop");v&&v.setAttribute("data-wait","true");const S=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry");if(S.length===0){console.warn("[app_events] No bets to submit"),v&&v.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("No bets to submit","warning");break}const k=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,R=e.appSession?.session_user?.guid;if(!k||!R){console.error("[app_events] Missing tournament or user GUID"),v&&v.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Missing tournament or user data","danger");break}const E=Array.from(S).map(H=>{const $=H.getAttribute("data-content-team-points"),te=H.getAttribute("data-content-odds"),ne=H.getAttribute("data-stake")||"0",ie=H.getAttribute("data-content-type"),le=H.getAttribute("data-payout")||"0",ge=H.getAttribute("data-match-guid")||"",Ee={};return Ee[$]={type:ie,stake:parseFloat(ne),odds:parseFloat(te),payout:parseFloat(le),reconciled:!1},{acctEntity__guid:R,coreTournaments__guid:k,coreMatches__guid:ge,bet:[Ee],status:"PENDING"}});console.log("[app_events] Submitting bet slips:",E),Se.postBetSlips(E).then(H=>{console.log("[app_events] Bet slips posted successfully:",H);const $=E.reduce((te,ne)=>{const ie=ne.bet[0],le=Object.keys(ie)[0];return te+(ie[le]?.stake||0)},0);typeof neodigmToast<"u"&&neodigmToast.q(`Bets placed successfully!|Total Stake: TD$ ${$.toFixed(2)}`,"success"),Se.fetchBetSlips(R,k).then(te=>{if(te?.rows){const ne={timestamp:Date.now(),source:"API",data:te.rows};e.pushcoreBetSlip(ne),console.log("[app_events] Refreshed bet slips in store:",te.rows.length,"items")}ke.refreshPlayPopupUI(k,te),v&&v.setAttribute("data-wait","false")}).catch(te=>{console.error("[app_events] Error fetching fresh bet slips:",te);const ne=document.querySelector("[data-publish-betslip]");ne&&delete ne.dataset.processing,g&&(g.dataset.betsValid="true"),v&&v.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Bets placed but failed to refresh|Please close and reopen","warning")})}).catch(H=>{console.error("[app_events] Error posting bet slips:",H);const $=document.querySelector("[data-publish-betslip]");$&&delete $.dataset.processing,g&&(g.dataset.betsValid="true"),typeof neodigmToast<"u"&&neodigmToast.q("Failed to place bets|Please try again","danger"),v&&v.setAttribute("data-wait","false")});break;case e.hierTopics.COREBETSLIP__SPREAD:break;case e.hierTopics.COREBETSLIP__MONEY:break;case e.hierTopics.COREBETSLIP__TOTAL:break;case e.hierTopics.COREBETSLIP__CLOSE:h=24;break;case e.hierTopics.COREBETSLIP__TDSYNC:if(document.querySelector("#summary-balance")&&m){m=JSON.parse(m);const $=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(e.coreBetSlip[0]?.data&&$){const te=e.coreBetSlip[0].data.filter(K=>K.coreTournaments__guid===$),ne=[],ie=[],le=[];te.forEach(K=>{const q=K.bet||[];if(q.length===0)return;const X=q[0];Object.keys(X).filter(T=>T!=="short_title").forEach(T=>{const w=X[T],O=w?.reconciled!==!1,N=parseFloat(w?.payout||0);O?N===0?ie.push(K):le.push(K):ne.push(K)})});const ge=nt.search(ne,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Ee=nt.search(ie,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Ie=ge+Ee,Te=nt.search(le,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,I=m.tournament_dollars-Ie+Te;ke.updateTDBalance("#summary-balance",I)}else ke.updateTDBalance("#summary-balance",m.tournament_dollars)}break;case e.hierTopics.COREBETSLIP__INTERSYNC:let L=0,P=0,z=0;if(e.coreBetSlip.length>0){const $=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if($){const ne=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(Te=>Te.coreTournaments__guid===$),ie=[],le=[],ge=[];ne.forEach(Te=>{const I=Te.bet||[];if(I.length===0)return;const K=I[0];Object.keys(K).filter(X=>X!=="short_title").forEach(X=>{const ce=K[X],T=ce?.reconciled!==!1,w=parseFloat(ce?.payout||0);T?w===0?le.push(Te):ge.push(Te):ie.push(Te)})}),L=nt.search(ie,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,P=nt.search(ge,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0;const Ee=nt.search(le,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Ie=nt.search(ge,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;z=Ee+Ie}}let Q=!0,oe=0;const G=document.querySelector("[data-bets-valid]");if(G&&m){m=JSON.parse(m),m.pending_stake_sum=0,m.pending_payout_sum=0,m.bets.length||(Q=!1),m.bets.forEach(ge=>{ge.stake=Number(ge.stake),ge.stake>0?m.pending_stake_sum+=ge.stake:Q=!1}),document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry").forEach(ge=>{const Ee=parseFloat(ge.getAttribute("data-payout")||"0");m.pending_payout_sum+=Ee});const $=L+m.pending_stake_sum,te=P+m.pending_payout_sum;oe=parseFloat((m.tournament_dollars-$-z+P).toFixed(2));const ie=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let le=null;ie&&e.coreTourn.length>0&&(le=e.coreTourn[e.coreTourn.length-1]?.data?.find(Ee=>Ee.guid===ie)),ke.updateSummaryAndDashboard(oe,$,te,le),(m.tournament_dollars<0||oe<0)&&(Q=!1),G.dataset.betsValid=Q}break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h,"QUITE").vibrate()});const r=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(S=>S.guid===m)?.tournament_dollars||0),V.publish(e.hierTopics.COREBETSLIP__TDSYNC,JSON.stringify({tournament_dollars:h,timestamp:Date.now()}))},o=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(x=>x.guid===m)?.tournament_dollars||0);let g=0,v=0,S=0;if(e.coreBetSlip.length>0&&m){const E=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(G=>G.coreTournaments__guid===m);console.log("[app_events] Filtered bets for tournament:",m,"found:",E.length),console.log("[app_events] Using latest bet slip data index:",e.coreBetSlip.length-1),console.log("[app_events] First bet sample:",E[0]);const x=[],L=[],P=[];E.forEach(G=>{const H=G.bet||[];if(H.length===0)return;const $=H[0];Object.keys($).filter(ne=>ne!=="short_title").forEach(ne=>{const ie=$[ne],le=ie?.reconciled!==!1,ge=parseFloat(ie?.payout||0);le?ge===0?L.push(G):P.push(G):x.push(G)})}),console.log("[app_events] Bet categories:",{unreconciled:x.length,reconciledZero:L.length,reconciledNonZero:P.length});const z=nt.search(x,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Q=nt.search(L,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,oe=nt.search(P,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;S=Q+oe,v=nt.search(P,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,g=z,console.log("[app_events] Balance calculation:",{unreconciledStakes:z,lostBetStakes:Q,wonBetStakes:oe,allReconciledStakes:S,displayedStake:g,reconciledPayouts:v,tournamentDollars:h,calculatedBalance:h-g-S+v})}else console.warn("[app_events] No bet data found in Pinia store or missing tournamentGuid");const A=h-g-S+v;let k=null;m&&e.coreTourn.length>0&&(k=e.coreTourn[e.coreTourn.length-1].data.find(E=>E.guid===m)),ke.updateSummaryAndDashboard(A,g,v,k)};V.subscribe(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,(y,m)=>{const g=JSON.parse(m)?.rows||[],v=document.querySelector(".bet-grid__slip-MYBETS");if(!v)return;if(v.innerHTML="",g.length===0){v.innerHTML='<p style="color: var(--app-core-color--gray-6__dark--brand, #969696); text-align: center; padding: 20px;">No active bets</p>';const R=document.querySelector(".bet-grid__slip");R&&R.setAttribute("data-active-bet-tab","BETSLIP"),r();return}g.forEach(R=>{if((R.bet||[]).length===0)return;const x=document.createElement("bma-bet-existing");x.setAttribute("data-corebetslip",JSON.stringify(R)),v.appendChild(x)});const S=document.querySelector(".bet-grid__slip");S&&S.setAttribute("data-active-bet-tab","MYBETS"),me().appCLIFeatures?.features?.disable_existing_bet_btn?.state===!0&&ke.disableExistingBetButtons(),setTimeout(()=>{console.log("[app_events] Calling updateSummaryFromExistingBets after HYDRATE, bet count:",g.length),console.log("[app_events] Pinia store bet count:",e.coreBetSlip[0]?.data?.length||0),o()},200)}),document.addEventListener("click",y=>{const m=y.target;if(m&&m.classList.contains("bet-slip-tab")){const h=m.dataset.betTab,g=document.querySelector(".bet-grid__slip");h&&g&&(g.setAttribute("data-active-bet-tab",h),neodigmWired4Sound&&neodigmWired4Sound.sound(3),console.log("[app_events] Switched to bet tab:",h))}if(m&&m.dataset.publishBetslip){if(document.querySelector("[data-bets-valid]")?.dataset?.betsValid!=="true"){console.warn("[app_events] Cannot submit - bets invalid or button disabled");return}if(m.dataset.processing==="true"){console.warn("[app_events] Bet submission already in progress");return}m.dataset.processing="true";const g=m.dataset.publishBetslip;V.publish(g,JSON.stringify({timestamp:Date.now()}))}});const i=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(k=>k.guid===m)?.tournament_dollars||0);const g=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry"),v=Array.from(g).map(S=>({teamPoints:S.getAttribute("data-content-team-points"),odds:S.getAttribute("data-content-odds"),stake:S.getAttribute("data-stake")||"0",type:S.getAttribute("data-content-type"),abbreviatedTitle:S.getAttribute("data-abbreviated-title"),scheduledAt:S.getAttribute("data-scheduled-at")}));V.publish(e.hierTopics.COREBETSLIP__INTERSYNC,JSON.stringify({tournament_dollars:h,bets:v,timestamp:Date.now()}))},l=(y,m)=>{const h=JSON.parse(m),g=document.querySelector(".bet-grid__slip-BETSLIP-content");if(!g){console.error("[app_events] Bet slip container not found");return}const v=g.querySelector(".bet-slip__empty");v&&v.remove();let S="";h.type==="total"?S=h.team==="over"?"Over":"Under":h.team==="home"?S=h.homeTeam:h.team==="away"&&(S=h.awayTeam);let A=S;if(h.point)if(h.type==="spread"){const P=parseFloat(h.point)>0?`+${h.point}`:h.point;A=`${S} ${P}`}else h.type==="total"&&(A=`${S} ${h.point}`);const k=document.createElement("bma-bet-entry");k.setAttribute("data-content-team-points",A),k.setAttribute("data-content-odds",h.price||"0"),k.setAttribute("data-content-stake-text","0"),k.setAttribute("data-content-type",h.type||""),k.setAttribute("data-abbreviated-title",h.abbreviatedTitle||`${h.homeTeam} vs ${h.awayTeam}`),k.setAttribute("data-scheduled-at",h.scheduledAt||""),k.setAttribute("data-match-guid",h.matchGuid||""),k.setAttribute("data-home-team",h.homeTeam||""),k.setAttribute("data-away-team",h.awayTeam||""),k.setAttribute("data-bet-team",S||"");const E=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(E&&e.coreTourn.length>0){const z=e.coreTourn[e.coreTourn.length-1].data.find(Q=>Q.guid===E);if(z?.sports_allowed&&z.sports_allowed.length>0){const Q=z.sports_allowed[0];k.setAttribute("data-sport-key",Q.key||"");const oe=qt.find(G=>G.key===Q.key);k.setAttribute("data-sport-group",oe?.group||"")}}g.appendChild(k),console.log("[app_events] Bet entry appended to container:",k,"Container children:",g.children.length),setTimeout(()=>{d(k)},0);const x=document.querySelector(".bet-grid__slip");x&&(x.setAttribute("data-active-bet-tab","BETSLIP"),console.log("[app_events] Switched to BETSLIP tab"));const L=g.querySelectorAll("bma-bet-entry").length;L>=3?requestAnimationFrame(()=>{setTimeout(()=>{const P=document.querySelector(".bet-grid__slip");P&&(console.log("[app_events] Scrolling parent to bottom - bet count:",L,"scrollHeight:",P.scrollHeight,"current scrollTop:",P.scrollTop),P.scrollTo({top:P.scrollHeight,behavior:"smooth"}),setTimeout(()=>{console.log("[app_events] After scroll - scrollTop:",P.scrollTop)},500))},150)}):console.log("[app_events] Skipping scroll - only",L,"bet cards (need 3+)"),setTimeout(()=>{i()},200),console.log("[app_events] Created bet entry:",{teamPoints:A,odds:h.price,type:h.type})};V.subscribe(e.hierTopics.COREBETSLIP__SPREAD,(y,m)=>{l(y,m),c()}),V.subscribe(e.hierTopics.COREBETSLIP__MONEY,(y,m)=>{l(y,m),c()}),V.subscribe(e.hierTopics.COREBETSLIP__TOTAL,(y,m)=>{l(y,m),c()});function c(){const y=window.matchMedia("(orientation: portrait)").matches,m=window.matchMedia("(max-width: 768px)").matches;if(y||m){const h=document.querySelector(".bet-grid__select"),g=document.querySelector(".bet-grid__slip"),v=document.querySelector(".bet-grid__toggle-text");h&&g&&v&&(h.classList.add("collapsed"),g.classList.remove("collapsed"),v.textContent="Close Bet Slip",console.log("[app_events] Bet slip expanded on portrait mode"))}}const d=y=>{const m={sport:{key:y.getAttribute("data-sport-key")||"",group:y.getAttribute("data-sport-group")||""},Match:{scheduled_at:y.getAttribute("data-scheduled-at")||"",home_team_id:y.getAttribute("data-home-team")||"",home_team_score:"",away_team_id:y.getAttribute("data-away-team")||"",away_team_score:""},Bet:{scope:{team_id:y.getAttribute("data-bet-team")||"",over:"",under:""},type:y.getAttribute("data-content-type")||"",odds:y.getAttribute("data-content-odds")||"0",stake:y.getAttribute("data-stake")||"0"}},h=ml.calcPayout(m);y.setAttribute("data-payout",h.toString()),console.log("[app_events] Payout calculated:",{stake:m.Bet.stake,odds:m.Bet.odds,payout:h})};new MutationObserver(y=>{y.forEach(m=>{m.type==="attributes"&&m.attributeName==="data-stake"&&m.target.tagName==="BMA-BET-ENTRY"&&(console.log("[app_events] Stake changed:",m.target.getAttribute("data-stake")),d(m.target),clearTimeout(window.betSlipStakeTimeout),window.betSlipStakeTimeout=setTimeout(()=>{i()},300))})}).observe(document.body,{attributes:!0,attributeFilter:["data-stake"],subtree:!0}),console.log("[app_events] MutationObserver for stake changes initialized"),document.addEventListener("bma-bet-entry-remove",y=>{console.log("[app_events] Removing bet entry:",y.detail);const m=y.target,h=y.detail;m.remove(),setTimeout(()=>{const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(A){const k=me(),R=k.coreTourn[k.coreTourn.length-1];if(R&&R.data){const E=R.data.find(x=>x.guid===A);E&&ke.updateTournamentDashboard(E)}}},100),document.querySelectorAll("bma-bet-match-card").forEach(S=>{(S.shadowRoot?.querySelectorAll(".btn")||[]).forEach(k=>{const R=k.dataset.betType,E=k.dataset.price,x=R===h.type,L=E===h.odds;x&&L&&k.classList.contains("btn--active")&&(k.classList.remove("btn--active"),console.log("[app_events] Deactivated button via X removal:",{btnType:R,btnPrice:E}))})});const v=document.querySelector(".bet-grid__slip-BETSLIP-content");v&&v.children.length,setTimeout(()=>{i()},100)}),V.subscribe(e.hierTopics.CANVAS__HYDRATE__STATUS,(y,m)=>{const h=JSON.parse(m);h&&h.guid&&neodigmUtils&&neodigmUtils.shake(`[data-bma-tourn-guid="${h.guid}"]`)}),V.subscribe(e.hierTopics.WC__APP__FOOT,(y,m)=>{JSON.parse(m);let h=16;switch(y){case e.hierTopics.WC__APP__FOOT__ALL_SPORTS:V.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LOBBY,JSON.stringify({tab:"ALL_SPORTS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_TOURNEYS:V.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_MY,JSON.stringify({tab:"MY_TOURNEYS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__LEADERBOARD:h=24,V.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LEADERBOARD,JSON.stringify({tab:"LEADERBOARD",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_PROFILE:V.publish(e.hierTopics.WC__APP__HEAD_TOP__USER_PROFILE,JSON.stringify({tab:"USER_PROFILE",timestamp:Date.now()}));break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()}),V.subscribe(e.hierTopics.WC__APP__HEAD_SPORTS,(y,m)=>{const h=JSON.parse(m);let g=16;y=="WC.APP.HEAD_SPORTS.PREV"||y=="WC.APP.HEAD_SPORTS.NEXT"||y=="WC.APP.HEAD_SPORTS.HEAD_SPORTS_DRAG_START"?g=3:(s=h.key||"all",console.log(`[app_events] Sports filter changed to: ${s} (${h.group})`),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SPORT_FILTER_CHANGE"}))),g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),V.subscribe(e.hierTopics.WC__APP__HEAD_MID,(y,m)=>{JSON.parse(m);let h=16;const g=()=>{s="all";const v=document.querySelector("bma-app-head-sports");v&&v.setAttribute("data-selected-chip","all")};switch(y){case"WC.APP.HEAD_MID.HEAD_MID_LOBBY":t="lobby",g(),n(),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_MY":if(!e.appSession?.session_user?.guid){console.warn("[app_events] No user GUID found, cannot filter tournaments"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to view your tournaments","warning");return}t="my",g(),n(),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_COMPLETED":t="completed",g(),n(),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_LEADERBOARD":t="leaderboard",g(),n(),a(),h=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break;case"WC.APP.HEAD_MID.HEAD_MID_SQUAD":t="squad",g(),n(),a(),h=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()}),V.subscribe(e.hierTopics.ROUTE__HOME__USER_PROFILE,async(y,m)=>{JSON.parse(m);let h=16;switch(y){case"ROUTE.HOME.USER_PROFILE.INSTALL_PWA":const g=window.deferredPWAPrompt;if(!g){console.log("No install prompt available"),typeof neodigmToast<"u"&&neodigmToast.q("PWA install not available","warning");return}try{g.prompt();const{outcome:S}=await g.userChoice;S==="accepted"?(console.log("PWA installed successfully"),window.deferredPWAPrompt=null,typeof neodigmToast<"u"&&neodigmToast.q("App installed successfully!","success"),h=8,neodigmSodaPop&&neodigmSodaPop.close()):(console.log("PWA install declined"),typeof neodigmToast<"u"&&neodigmToast.q("Install cancelled","night"))}catch(S){console.error("PWA install error:",S),typeof neodigmToast<"u"&&neodigmToast.q("Install failed","danger")}break;case"ROUTE.HOME.USER_PROFILE.TOGGLE_THEME":(()=>{const A=e.appCLIFeatures.features.theme.state==="dark"?"light":"dark";e.setFeaturePersistPub("theme",{state:A});const k=document.getElementById("app");k&&k.setAttribute("data-sync-theme",A),document.body.setAttribute("data-sync-theme",A)})(),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.RESET_PASSWORD":Fa.push({name:"resethash_route"}),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.SIGNOUT":Fa.push({name:"signout_route"}),neodigmSodaPop&&neodigmSodaPop.close();break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()});const p=(y,m)=>{const h=y.split(".").pop();console.log("[app_events] SODAPOP_PLAY subscriber - action:",h,"topic:",y),setTimeout(()=>{const g=document.querySelectorAll(".play-cntr .btn-info-sm"),S=document.querySelector(".play-cntr")?.closest("neodigm-sodapop");g.forEach(A=>{const k=A.dataset.publishRouteHome?.split(".").pop()||"";if(k===h){if(console.log("[app_events] Activating button:",k),A.classList.add("btn-info-sm--active"),A.classList.remove("btn-info-sm--inactive"),neodigmCarousel.nav({id:"caro-play",nav:k}),neodigmWired4Sound.sound(3),S){const R=h==="PLAY"?"hidden":"auto";S.style.overflow=R,console.log("[app_events] Set sodapop overflow to:",R)}}else A.classList.add("btn-info-sm--inactive"),A.classList.remove("btn-info-sm--active")})},100)};V.subscribe("ROUTE.HOME.SODAPOP_PLAY.INFO",p);let f=null;V.subscribe("ROUTE.HOME.SODAPOP_PLAY.LEADERBOARD",(y,m)=>{if(p(y),f){console.log("[appEvents] Leaderboard refresh debounced (cooling down)");return}f=setTimeout(()=>{f=null},2e3),console.log("[appEvents] Refreshing leaderboard data"),ke.populateLeaderboard()}),V.subscribe("ROUTE.HOME.SODAPOP_PLAY.PLAY",(y,m)=>{JSON.parse(m);const h=document.getElementById("btn-join__play--id");if(!h){console.warn("[app_events] PLAY button not found");return}const g=h.dataset.requiresJoin==="true",v=h.textContent.trim().toLowerCase();if(console.log("[app_events] PLAY button clicked - text:",v,"requiresJoin:",g),g&&v==="join"){const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,k=me();if(!A){console.error("[app_events] No tournament GUID found");return}console.log("[app_events] Executing JOIN for tournament:",A),V.publish(k.hierTopics.WC__TOURN_ACTION,JSON.stringify({action:"JOIN",tournamentGuid:A,timestamp:Date.now()})),h.textContent="Play",h.dataset.requiresJoin="false",setTimeout(()=>{p(y),console.log("[app_events] JOIN triggered, advancing carousel to PLAY page")},500)}else p(y),console.log("[app_events] Advancing carousel to PLAY page")}),V.subscribe(e.hierTopics.WC__APP__HEAD_TOP,(y,m)=>{JSON.parse(m);let h=10;switch(y){case"WC.APP.HEAD_TOP.USER_PROFILE":h=10,neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_my_profile"),setTimeout(()=>{neodigmCarousel.nav({id:"caro-user_profile",nav:"caro-page__badges"})},1200);break;case"WC.APP.HEAD_TOP.LOGO":const g=e.appSession.session_app.version,v=new Date().getFullYear();g&&neodigmToast&&neodigmToast.q(`${g} 3/20/2026, 12:27:54 PM|© ${v} Bet Max Action`,"night");break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()});let b=null,_=null;V.subscribe(e.hierTopics.WC__TOURN_ACTION,(y,m)=>{const h=JSON.parse(m);let g=0;switch(h?.action){case"JOIN":g=512;break;case"PLAY":g=8;break;case"INFO":g=8;break}if(g){b=h?.tournamentGuid,_=h?.action,console.log("[app_events] Storing pending tourn data:",b,_);const v=e.appSession?.session_user?.guid,S=h?.tournamentGuid;v&&S?Se.fetchBetSlips(v,S).then(A=>{if(A?.rows){const k={timestamp:Date.now(),source:"API",data:A.rows};e.pushcoreBetSlip(k),console.log("[app_events] Bet slips fetched:",A.rows.length,"items")}setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play"),setTimeout(()=>{V.publish(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,JSON.stringify(A))},3e3)},g)}).catch(A=>{console.error("[app_events] Error fetching bet slips:",A),setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}):setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}}),V.subscribe(e.hierTopics.WC__TOURN_ACTION,(y,m)=>{const h=JSON.parse(m);let g=16;switch(h?.action){case"FOCUS":g=3;break;case"SPORT_ICON":g=10;break;case"JOIN":g=5;break;case"PLAY":g=16;break}g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),V.subscribe(e.hierTopics.WC__TOURN_ACTION,(y,m)=>{const h=JSON.parse(m);let g=0,v="night";switch(neodigmOpt&&neodigmOpt.EVENT_SOUNDS&&(neodigmOpt.EVENT_SOUNDS=!1),h?.tournamentStatus){case"UPCOMING":v="success";break;case"LOCKED":v="warning";break;case"COMPLETED":v="danger";break}switch(h?.action){case"SPORT_ICON":h?.sportTitle==h?.sportDescription?g=h?.sportTitle:g=h?.sportTitle+"|"+h?.sportDescription;break}g&&neodigmToast&&neodigmToast.q(g,v)}),V.subscribe(e.hierTopics.WC__TOURN_ACTION,async(y,m)=>{const h=JSON.parse(m);if(h?.action==="JOIN"){const v=me().appSession?.session_user?.guid;if(!v){console.error("No entity guid found in session"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to join tournaments","warning");return}const S=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);S&&(S.setAttribute("data-bma-tourn-wait","true"),qs.shootConfetti());const A={acctEntityGuid:v,tournamentGuid:h.tournamentGuid};try{const k={method:"POST",body:JSON.stringify(A),headers:Se.genHeaders()};console.log("Posting to:",Se.API_baseURI+"/m5t/"+Se.API_ver+"/coreTournaments/join",A);const E=await(await fetch(Se.API_baseURI+"/m5t/"+Se.API_ver+"/coreTournaments/join",k)).json();if(console.log("Join response:",E),E.ok)typeof neodigmToast<"u"&&neodigmToast.q("Successfully joined tournament!","success"),setTimeout(()=>{const x=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);if(x){x.setAttribute("data-bma-tourn-wait","false");const L=x.getAttribute("data-bma-tourn-entities");if(L)try{const P=JSON.parse(L);P.guids.includes(v)||(P.guids.push(v),x.setAttribute("data-bma-tourn-entities",JSON.stringify(P)))}catch(P){console.error("Failed to parse entities:",P)}}},3e3);else{const x=E?.message||"Failed to join tournament";typeof neodigmToast<"u"&&neodigmToast.q(x,"danger");const L=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);L&&L.setAttribute("data-bma-tourn-wait","false")}}catch(k){console.error("Join tournament error:",k),typeof neodigmToast<"u"&&neodigmToast.q("Network error joining tournament","danger");const R=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);R&&R.setAttribute("data-bma-tourn-wait","false")}}}),V.subscribe(e.hierTopics.ROUTE__HOME_ONMOUNT,async(y,m)=>{console.log("[app_events] ROUTE__HOME_ONMOUNT event received");const h=me();try{const g=await Se.fetchTournaments();console.log("[app_events] Tournaments fetched:",g),g?.rows&&Array.isArray(g.rows)?(h.pushCoreTourn({timestamp:Date.now(),source:"API",data:g.rows}),console.log("[app_events] Pushed to coreTourn, length:",h.coreTourn.length),V.publish(h.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now()}))):console.warn("[app_events] Invalid API response format:",g)}catch(g){console.error("[app_events] Error fetching tournaments:",g),typeof neodigmToast<"u"&&neodigmToast.q("Please Sign In","success")}}),V.subscribe(e.hierTopics.PROMOTION__LOAD,async(y,m)=>{console.log("[app_events] PROMOTION__LOAD event received");try{const h=await Se.fetchPromotions();if(console.log("[app_events] Promotions fetched:",h),h?.ok&&h?.data&&Array.isArray(h.data)){const g=h.data.filter(S=>S.class==="banner"&&S.status==="LIVE");console.log("[app_events] Banner promotions:",g);const v=document.querySelector(".featured-swiper .swiper-wrapper");if(!v){console.warn("[app_events] Swiper wrapper not found");return}v.innerHTML="",g.forEach((S,A)=>{const k=document.createElement("div");k.className="swiper-slide";const R=document.createElement("div");R.className=`featured-card featured-card--${A+1}`;const E=S.hero_img?encodeURI(S.hero_img):"";R.style.backgroundImage=`url("${E}")`,R.setAttribute("data-promotion-hero-img",S.hero_img||""),R.setAttribute("data-promotion-caption",S.caption||""),R.setAttribute("data-promotion-tagline",S.tagline||""),R.setAttribute("data-promotion-toast",S.toast||""),R.setAttribute("data-promotion-topic",S.topic?.topic||""),R.setAttribute("data-promotion-topic-token",S.topic?.token||""),R.setAttribute("data-promotion-marquee",S.maquee||""),k.appendChild(R),v.appendChild(k)}),console.log("[app_events] Created",g.length,"promotion slides"),window.featuredSwiper&&window.featuredSwiper.update()}}catch(h){console.error("[app_events] Error fetching promotions:",h)}}),V.subscribe(e.hierTopics.PROMOTION__CLICK,(y,m)=>{console.log("[app_events] PROMOTION__CLICK event received");try{const h=JSON.parse(m);console.log("[app_events] Promotion clicked:",h),h.toast&&typeof neodigmToast<"u"&&neodigmToast.q(h.toast,"success"),typeof neodigmSodaPop<"u"&&(neodigmSodaPop.setOnAfterOpen(()=>{const g=document.querySelector(".l-promotion #promoHero");g&&h.heroImg&&(g.src=h.heroImg);const v=document.querySelector(".l-promotion #promCaption");v&&h.caption&&(v.textContent=h.caption);const S=document.querySelector(".l-promotion #promoTagline");return S&&h.tagline&&(/<[^>]+>/.test(h.tagline)?S.innerHTML=h.tagline:S.textContent=h.tagline),!0},"sodapop_promotion"),neodigmSodaPop.autoOpen("sodapop_promotion"))}catch(h){console.error("[app_events] Error handling promotion click:",h)}}),V.subscribe(e.hierTopics.ROUTE__HOME_HYDRATE,(y,m)=>{console.log("[app_events] ROUTE__HOME_HYDRATE event received");const h=me();if(h.coreTourn.length===0){console.warn("[app_events] No tournament data in coreTourn array");return}const g=h.coreTourn[h.coreTourn.length-1],v=h.coreTourn.length>1?h.coreTourn[h.coreTourn.length-2]:null;console.log("[app_events] Hydrating with latest data:",g);const S=[];if(v){const E=g.data,x=v.data;E.forEach(L=>{const P=x.find(z=>z.guid===L.guid);P&&P.status!==L.status&&(console.log(`[app_events] Status changed for tournament ${L.guid}: ${P.status} -> ${L.status}`),S.push(L.guid))})}const A=h.appSession?.session_user?.guid,k=[...g.data].sort((E,x)=>{const L=E.status||E.class,P=x.status||x.class,z=E.entities?.guids||[],Q=x.entities?.guids||[],oe=z.includes(A),G=Q.includes(A),H=(Te,I)=>{const K=new Date(Te.status_time||0).getTime();return new Date(I.status_time||0).getTime()-K},$=L==="LOCKED"&&oe,te=P==="LOCKED"&&G;if($&&!te)return-1;if(!$&&te)return 1;if($&&te)return H(E,x);const ne=L==="UPCOMING",ie=P==="UPCOMING";if(ne&&!ie)return-1;if(!ne&&ie)return 1;if(ne&&ie)return H(E,x);const le=L==="COMPLETED"&&oe,ge=P==="COMPLETED"&&G;if(le&&!ge)return-1;if(!le&&ge)return 1;if(le&&ge)return H(E,x);const Ee=L==="COMPLETED"&&!oe,Ie=P==="COMPLETED"&&!G;return Ee&&!Ie?1:!Ee&&Ie?-1:H(E,x)}),R=ke.categorizeTournaments(k,t,s,A);if(console.log(`[app_events] Categorized into ${R.length} categories for filter: ${t}/${s}`),window._homeRouteRenderCards&&typeof window._homeRouteRenderCards=="function"){if(window._homeRouteRenderCards(R,!0),setTimeout(()=>{a(),console.log("[app_events] Re-applied filter after hydration:",t)},100),S.length>0){const E=g.data;S.forEach(x=>{const L=E.find(P=>P.guid===x);if(L){const P=Math.floor(Math.random()*2e3)+1e3;setTimeout(()=>{V.publish(h.hierTopics.CANVAS__HYDRATE__STATUS,JSON.stringify({guid:x,status:L.status,timestamp:Date.now()}))},P)}})}}else console.warn("[app_events] window._homeRouteRenderCards not available")}),V.subscribe(e.hierTopics.SSE__CORE__TOURN_SYNC,(y,m)=>{console.log("[app_events] SSE__CORE__TOURN_SYNC event received:",m);const h=me();try{const g=JSON.parse(m);console.log("[app_events] SSE tournament sync data:",g);const v=JSON.parse(g.msg);if(console.log("[app_events] Parsed tournaments array:",v),!Array.isArray(v)){console.warn("[app_events] SSE msg is not an array:",v);return}h.pushCoreTourn({timestamp:Date.now(),source:"SSE",data:v}),console.log("[app_events] Pushed SSE data to coreTourn, length:",h.coreTourn.length),V.publish(h.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SSE"})),typeof neodigmSodaPop<"u"&&neodigmSodaPop.isOpen()==="sodapop_play"&&(console.log("[SSE] Play popup is open, triggering real-time update"),ke.hydratePlayPopup())}catch(g){console.error("[app_events] Error processing SSE tournament sync:",g)}}),setTimeout(function(){neodigmSodaPop&&neodigmMarquee&&neodigmEnchantedCTA&&(neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{},1e3),neodigmMetronome.init().pause(800),neodigmMarquee.init().pause(1200),neodigmEnchantedCTA.init(),neodigmCarousel.init()}),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{NeodigmClaire.showCanv("neodigm-sodapop",.5).setTheme(["warning","success"]).initCanvOn("neodigm-sodapop").waxOn("neodigm-sodapop");const y=document.querySelector("#caption__my-profile");y&&(y.textContent="My Profile");const m=e.appSession?.session_user?.guid;if(console.log("[app_events] User GUID:",m),console.log("[app_events] coreTourn length:",e.coreTourn.length),!m||e.coreTourn.length===0){console.warn("[app_events] Missing user GUID or tournament data");return}const g=e.coreTourn[e.coreTourn.length-1].data;console.log("[app_events] Processing tournaments:",g.length);let v=0,S=0,A=0;g.forEach((k,R)=>{if(!k.tags||!Array.isArray(k.tags)){console.log(`[app_events] Tournament ${R} has no tags or tags not an array`);return}k.tags.forEach((E,x)=>{if(typeof E=="object"&&E!==null){let L=null;E.entity_guid===m&&E.badge?L=E.badge:E[m]&&(L=E[m]),L==="--badge__ribbon--gold"?v++:L==="--badge__ribbon--silver"?S++:L==="--badge__ribbon--bronze"&&A++}})}),setTimeout(()=>{let k=document.querySelectorAll(".badge-counter");if(k.length===0){const R=document.querySelector("neodigm-sodapop");R&&(k=R.querySelectorAll(".badge-counter"))}k.length>=3?(k[0].textContent=v,k[1].textContent=S,k[2].textContent=A,console.log("[app_events] Updated UI counters successfully")):console.warn("[app_events] Not enough counter elements found")},500)},1e3)},"sodapop_my_profile"),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{const y=me(),m=document.querySelector(".play-cntr");b&&(m.dataset.currentTournGuid=b),_&&(m.dataset.currentTournAction=_);const h=m?.dataset?.currentTournGuid,g=m?.dataset?.currentTournAction||"INFO";console.log("[app_events] sodapop_play opened - GUID:",h,"action:",g),window.initBetGridToggle&&window.initBetGridToggle(),ke.updatePlayButton(),ke.populateInfoPage(),setTimeout(()=>{ke.populateLeaderboard()},600);let v=g==="PLAY"||g==="JOIN"?"PLAY":g;if(!h||y.coreTourn.length===0||y.coreTourn[y.coreTourn.length-1].data.find(z=>z.guid===h)?.status==="COMPLETED"&&(v="LEADERBOARD",console.log("[app_events] Tournament is COMPLETED, defaulting to LEADERBOARD")),v){const L=`ROUTE.HOME.SODAPOP_PLAY.${v}`;console.log("[app_events] Publishing initial topic:",L),V.publish(L,JSON.stringify({tournamentGuid:h,timestamp:Date.now()}))}if(!h||y.coreTourn.length===0)return;const A=y.coreTourn[y.coreTourn.length-1].data.find(L=>L.guid===h);ke.renderMatchCards(A);const k=document.querySelector("#pop-play__tourn-caption--id"),R=document.querySelector("#pop-play__tourn-tagline--id");k&&(k.textContent=A.caption),R&&(R.textContent=A.tagline),console.log("[Modal Debug] Updating dashboard for tournament:",A?.caption);const E=document.querySelector(".head-caption");console.log("[Modal Debug] Modal header found:",!!E),E&&console.log("[Modal Debug] Modal classes:",E.className),ke.updateTournamentDashboard(A);const x=document.getElementById("dashboard-rank");if(x){const L=A.entities?.guids?.length||0;x.innerHTML=`
