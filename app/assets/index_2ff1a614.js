...
/*  Neodigm JuiceBar End  */ 
    ...
    `,this.setupEventListeners()}}customElements.define("bma-bet-entry",ef);class tf extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get observedAttributes(){return["data-match-id","data-match-guid","data-match-title","data-match-short-title","data-scheduled-at","data-home-team","data-away-team","data-odds-markets","data-is-disabled","data-sync-theme","data-scoreboard","data-sport-key","data-tournament-tags"]}get matchId(){return this.getAttribute("data-match-id")||""}get matchGuid(){return this.getAttribute("data-match-guid")||""}get matchTitle(){return this.getAttribute("data-match-title")||""}get shortTitle(){return this.getAttribute("data-match-short-title")||this.matchTitle}get scheduledAt(){return this.getAttribute("data-scheduled-at")||""}get homeTeam(){return this.getAttribute("data-home-team")||"Home"}get awayTeam(){return this.getAttribute("data-away-team")||"Away"}get oddsMarkets(){const t=this.getAttribute("data-odds-markets");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing odds_markets:",s),null}}get isDisabled(){return this.getAttribute("data-is-disabled")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}get scoreboard(){const t=this.getAttribute("data-scoreboard");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing scoreboard:",s),null}}get sportKey(){return this.getAttribute("data-sport-key")||""}get sportGroup(){const t=this.sportKey;if(!t)return"";const s=qt.find(n=>n.key===t);return s?s.group:""}get tournamentTags(){const t=this.getAttribute("data-tournament-tags");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing tournament tags:",s),[]}}isMatchInProgressLocked(){if(!this.tournamentTags.some(o=>o.match_inprogress_lock===!0))return!1;const n=this.scoreboard;if(!n||!n.time_remaining)return!1;const a=n.time_remaining.toLowerCase();return a==="in progress"||a.includes("q")||a.includes("half")||a.includes("period")}connectedCallback(){this.render(),this.startCountdownTimer()}disconnectedCallback(){this.stopCountdownTimer()}startCountdownTimer(){this.stopCountdownTimer(),this.countdownInterval=setInterval(()=>{this.updateCountdown()},6e4)}stopCountdownTimer(){this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null)}updateCountdown(){const t=this.shadowRoot?.querySelector(".countdown");t&&(t.textContent=this.getCountdownText())}attributeChangedCallback(t,s,n){s!==n&&this.render()}formatDate(t){if(!t)return"TBD";let s=new Date(t);return!t.includes("Z")&&!t.match(/[+-]\d{2}:\d{2}$/)&&(s=new Date(t+"Z")),s.toLocaleString(void 0,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}getCountdownText(){if(!this.scheduledAt)return"";const t=new Date,s=new Date(this.scheduledAt);!this.scheduledAt.includes("Z")&&!this.scheduledAt.match(/[+-]\d{2}:\d{2}$/)&&s.setTime(new Date(this.scheduledAt+"Z").getTime());const n=s-t;if(n<=0)return"";const a=Math.floor(n/(1e3*60*60)),r=Math.floor(n%(1e3*60*60)/(1e3*60));return a>24?`${Math.floor(a/24)}d ${a%24}h`:a>0?`${a}h ${r}m`:r>5?`${r} min`:"Starting Soon"}getGameStatus(){const t=this.getCountdownText();if(t)return{type:"upcoming",text:t,color:"#F7C60D"};const s=this.scoreboard;if(s&&s.time_remaining){const n=s.time_remaining.toLowerCase();if(n==="final")return{type:"final",text:"Final",color:"#969696"};if(n==="in progress"||n.includes("q")||n.includes("half")||n.includes("period")){const a=s.home_score!==void 0&&s.home_score!==null||s.away_score!==void 0&&s.away_score!==null,r=s.period!==void 0&&s.period!==null&&s.period!==0;if(a||r)return{type:"live",text:s.time_remaining,color:"#00E676"}}}return{type:"scheduled",text:this.formatDate(this.scheduledAt),color:"#969696"}}handleButtonClick(t,s,n,a,r){t.stopPropagation();const o=t.currentTarget;if(o.classList.contains("btn--disabled")||o.dataset.disabled==="true")return;const i=o.classList.contains("btn--active"),l={matchId:this.matchId,matchGuid:this.matchGuid,matchTitle:this.matchTitle,abbreviatedTitle:this.shortTitle,homeTeam:this.homeTeam,awayTeam:this.awayTeam,scheduledAt:this.scheduledAt,type:s,team:n,price:a,point:r,timestamp:Date.now()};this.dispatchEvent(new CustomEvent("bet-button-click",{bubbles:!0,composed:!0,detail:{betData:l,isActive:i,button:o}}))}renderButton(t,s,n,a,r=!1){if(!n)return'<button class="btn btn--empty" disabled></button>';const o=n.price,i=n.point;let l="";return t==="spread"?l=`${i>0?`+${i}`:i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`:t==="money"?l=o:t==="total"&&(l=`${a}${i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`),`<button class="btn ${r?"btn--disabled":""}"
            ...
            ${l}
        </button>`}render(){const t=this.theme==="dark",s=this.oddsMarkets,n=s&&(s.h2h||s.totals||s.spreads),a=this.isMatchInProgressLocked(),r=this.isDisabled||a,o=n,i=this.getGameStatus(),l=this.scoreboard,c=l?.home_score||0,d=l?.away_score||0,u=i?.type==="live"||i?.type==="final",p=s?.spreads?.outcomes?.find(h=>h.name===this.homeTeam),f=s?.spreads?.outcomes?.find(h=>h.name===this.awayTeam),_=s?.h2h?.outcomes?.find(h=>h.name===this.homeTeam),y=s?.h2h?.outcomes?.find(h=>h.name===this.awayTeam),b=s?.totals?.outcomes?.find(h=>h.name==="Over"),m=s?.totals?.outcomes?.find(h=>h.name==="Under");this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: block;
                }
                    ...
                    <div class="row row--home">
                        ...
                        ${o?`
                            <div class="buttons">
                                ...
                                ${this.renderButton("money","home",_,null,r)}
                                ${this.renderButton("total","over",b,"O",r)}
                            </div>
                        `:""}
                    </div>
    ...
    `}}customElements.define("bma-match-status",sf);class Yt extends HTMLElement{static currentlyExpanded=null;constructor(){super(),this.attachShadow({mode:"open"}),this.expanded=!1}static get observedAttributes(){return["data-rank","data-username","data-user-guid","data-tournament-dollars","data-total-betslips","data-total-payout","data-combined-betslips","data-user-matches","data-badge-class","data-is-current-user","data-sync-theme"]}get rank(){return this.getAttribute("data-rank")||""}get username(){return this.getAttribute("data-username")||"Unknown"}get userGuid(){return this.getAttribute("data-user-guid")||""}get tournamentDollars(){return this.getAttribute("data-tournament-dollars")||"0"}get totalBetslips(){return this.getAttribute("data-total-betslips")||"0"}get totalPayout(){return this.getAttribute("data-total-payout")||"0"}get combinedBetslips(){const t=this.getAttribute("data-combined-betslips");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing combined_betslips:",s),[]}}get userMatches(){const t=this.getAttribute("data-user-matches");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing user_matches:",s),[]}}get badgeClass(){return this.getAttribute("data-badge-class")||""}get isCurrentUser(){return this.getAttribute("data-is-current-user")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}connectedCallback(){this.render(),this.attachEventListeners()}attributeChangedCallback(t,s,n){s!==n&&(this.render(),this.attachEventListeners())}toggleExpanded(){Yt.currentlyExpanded&&Yt.currentlyExpanded!==this&&Yt.currentlyExpanded.collapse(),this.expanded=!this.expanded;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");if(this.expanded){t.style.display="block",s.textContent="▼",Yt.currentlyExpanded=this;const n=document.querySelector("body > neodigm-sodapop");n&&n.scrollTo(0,0)}else t.style.display="none",s.textContent="▶",Yt.currentlyExpanded===this&&(Yt.currentlyExpanded=null)}collapse(){this.expanded=!1;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");t&&(t.style.display="none"),s&&(s.textContent="▶")}attachEventListeners(){const t=this.shadowRoot.querySelector(".leaderboard-card");t&&t.addEventListener("click",()=>this.toggleExpanded())}renderBetsTable(){const t=this.combinedBetslips,s=this.userMatches;if(!t||t.length===0)return'<p class="no-bets">No bets available</p>';const n=t.filter(i=>{const l=i.bet||[];if(l.length===0)return!1;const c=l[0],d=Object.keys(c).filter(u=>u!=="short_title")[0];return c[d]?.reconciled===!0});if(n.length===0)return'<p class="no-bets">No reconciled bets</p>';const a={};s.forEach(i=>{const l=i.guid||i.id||i.odds_id;l&&(a[l]=i)});const r={};n.forEach(i=>{const l=i.coreMatches__guid||i.match_guid||i.odds_id||"unknown";r[l]||(r[l]={bets:[],matchTitle:"Unknown Match",sportKey:"default"});const c=a[l];if(c){r[l].matchTitle=c.short_title||c.title||"Unknown Match";const d=c.sport_id,u=qt.find(p=>p.key===d);r[l].sportKey=u?.group||"default"}r[l].bets.push(i)});let o="";return Object.keys(r).forEach(i=>{const l=r[i],c=l.sportKey||"default",d=l.matchTitle||"Unknown Match";o+=`
            ...
            `,o+=`
                <table class="bets-table">
                    ...
                    <tbody>
            `,l.bets.forEach(u=>{const p=u.bet||[];if(p.length===0)return;const f=p[0],_=Object.keys(f).filter(m=>m!=="short_title")[0],y=f[_],b=parseFloat(y.payout||0)>0;o+=`
                    <tr class="${b?"bet-win":"bet-loss"}">
                        <td class="bet-selection">${_}</td>
                        <td class="bet-type">${y.type||"-"}</td>
                        <td class="bet-odds">${y.odds>0?"+":""}${y.odds||"-"}</td>
                        <td class="bet-stake">$${parseFloat(y.stake||0).toFixed(2)}</td>
                        <td class="bet-payout ${b?"payout-win":"payout-loss"}">$${parseFloat(y.payout||0).toFixed(2)}</td>
                    </tr>
                `}),o+=`
                    </tbody>
                </table>
  ...
  background-image: var(--badge__icon--bronze);
        ...
        `;const c=document.getElementById("pop-play__caro-info-summary2--id");c&&(c.innerHTML="");const d=document.getElementById("pop-play__caro-info-list--id");d&&a.matches_expanded&&Array.isArray(a.matches_expanded)&&(d.innerHTML=`
            ...
            `+a.matches_expanded.map(u=>{const p=u.scoreboard_data?typeof u.scoreboard_data=="string"?u.scoreboard_data:JSON.stringify(u.scoreboard_data):"";return`
                    ...
                    ></bma-match-status>
                `}).join(""))},async populateLeaderboard(){const e=me(),s=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,n=e.appSession?.session_user?.guid;if(!s){console.warn("[appEvents] Cannot populate leaderboard: no tournament GUID");return}const a=document.getElementById("pop-play__caro-leaderboard--id");if(a){a.innerHTML='<div class="leaderboard-loading"><div class="n55-juicebar"></div></div>',typeof neodigmJuiceBar<"u"&&neodigmJuiceBar.init();try{const r=await Te.fetchLeaderboard(s),o=r?.data||r?.rows;if(o&&o.length>0){const l=e.coreTourn[e.coreTourn.length-1]?.data.find(f=>f.guid===s);a.innerHTML=o.map((f,_)=>{let y="";if(l?.tags&&Array.isArray(l.tags)){const h=l.tags.find(g=>!!(typeof g=="object"&&g!==null&&(g.entity_guid===f.user_guid&&g.badge&&g.badge.startsWith("--badge__ribbon--")||g[f.user_guid]&&g[f.user_guid].startsWith("--badge__ribbon--"))));if(h){const g=h.badge||h[f.user_guid];g==="--badge__ribbon--gold"?y="badge-trophy--gold":g==="--badge__ribbon--silver"?y="badge-trophy--silver":g==="--badge__ribbon--bronze"&&(y="badge-trophy--bronze")}}const b=f.combined_betslips?typeof f.combined_betslips=="string"?f.combined_betslips:JSON.stringify(f.combined_betslips):"[]";let m="[]";if(f.combined_betslips&&l?.matches_expanded){const h=typeof f.combined_betslips=="string"?JSON.parse(f.combined_betslips):f.combined_betslips,g=[...new Set(h.map(T=>T.coreMatches__guid||T.match_guid||T.odds_id).filter(Boolean))],v=l.matches_expanded.filter(T=>{const P=T.guid||T.id||T.odds_id;return g.includes(P)});m=JSON.stringify(v)}return`
                        <bma-leaderboard-card
                            data-rank="${_+1}"
                            data-username="${f.username||"Unknown"}"
                            data-user-guid="${f.user_guid||""}"
                            data-tournament-dollars="${f.calculated_tournament_dollars||0}"
                            data-total-betslips="${f.total_betslips||0}"
                            data-total-payout="${f.total_payout||0}"
                            data-combined-betslips="${b.replace(/"/g,"&quot;")}"
                            data-user-matches="${m.replace(/"/g,"&quot;")}"
                            data-badge-class="${y}"
                            data-is-current-user="${f.user_guid===n}"
                            data-sync-theme="dark"
                        ></bma-leaderboard-card>
                    `}).join("");const c=o.findIndex(f=>f.user_guid===n),d=c>=0?c+1:o.length,u=l?.entities?.guids?.length||o.length,p=document.getElementById("dashboard-rank");p&&(p.innerHTML=`<span class="tourn-dashboard__rank-text">${d}/${u}</span>`)}else a.innerHTML=`
                    <div class="leaderboard-empty">
                        <p>No leaderboard data available yet.</p>
                        <p class="leaderboard-empty__subtitle">Be the first to place a bet!</p>
                    </div>
                `}catch(r){console.error("[appEvents] Error fetching leaderboard:",r),a.innerHTML=`
                <div class="leaderboard-empty">
                    <p>Unable to load leaderboard data.</p>
                </div>
            `}}},hydratePlayPopup(){const e=me(),s=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(!s||e.coreTourn.length===0)return;const a=e.coreTourn[e.coreTourn.length-1].data.find(r=>r.guid===s);a&&(console.log("[appEvents] Hydrating play popup for tournament:",s),this.populateInfoPage(),this.renderMatchCards(a),this.updateTournamentDashboard(a),this.updatePlayButton())},bindAppListeners(e){W.subscribe("APP",(b,m)=>{console.warn("~~ sub all APP | "+b+"|"+m)}),W.subscribe("WC",(b,m)=>{console.warn("~~ sub all WC  | "+b+"|"+m)}),W.subscribe("ROUTE",(b,m)=>{console.warn("~~ sub all ROUTE  | "+b+"|"+m)});let t="lobby",s="all";const n=()=>{const b=document.querySelector(".featured-carousel-section");b&&(t==="lobby"?b.style.display="block":b.style.display="none")},a=()=>{if(e.coreTourn.length===0)return;const m=e.coreTourn[e.coreTourn.length-1].data,h=e.appSession?.session_user?.guid;document.querySelectorAll(".tournament-item").forEach(T=>T.classList.remove("h-ds__none"));let v=0;if(m.forEach(T=>{const P=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${T.guid}"]`);if(!P)return;const k=P.parentElement;let R=!0;if(t==="my"?(T.entities?.guids||[]).includes(h)||(R=!1):t==="completed"&&(T.status||T.class)!=="COMPLETED"&&(R=!1),s!=="all"&&R){const E=T.sports_allowed||[];s==="multi"?E.length<=1&&(R=!1):E.some(L=>L.key===s)||(R=!1)}R?v++:k&&k.classList.add("h-ds__none")}),console.log(`[app_events] Filter applied - ${t}/${s} - showing ${v} of ${m.length} tournaments`),v===0){let T="No tournaments match the current filters";t==="my"&&(T="You haven't joined any tournaments yet"),t==="completed"&&(T="No completed tournaments yet"),typeof neodigmToast<"u"&&neodigmToast.q(T,"night")}};W.subscribe(e.hierTopics.COREBETSLIP,(b,m)=>{JSON.parse(m);let h=16;switch(b){case e.hierTopics.COREBETSLIP__BET:h=5,qs.shootConfetti(),console.log("~~~  |  "+b+" | ",m);const g=document.querySelector("neodigm-sodapop");g&&g.setAttribute("data-wait","true");const v=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry");if(v.length===0){console.warn("[app_events] No bets to submit"),g&&g.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("No bets to submit","warning");break}const P=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,k=e.appSession?.session_user?.guid;if(!P||!k){console.error("[app_events] Missing tournament or user GUID"),g&&g.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Missing tournament or user data","danger");break}const R=Array.from(v).map(H=>{const G=H.getAttribute("data-content-team-points"),$=H.getAttribute("data-content-odds"),se=H.getAttribute("data-stake")||"0",ne=H.getAttribute("data-content-type"),ce=H.getAttribute("data-payout")||"0",oe=H.getAttribute("data-match-guid")||"",ve={};return ve[G]={type:ne,stake:parseFloat(se),odds:parseFloat($),payout:parseFloat(ce),reconciled:!1},{acctEntity__guid:k,coreTournaments__guid:P,coreMatches__guid:oe,bet:[ve],status:"PENDING"}});console.log("[app_events] Submitting bet slips:",R),Te.postBetSlips(R).then(H=>{console.log("[app_events] Bet slips posted successfully:",H);const G=R.reduce(($,se)=>{const ne=se.bet[0],ce=Object.keys(ne)[0];return $+(ne[ce]?.stake||0)},0);typeof neodigmToast<"u"&&neodigmToast.q(`Bets placed successfully!|Total Stake: TD$ ${G.toFixed(2)}`,"success"),Te.fetchBetSlips(k,P).then($=>{if($?.rows){const se={timestamp:Date.now(),source:"API",data:$.rows};e.pushcoreBetSlip(se),console.log("[app_events] Refreshed bet slips in store:",$.rows.length,"items")}ke.refreshPlayPopupUI(P,$),g&&g.setAttribute("data-wait","false")}).catch($=>{console.error("[app_events] Error fetching fresh bet slips:",$),g&&g.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Bets placed but failed to refresh|Please close and reopen","warning")})}).catch(H=>{console.error("[app_events] Error posting bet slips:",H),typeof neodigmToast<"u"&&neodigmToast.q("Failed to place bets|Please try again","danger"),g&&g.setAttribute("data-wait","false")});break;case e.hierTopics.COREBETSLIP__SPREAD:break;case e.hierTopics.COREBETSLIP__MONEY:break;case e.hierTopics.COREBETSLIP__TOTAL:break;case e.hierTopics.COREBETSLIP__CLOSE:h=24;break;case e.hierTopics.COREBETSLIP__TDSYNC:if(document.querySelector("#summary-balance")&&m){m=JSON.parse(m);const G=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(e.coreBetSlip[0]?.data&&G){const $=e.coreBetSlip[0].data.filter(I=>I.coreTournaments__guid===G),se=[],ne=[],ce=[];$.forEach(I=>{const V=I.bet||[];if(V.length===0)return;const j=V[0];Object.keys(j).filter(le=>le!=="short_title").forEach(le=>{const S=j[le],w=S?.reconciled!==!1,O=parseFloat(S?.payout||0);w?O===0?ne.push(I):ce.push(I):se.push(I)})});const oe=nt.search(se,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,ve=nt.search(ne,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,$e=oe+ve,xe=nt.search(ce,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,Ae=m.tournament_dollars-$e+xe;ke.updateTDBalance("#summary-balance",Ae)}else ke.updateTDBalance("#summary-balance",m.tournament_dollars)}break;case e.hierTopics.COREBETSLIP__INTERSYNC:let x=0,L=0,A=0;if(e.coreBetSlip.length>0){const G=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(G){const se=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(xe=>xe.coreTournaments__guid===G),ne=[],ce=[],oe=[];se.forEach(xe=>{const Ae=xe.bet||[];if(Ae.length===0)return;const I=Ae[0];Object.keys(I).filter(j=>j!=="short_title").forEach(j=>{const X=I[j],le=X?.reconciled!==!1,S=parseFloat(X?.payout||0);le?S===0?ce.push(xe):oe.push(xe):ne.push(xe)})}),x=nt.search(ne,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,L=nt.search(oe,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0;const ve=nt.search(ce,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,$e=nt.search(oe,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;A=ve+$e}}let z=!0,Q=0;const ie=document.querySelector("[data-bets-valid]");if(ie&&m){m=JSON.parse(m),m.pending_stake_sum=0,m.pending_payout_sum=0,m.bets.length||(z=!1),m.bets.forEach(oe=>{oe.stake=Number(oe.stake),oe.stake?m.pending_stake_sum+=oe.stake:z=!1}),document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry").forEach(oe=>{const ve=parseFloat(oe.getAttribute("data-payout")||"0");m.pending_payout_sum+=ve});const G=x+m.pending_stake_sum,$=L+m.pending_payout_sum;Q=m.tournament_dollars-G-A+L;const ne=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let ce=null;ne&&e.coreTourn.length>0&&(ce=e.coreTourn[e.coreTourn.length-1]?.data?.find(ve=>ve.guid===ne)),ke.updateSummaryAndDashboard(Q,G,$,ce),(m.tournament_dollars<0||Q<0)&&(z=!1),ie.dataset.betsValid=z}break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h,"QUITE").vibrate()});const r=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(T=>T.guid===m)?.tournament_dollars||0),W.publish(e.hierTopics.COREBETSLIP__TDSYNC,JSON.stringify({tournament_dollars:h,timestamp:Date.now()}))},o=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(x=>x.guid===m)?.tournament_dollars||0);let g=0,v=0,T=0;if(e.coreBetSlip.length>0&&m){const E=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(H=>H.coreTournaments__guid===m);console.log("[app_events] Filtered bets for tournament:",m,"found:",E.length),console.log("[app_events] Using latest bet slip data index:",e.coreBetSlip.length-1),console.log("[app_events] First bet sample:",E[0]);const x=[],L=[],A=[];E.forEach(H=>{const G=H.bet||[];if(G.length===0)return;const $=G[0];Object.keys($).filter(ne=>ne!=="short_title").forEach(ne=>{const ce=$[ne],oe=ce?.reconciled!==!1,ve=parseFloat(ce?.payout||0);oe?ve===0?L.push(H):A.push(H):x.push(H)})}),console.log("[app_events] Bet categories:",{unreconciled:x.length,reconciledZero:L.length,reconciledNonZero:A.length});const z=nt.search(x,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Q=nt.search(L,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,ie=nt.search(A,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;T=Q+ie,v=nt.search(A,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,g=z,console.log("[app_events] Balance calculation:",{unreconciledStakes:z,lostBetStakes:Q,wonBetStakes:ie,allReconciledStakes:T,displayedStake:g,reconciledPayouts:v,tournamentDollars:h,calculatedBalance:h-g-T+v})}else console.warn("[app_events] No bet data found in Pinia store or missing tournamentGuid");const P=h-g-T+v;let k=null;m&&e.coreTourn.length>0&&(k=e.coreTourn[e.coreTourn.length-1].data.find(E=>E.guid===m)),ke.updateSummaryAndDashboard(P,g,v,k)};W.subscribe(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,(b,m)=>{const g=JSON.parse(m)?.rows||[],v=document.querySelector(".bet-grid__slip-MYBETS");if(!v)return;if(v.innerHTML="",g.length===0){v.innerHTML='<p style="color: var(--app-core-color--gray-6__dark--brand, #969696); text-align: center; padding: 20px;">No active bets</p>';const R=document.querySelector(".bet-grid__slip");R&&R.setAttribute("data-active-bet-tab","BETSLIP"),r();return}g.forEach(R=>{if((R.bet||[]).length===0)return;const x=document.createElement("bma-bet-existing");x.setAttribute("data-corebetslip",JSON.stringify(R)),v.appendChild(x)});const T=document.querySelector(".bet-grid__slip");T&&T.setAttribute("data-active-bet-tab","MYBETS"),me().appCLIFeatures?.features?.disable_existing_bet_btn?.state===!0&&ke.disableExistingBetButtons(),setTimeout(()=>{console.log("[app_events] Calling updateSummaryFromExistingBets after HYDRATE, bet count:",g.length),console.log("[app_events] Pinia store bet count:",e.coreBetSlip[0]?.data?.length||0),o()},200)}),document.addEventListener("click",b=>{const m=b.target;if(m&&m.classList.contains("bet-slip-tab")){const h=m.dataset.betTab,g=document.querySelector(".bet-grid__slip");h&&g&&(g.setAttribute("data-active-bet-tab",h),neodigmWired4Sound&&neodigmWired4Sound.sound(3),console.log("[app_events] Switched to bet tab:",h))}if(m&&m.dataset.publishBetslip){if(document.querySelector("[data-bets-valid]")?.dataset?.betsValid!=="true"){console.warn("[app_events] Cannot submit - bets invalid or button disabled");return}if(m.dataset.processing==="true"){console.warn("[app_events] Bet submission already in progress");return}m.dataset.processing="true";const g=m.dataset.publishBetslip;W.publish(g,JSON.stringify({timestamp:Date.now()})),setTimeout(()=>{delete m.dataset.processing},2e3)}});const i=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(k=>k.guid===m)?.tournament_dollars||0);const g=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry"),v=Array.from(g).map(T=>({teamPoints:T.getAttribute("data-content-team-points"),odds:T.getAttribute("data-content-odds"),stake:T.getAttribute("data-stake")||"0",type:T.getAttribute("data-content-type"),abbreviatedTitle:T.getAttribute("data-abbreviated-title"),scheduledAt:T.getAttribute("data-scheduled-at")}));W.publish(e.hierTopics.COREBETSLIP__INTERSYNC,JSON.stringify({tournament_dollars:h,bets:v,timestamp:Date.now()}))},l=(b,m)=>{const h=JSON.parse(m),g=document.querySelector(".bet-grid__slip-BETSLIP-content");if(!g){console.error("[app_events] Bet slip container not found");return}const v=g.querySelector(".bet-slip__empty");v&&v.remove();let T="";h.type==="total"?T=h.team==="over"?"Over":"Under":h.team==="home"?T=h.homeTeam:h.team==="away"&&(T=h.awayTeam);let P=T;if(h.point)if(h.type==="spread"){const A=parseFloat(h.point)>0?`+${h.point}`:h.point;P=`${T} ${A}`}else h.type==="total"&&(P=`${T} ${h.point}`);const k=document.createElement("bma-bet-entry");k.setAttribute("data-content-team-points",P),k.setAttribute("data-content-odds",h.price||"0"),k.setAttribute("data-content-stake-text","0"),k.setAttribute("data-content-type",h.type||""),k.setAttribute("data-abbreviated-title",h.abbreviatedTitle||`${h.homeTeam} vs ${h.awayTeam}`),k.setAttribute("data-scheduled-at",h.scheduledAt||""),k.setAttribute("data-match-guid",h.matchGuid||""),k.setAttribute("data-home-team",h.homeTeam||""),k.setAttribute("data-away-team",h.awayTeam||""),k.setAttribute("data-bet-team",T||"");const E=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(E&&e.coreTourn.length>0){const z=e.coreTourn[e.coreTourn.length-1].data.find(Q=>Q.guid===E);if(z?.sports_allowed&&z.sports_allowed.length>0){const Q=z.sports_allowed[0];k.setAttribute("data-sport-key",Q.key||"");const ie=qt.find(H=>H.key===Q.key);k.setAttribute("data-sport-group",ie?.group||"")}}g.appendChild(k),console.log("[app_events] Bet entry appended to container:",k,"Container children:",g.children.length),setTimeout(()=>{d(k)},0);const x=document.querySelector(".bet-grid__slip");x&&(x.setAttribute("data-active-bet-tab","BETSLIP"),console.log("[app_events] Switched to BETSLIP tab"));const L=g.querySelectorAll("bma-bet-entry").length;L>=3?requestAnimationFrame(()=>{setTimeout(()=>{const A=document.querySelector(".bet-grid__slip");A&&(console.log("[app_events] Scrolling parent to bottom - bet count:",L,"scrollHeight:",A.scrollHeight,"current scrollTop:",A.scrollTop),A.scrollTo({top:A.scrollHeight,behavior:"smooth"}),setTimeout(()=>{console.log("[app_events] After scroll - scrollTop:",A.scrollTop)},500))},150)}):console.log("[app_events] Skipping scroll - only",L,"bet cards (need 3+)"),setTimeout(()=>{i()},200),console.log("[app_events] Created bet entry:",{teamPoints:P,odds:h.price,type:h.type})};W.subscribe(e.hierTopics.COREBETSLIP__SPREAD,(b,m)=>{l(b,m),c()}),W.subscribe(e.hierTopics.COREBETSLIP__MONEY,(b,m)=>{l(b,m),c()}),W.subscribe(e.hierTopics.COREBETSLIP__TOTAL,(b,m)=>{l(b,m),c()});function c(){const b=window.matchMedia("(orientation: portrait)").matches,m=window.matchMedia("(max-width: 768px)").matches;if(b||m){const h=document.querySelector(".bet-grid__select"),g=document.querySelector(".bet-grid__slip"),v=document.querySelector(".bet-grid__toggle-text");h&&g&&v&&(h.classList.add("collapsed"),g.classList.remove("collapsed"),v.textContent="Close Bet Slip",console.log("[app_events] Bet slip expanded on portrait mode"))}}const d=b=>{const m={sport:{key:b.getAttribute("data-sport-key")||"",group:b.getAttribute("data-sport-group")||""},Match:{scheduled_at:b.getAttribute("data-scheduled-at")||"",home_team_id:b.getAttribute("data-home-team")||"",home_team_score:"",away_team_id:b.getAttribute("data-away-team")||"",away_team_score:""},Bet:{scope:{team_id:b.getAttribute("data-bet-team")||"",over:"",under:""},type:b.getAttribute("data-content-type")||"",odds:b.getAttribute("data-content-odds")||"0",stake:b.getAttribute("data-stake")||"0"}},h=ml.calcPayout(m);b.setAttribute("data-payout",h.toString()),console.log("[app_events] Payout calculated:",{stake:m.Bet.stake,odds:m.Bet.odds,payout:h})};new MutationObserver(b=>{b.forEach(m=>{m.type==="attributes"&&m.attributeName==="data-stake"&&m.target.tagName==="BMA-BET-ENTRY"&&(console.log("[app_events] Stake changed:",m.target.getAttribute("data-stake")),d(m.target),clearTimeout(window.betSlipStakeTimeout),window.betSlipStakeTimeout=setTimeout(()=>{i()},300))})}).observe(document.body,{attributes:!0,attributeFilter:["data-stake"],subtree:!0}),console.log("[app_events] MutationObserver for stake changes initialized"),document.addEventListener("bma-bet-entry-remove",b=>{console.log("[app_events] Removing bet entry:",b.detail);const m=b.target,h=b.detail;m.remove(),setTimeout(()=>{const P=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(P){const k=me(),R=k.coreTourn[k.coreTourn.length-1];if(R&&R.data){const E=R.data.find(x=>x.guid===P);E&&ke.updateTournamentDashboard(E)}}},100),document.querySelectorAll("bma-bet-match-card").forEach(T=>{(T.shadowRoot?.querySelectorAll(".btn")||[]).forEach(k=>{const R=k.dataset.betType,E=k.dataset.price,x=R===h.type,L=E===h.odds;x&&L&&k.classList.contains("btn--active")&&(k.classList.remove("btn--active"),console.log("[app_events] Deactivated button via X removal:",{btnType:R,btnPrice:E}))})});const v=document.querySelector(".bet-grid__slip-BETSLIP-content");v&&v.children.length,setTimeout(()=>{i()},100)}),W.subscribe(e.hierTopics.CANVAS__HYDRATE__STATUS,(b,m)=>{const h=JSON.parse(m);h&&h.guid&&neodigmUtils&&neodigmUtils.shake(`[data-bma-tourn-guid="${h.guid}"]`)}),W.subscribe(e.hierTopics.WC__APP__FOOT,(b,m)=>{JSON.parse(m);let h=16;switch(b){case e.hierTopics.WC__APP__FOOT__ALL_SPORTS:W.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LOBBY,JSON.stringify({tab:"ALL_SPORTS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_TOURNEYS:W.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_MY,JSON.stringify({tab:"MY_TOURNEYS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__LEADERBOARD:h=24,W.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LEADERBOARD,JSON.stringify({tab:"LEADERBOARD",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_PROFILE:W.publish(e.hierTopics.WC__APP__HEAD_TOP__USER_PROFILE,JSON.stringify({tab:"USER_PROFILE",timestamp:Date.now()}));break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()}),W.subscribe(e.hierTopics.WC__APP__HEAD_SPORTS,(b,m)=>{const h=JSON.parse(m);let g=16;b=="WC.APP.HEAD_SPORTS.PREV"||b=="WC.APP.HEAD_SPORTS.NEXT"||b=="WC.APP.HEAD_SPORTS.HEAD_SPORTS_DRAG_START"?g=3:(s=h.key||"all",console.log(`[app_events] Sports filter changed to: ${s} (${h.group})`),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SPORT_FILTER_CHANGE"}))),g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),W.subscribe(e.hierTopics.WC__APP__HEAD_MID,(b,m)=>{JSON.parse(m);let h=16;const g=()=>{s="all";const v=document.querySelector("bma-app-head-sports");v&&v.setAttribute("data-selected-chip","all")};switch(b){case"WC.APP.HEAD_MID.HEAD_MID_LOBBY":t="lobby",g(),n(),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_MY":if(!e.appSession?.session_user?.guid){console.warn("[app_events] No user GUID found, cannot filter tournaments"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to view your tournaments","warning");return}t="my",g(),n(),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_COMPLETED":t="completed",g(),n(),W.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_LEADERBOARD":t="leaderboard",g(),n(),a(),h=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break;case"WC.APP.HEAD_MID.HEAD_MID_SQUAD":t="squad",g(),n(),a(),h=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()}),W.subscribe(e.hierTopics.ROUTE__HOME__USER_PROFILE,async(b,m)=>{JSON.parse(m);let h=16;switch(b){case"ROUTE.HOME.USER_PROFILE.INSTALL_PWA":const g=window.deferredPWAPrompt;if(!g){console.log("No install prompt available"),typeof neodigmToast<"u"&&neodigmToast.q("PWA install not available","warning");return}try{g.prompt();const{outcome:T}=await g.userChoice;T==="accepted"?(console.log("PWA installed successfully"),window.deferredPWAPrompt=null,typeof neodigmToast<"u"&&neodigmToast.q("App installed successfully!","success"),h=8,neodigmSodaPop&&neodigmSodaPop.close()):(console.log("PWA install declined"),typeof neodigmToast<"u"&&neodigmToast.q("Install cancelled","night"))}catch(T){console.error("PWA install error:",T),typeof neodigmToast<"u"&&neodigmToast.q("Install failed","danger")}break;case"ROUTE.HOME.USER_PROFILE.TOGGLE_THEME":(()=>{const P=e.appCLIFeatures.features.theme.state==="dark"?"light":"dark";e.setFeaturePersistPub("theme",{state:P});const k=document.getElementById("app");k&&k.setAttribute("data-sync-theme",P),document.body.setAttribute("data-sync-theme",P)})(),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.RESET_PASSWORD":Fa.push({name:"resethash_route"}),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.SIGNOUT":Fa.push({name:"signout_route"}),neodigmSodaPop&&neodigmSodaPop.close();break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()});const p=(b,m)=>{const h=b.split(".").pop();console.log("[app_events] SODAPOP_PLAY subscriber - action:",h,"topic:",b),setTimeout(()=>{const g=document.querySelectorAll(".play-cntr .btn-info-sm"),T=document.querySelector(".play-cntr")?.closest("neodigm-sodapop");g.forEach(P=>{const k=P.dataset.publishRouteHome?.split(".").pop()||"";if(k===h){if(console.log("[app_events] Activating button:",k),P.classList.add("btn-info-sm--active"),P.classList.remove("btn-info-sm--inactive"),neodigmCarousel.nav({id:"caro-play",nav:k}),neodigmWired4Sound.sound(3),T){const R=h==="PLAY"?"hidden":"auto";T.style.overflow=R,console.log("[app_events] Set sodapop overflow to:",R)}}else P.classList.add("btn-info-sm--inactive"),P.classList.remove("btn-info-sm--active")})},100)};W.subscribe("ROUTE.HOME.SODAPOP_PLAY.INFO",p);let f=null;W.subscribe("ROUTE.HOME.SODAPOP_PLAY.LEADERBOARD",(b,m)=>{if(p(b),f){console.log("[appEvents] Leaderboard refresh debounced (cooling down)");return}f=setTimeout(()=>{f=null},2e3),console.log("[appEvents] Refreshing leaderboard data"),ke.populateLeaderboard()}),W.subscribe("ROUTE.HOME.SODAPOP_PLAY.PLAY",(b,m)=>{JSON.parse(m);const h=document.getElementById("btn-join__play--id");if(!h){console.warn("[app_events] PLAY button not found");return}const g=h.dataset.requiresJoin==="true",v=h.textContent.trim().toLowerCase();if(console.log("[app_events] PLAY button clicked - text:",v,"requiresJoin:",g),g&&v==="join"){const P=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,k=me();if(!P){console.error("[app_events] No tournament GUID found");return}console.log("[app_events] Executing JOIN for tournament:",P),W.publish(k.hierTopics.WC__TOURN_ACTION,JSON.stringify({action:"JOIN",tournamentGuid:P,timestamp:Date.now()})),h.textContent="Play",h.dataset.requiresJoin="false",setTimeout(()=>{p(b),console.log("[app_events] JOIN triggered, advancing carousel to PLAY page")},500)}else p(b),console.log("[app_events] Advancing carousel to PLAY page")}),W.subscribe(e.hierTopics.WC__APP__HEAD_TOP,(b,m)=>{JSON.parse(m);let h=10;switch(b){case"WC.APP.HEAD_TOP.USER_PROFILE":h=10,neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_my_profile"),setTimeout(()=>{neodigmCarousel.nav({id:"caro-user_profile",nav:"caro-page__badges"})},1200);break;case"WC.APP.HEAD_TOP.LOGO":const g=e.appSession.session_app.version,v=new Date().getFullYear();g&&neodigmToast&&neodigmToast.q(`${g} 3/15/2026, 9:12:23 PM|© ${v} Bet Max Action`,"night");break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()});let _=null,y=null;W.subscribe(e.hierTopics.WC__TOURN_ACTION,(b,m)=>{const h=JSON.parse(m);let g=0;switch(h?.action){case"JOIN":g=512;break;case"PLAY":g=8;break;case"INFO":g=8;break}if(g){_=h?.tournamentGuid,y=h?.action,console.log("[app_events] Storing pending tourn data:",_,y);const v=e.appSession?.session_user?.guid,T=h?.tournamentGuid;v&&T?Te.fetchBetSlips(v,T).then(P=>{if(P?.rows){const k={timestamp:Date.now(),source:"API",data:P.rows};e.pushcoreBetSlip(k),console.log("[app_events] Bet slips fetched:",P.rows.length,"items")}setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play"),setTimeout(()=>{W.publish(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,JSON.stringify(P))},3e3)},g)}).catch(P=>{console.error("[app_events] Error fetching bet slips:",P),setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}):setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}}),W.subscribe(e.hierTopics.WC__TOURN_ACTION,(b,m)=>{const h=JSON.parse(m);let g=16;switch(h?.action){case"FOCUS":g=3;break;case"SPORT_ICON":g=10;break;case"JOIN":g=5;break;case"PLAY":g=16;break}g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),W.subscribe(e.hierTopics.WC__TOURN_ACTION,(b,m)=>{const h=JSON.parse(m);let g=0,v="night";switch(neodigmOpt&&neodigmOpt.EVENT_SOUNDS&&(neodigmOpt.EVENT_SOUNDS=!1),h?.tournamentStatus){case"UPCOMING":v="success";break;case"LOCKED":v="warning";break;case"COMPLETED":v="danger";break}switch(h?.action){case"SPORT_ICON":h?.sportTitle==h?.sportDescription?g=h?.sportTitle:g=h?.sportTitle+"|"+h?.sportDescription;break}g&&neodigmToast&&neodigmToast.q(g,v)}),W.subscribe(e.hierTopics.WC__TOURN_ACTION,async(b,m)=>{const h=JSON.parse(m);if(h?.action==="JOIN"){const v=me().appSession?.session_user?.guid;if(!v){console.error("No entity guid found in session"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to join tournaments","warning");return}const T=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);T&&(T.setAttribute("data-bma-tourn-wait","true"),qs.shootConfetti());const P={acctEntityGuid:v,tournamentGuid:h.tournamentGuid};try{const k={method:"POST",body:JSON.stringify(P),headers:Te.genHeaders()};console.log("Posting to:",Te.API_baseURI+"/m5t/"+Te.API_ver+"/coreTournaments/join",P);const E=await(await fetch(Te.API_baseURI+"/m5t/"+Te.API_ver+"/coreTournaments/join",k)).json();if(console.log("Join response:",E),E.ok)typeof neodigmToast<"u"&&neodigmToast.q("Successfully joined tournament!","success"),setTimeout(()=>{const x=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);if(x){x.setAttribute("data-bma-tourn-wait","false");const L=x.getAttribute("data-bma-tourn-entities");if(L)try{const A=JSON.parse(L);A.guids.includes(v)||(A.guids.push(v),x.setAttribute("data-bma-tourn-entities",JSON.stringify(A)))}catch(A){console.error("Failed to parse entities:",A)}}},3e3);else{const x=E?.message||"Failed to join tournament";typeof neodigmToast<"u"&&neodigmToast.q(x,"danger");const L=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);L&&L.setAttribute("data-bma-tourn-wait","false")}}catch(k){console.error("Join tournament error:",k),typeof neodigmToast<"u"&&neodigmToast.q("Network error joining tournament","danger");const R=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);R&&R.setAttribute("data-bma-tourn-wait","false")}}}),W.subscribe(e.hierTopics.ROUTE__HOME_ONMOUNT,async(b,m)=>{console.log("[app_events] ROUTE__HOME_ONMOUNT event received");const h=me();try{const g=await Te.fetchTournaments();console.log("[app_events] Tournaments fetched:",g),g?.rows&&Array.isArray(g.rows)?(h.pushCoreTourn({timestamp:Date.now(),source:"API",data:g.rows}),console.log("[app_events] Pushed to coreTourn, length:",h.coreTourn.length),W.publish(h.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now()}))):console.warn("[app_events] Invalid API response format:",g)}catch(g){console.error("[app_events] Error fetching tournaments:",g),typeof neodigmToast<"u"&&neodigmToast.q("Please Sign In","success")}}),W.subscribe(e.hierTopics.PROMOTION__LOAD,async(b,m)=>{console.log("[app_events] PROMOTION__LOAD event received");try{const h=await Te.fetchPromotions();if(console.log("[app_events] Promotions fetched:",h),h?.ok&&h?.data&&Array.isArray(h.data)){const g=h.data.filter(T=>T.class==="banner"&&T.status==="LIVE");console.log("[app_events] Banner promotions:",g);const v=document.querySelector(".featured-swiper .swiper-wrapper");if(!v){console.warn("[app_events] Swiper wrapper not found");return}v.innerHTML="",g.forEach((T,P)=>{const k=document.createElement("div");k.className="swiper-slide";const R=document.createElement("div");R.className=`featured-card featured-card--${P+1}`;const E=T.hero_img?encodeURI(T.hero_img):"";R.style.backgroundImage=`url("${E}")`,R.setAttribute("data-promotion-hero-img",T.hero_img||""),R.setAttribute("data-promotion-caption",T.caption||""),R.setAttribute("data-promotion-tagline",T.tagline||""),R.setAttribute("data-promotion-toast",T.toast||""),R.setAttribute("data-promotion-topic",T.topic?.topic||""),R.setAttribute("data-promotion-topic-token",T.topic?.token||""),R.setAttribute("data-promotion-marquee",T.maquee||""),k.appendChild(R),v.appendChild(k)}),console.log("[app_events] Created",g.length,"promotion slides"),window.featuredSwiper&&window.featuredSwiper.update()}}catch(h){console.error("[app_events] Error fetching promotions:",h)}}),W.subscribe(e.hierTopics.PROMOTION__CLICK,(b,m)=>{console.log("[app_events] PROMOTION__CLICK event received");try{const h=JSON.parse(m);console.log("[app_events] Promotion clicked:",h),h.toast&&typeof neodigmToast<"u"&&neodigmToast.q(h.toast,"success"),typeof neodigmSodaPop<"u"&&(neodigmSodaPop.setOnAfterOpen(()=>{const g=document.querySelector(".l-promotion #promoHero");g&&h.heroImg&&(g.src=h.heroImg);const v=document.querySelector(".l-promotion #promCaption");v&&h.caption&&(v.textContent=h.caption);const T=document.querySelector(".l-promotion #promoTagline");return T&&h.tagline&&(/<[^>]+>/.test(h.tagline)?T.innerHTML=h.tagline:T.textContent=h.tagline),!0},"sodapop_promotion"),neodigmSodaPop.autoOpen("sodapop_promotion"))}catch(h){console.error("[app_events] Error handling promotion click:",h)}}),W.subscribe(e.hierTopics.ROUTE__HOME_HYDRATE,(b,m)=>{console.log("[app_events] ROUTE__HOME_HYDRATE event received");const h=me();if(h.coreTourn.length===0){console.warn("[app_events] No tournament data in coreTourn array");return}const g=h.coreTourn[h.coreTourn.length-1],v=h.coreTourn.length>1?h.coreTourn[h.coreTourn.length-2]:null;console.log("[app_events] Hydrating with latest data:",g);const T=[];if(v){const E=g.data,x=v.data;E.forEach(L=>{const A=x.find(z=>z.guid===L.guid);A&&A.status!==L.status&&(console.log(`[app_events] Status changed for tournament ${L.guid}: ${A.status} -> ${L.status}`),T.push(L.guid))})}const P=h.appSession?.session_user?.guid,k=[...g.data].sort((E,x)=>{const L=E.status||E.class,A=x.status||x.class,z=E.entities?.guids||[],Q=x.entities?.guids||[],ie=z.includes(P),H=Q.includes(P),G=(Ae,I)=>{const V=new Date(Ae.status_time||0).getTime();return new Date(I.status_time||0).getTime()-V},$=L==="LOCKED"&&ie,se=A==="LOCKED"&&H;if($&&!se)return-1;if(!$&&se)return 1;if($&&se)return G(E,x);const ne=L==="UPCOMING",ce=A==="UPCOMING";if(ne&&!ce)return-1;if(!ne&&ce)return 1;if(ne&&ce)return G(E,x);const oe=L==="COMPLETED"&&ie,ve=A==="COMPLETED"&&H;if(oe&&!ve)return-1;if(!oe&&ve)return 1;if(oe&&ve)return G(E,x);const $e=L==="COMPLETED"&&!ie,xe=A==="COMPLETED"&&!H;return $e&&!xe?1:!$e&&xe?-1:G(E,x)}),R=ke.categorizeTournaments(k,t,s,P);if(console.log(`[app_events] Categorized into ${R.length} categories for filter: ${t}/${s}`),window._homeRouteRenderCards&&typeof window._homeRouteRenderCards=="function"){if(window._homeRouteRenderCards(R,!0),setTimeout(()=>{a(),console.log("[app_events] Re-applied filter after hydration:",t)},100),T.length>0){const E=g.data;T.forEach(x=>{const L=E.find(A=>A.guid===x);if(L){const A=Math.floor(Math.random()*2e3)+1e3;setTimeout(()=>{W.publish(h.hierTopics.CANVAS__HYDRATE__STATUS,JSON.stringify({guid:x,status:L.status,timestamp:Date.now()}))},A)}})}}else console.warn("[app_events] window._homeRouteRenderCards not available")}),W.subscribe(e.hierTopics.SSE__CORE__TOURN_SYNC,(b,m)=>{console.log("[app_events] SSE__CORE__TOURN_SYNC event received:",m);const h=me();try{const g=JSON.parse(m);console.log("[app_events] SSE tournament sync data:",g);const v=JSON.parse(g.msg);if(console.log("[app_events] Parsed tournaments array:",v),!Array.isArray(v)){console.warn("[app_events] SSE msg is not an array:",v);return}h.pushCoreTourn({timestamp:Date.now(),source:"SSE",data:v}),console.log("[app_events] Pushed SSE data to coreTourn, length:",h.coreTourn.length),W.publish(h.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SSE"})),typeof neodigmSodaPop<"u"&&neodigmSodaPop.isOpen()==="sodapop_play"&&(console.log("[SSE] Play popup is open, triggering real-time update"),ke.hydratePlayPopup())}catch(g){console.error("[app_events] Error processing SSE tournament sync:",g)}}),setTimeout(function(){neodigmSodaPop&&neodigmMarquee&&neodigmEnchantedCTA&&(neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{},1e3),neodigmMetronome.init().pause(800),neodigmMarquee.init().pause(1200),neodigmEnchantedCTA.init(),neodigmCarousel.init()}),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{NeodigmClaire.showCanv("neodigm-sodapop",.5).setTheme(["warning","success"]).initCanvOn("neodigm-sodapop").waxOn("neodigm-sodapop");const b=document.querySelector("#caption__my-profile");b&&(b.textContent="My Profile");const m=e.appSession?.session_user?.guid;if(console.log("[app_events] User GUID:",m),console.log("[app_events] coreTourn length:",e.coreTourn.length),!m||e.coreTourn.length===0){console.warn("[app_events] Missing user GUID or tournament data");return}const g=e.coreTourn[e.coreTourn.length-1].data;console.log("[app_events] Processing tournaments:",g.length);let v=0,T=0,P=0;g.forEach((k,R)=>{if(!k.tags||!Array.isArray(k.tags)){console.log(`[app_events] Tournament ${R} has no tags or tags not an array`);return}k.tags.forEach((E,x)=>{if(typeof E=="object"&&E!==null){let L=null;E.entity_guid===m&&E.badge?L=E.badge:E[m]&&(L=E[m]),L==="--badge__ribbon--gold"?v++:L==="--badge__ribbon--silver"?T++:L==="--badge__ribbon--bronze"&&P++}})}),setTimeout(()=>{let k=document.querySelectorAll(".badge-counter");if(k.length===0){const R=document.querySelector("neodigm-sodapop");R&&(k=R.querySelectorAll(".badge-counter"))}k.length>=3?(k[0].textContent=v,k[1].textContent=T,k[2].textContent=P,console.log("[app_events] Updated UI counters successfully")):console.warn("[app_events] Not enough counter elements found")},500)},1e3)},"sodapop_my_profile"),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{const b=me(),m=document.querySelector(".play-cntr");_&&(m.dataset.currentTournGuid=_),y&&(m.dataset.currentTournAction=y);const h=m?.dataset?.currentTournGuid,g=m?.dataset?.currentTournAction||"INFO";console.log("[app_events] sodapop_play opened - GUID:",h,"action:",g),window.initBetGridToggle&&window.initBetGridToggle(),ke.updatePlayButton(),ke.populateInfoPage(),setTimeout(()=>{ke.populateLeaderboard()},600);let v=g==="PLAY"||g==="JOIN"?"PLAY":g;if(!h||b.coreTourn.length===0||b.coreTourn[b.coreTourn.length-1].data.find(z=>z.guid===h)?.status==="COMPLETED"&&(v="LEADERBOARD",console.log("[app_events] Tournament is COMPLETED, defaulting to LEADERBOARD")),v){const L=`ROUTE.HOME.SODAPOP_PLAY.${v}`;console.log("[app_events] Publishing initial topic:",L),W.publish(L,JSON.stringify({tournamentGuid:h,timestamp:Date.now()}))}if(!h||b.coreTourn.length===0)return;const P=b.coreTourn[b.coreTourn.length-1].data.find(L=>L.guid===h);ke.renderMatchCards(P);const k=document.querySelector("#pop-play__tourn-caption--id"),R=document.querySelector("#pop-play__tourn-tagline--id");k&&(k.textContent=P.caption),R&&(R.textContent=P.tagline),console.log("[Modal Debug] Updating dashboard for tournament:",P?.caption);const E=document.querySelector(".head-caption");console.log("[Modal Debug] Modal header found:",!!E),E&&console.log("[Modal Debug] Modal classes:",E.className),ke.updateTournamentDashboard(P);const x=document.getElementById("dashboard-rank");if(x){const L=P.entities?.guids?.length||0;x.innerHTML=`
                                <span class="tourn-dashboard__rank-text">-/${L}</span>
                            `}},1e3)},"sodapop_play"))},3e3),setTimeout(function(){neodigmSodaPop&&NeodigmClaire&&neodigmSodaPop.setOnClose(function(){return!0})},4e3)}},Nm={__name:"App",setup(e){const t=vt(),s=me();return Dt(()=>{setTimeout(()=>{const n=s.appSession?.session_user?.guid,a=s.appSession?.session_user?.authenticated;n&&a&&s.appCLIFeatures.features.sse.state&&(console.log("[App] Reconnecting SSE for existing session, GUID:",n),xn.establishSSE("https://machfive-bmacdev-rest.onrender.com/v5/sse?appid=BMACORE&sseid=##SSEID##&lastkey=##LASTKEY##",n,s,!0))},2e3)}),setTimeout(function(){neodigmOpt.mvvLegit&&mvvLegit.setNavConroller((n=null)=>{neodigmOpt.N55_DEBUG_lOG&&neodigmToast.q(`Legit route: ${n}`,"primary"),t.push({name:n})}).setOnState((n=null)=>{n&&neodigmOpt.ROOT&&(neodigmOpt.ROOT.dataset.mvvLegit=n)}).init({BASE:Te.API_baseURI})},2e3),setTimeout(()=>{ke.bindAppListeners(s)},3e3),t.beforeEach((n,a,r)=>{n.name=="splash_route"||n.name&&mvvLegit&&mvvLegit.isRouteAllowed(n.name)?r():r(!1)}),(n,a)=>(st(),kr(Kt(dl),null,{default:Sa(({Component:r})=>[Ge(kd,{name:"slide-left"},{default:Sa(()=>[(st(),kr(Lc(r)))]),_:2},1024)]),_:1}))}};class Dm extends HTMLElement{constructor(){super(),this.clockInterval=null}connectedCallback(){this.attachShadow({mode:"open"}),this.render(),this.startClock()}disconnectedCallback(){this.clockInterval&&clearInterval(this.clockInterval)}static get observedAttributes(){return["data-sync-theme","data-user-name"]}attributeChangedCallback(t,s,n){this.shadowRoot&&s!==n&&this.render()}startClock(){this.updateTime(),this.clockInterval=setInterval(()=>{this.updateTime()},24e3)}updateTime(){const t=this.shadowRoot?.querySelector(".status-time");if(t){const s=new Date,n=s.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",hour12:!0}),a=s.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"});t.textContent=`${a} ${n}`}}handleLogoClick(t){t.preventDefault(),W.publish("WC.APP.HEAD_TOP.LOGO",JSON.stringify({timestamp:Date.now()}))}handleProfileClick(t){t.preventDefault(),W.publish("WC.APP.HEAD_TOP.USER_PROFILE",JSON.stringify({timestamp:Date.now()}))}render(){const n=(document.getElementById("app")?.getAttribute("data-sync-theme")||"dark")==="dark",a=this.getAttribute("data-user-name")||"Guest";this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          position: sticky;
    ...
    `,a.forEach(o=>{const i=this.shadowRoot.querySelector(`[data-item="${o.name}"]`);i&&(i.addEventListener("click",()=>this.handleItemClick(o.name,o.topic)),i.addEventListener("keypress",l=>{(l.key==="Enter"||l.key===" ")&&(l.preventDefault(),this.handleItemClick(o.name,o.topic))}))})}}customElements.define("bma-app-foot",$m);class Um extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render()}static get observedAttributes(){return["data-corebetslip"]}attributeChangedCallback(t,s,n){s!==n&&this.render()}get betSlipData(){const t=this.getAttribute("data-corebetslip");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-existing] Failed to parse data-corebetslip:",s),null}}render(){const t=this.betSlipData;if(!t){this.shadowRoot.innerHTML="<div>Invalid bet data</div>";return}const s=t.bet||[];if(s.length===0){this.shadowRoot.innerHTML="<div>No bet data</div>";return}const n=s[0],a=n.short_title||"Match Info N/A",o=Object.keys(n).filter(h=>h!=="short_title")[0]||"Unknown Team",i=n[o],l=i.odds||"N/A",c=i.type||"N/A",d=parseFloat(i.stake||0).toFixed(2),u=parseFloat(i.payout||0).toFixed(2),p=i.reconciled!==!1,f=p&&parseFloat(u)>parseFloat(d),_=p&&parseFloat(u)<=parseFloat(d);let y="";p?f?y="win":_&&(y="loss"):y="unreconciled";const b=t.status_time?qs.formatDateLocal(t.status_time):"";let m=o;if(c==="spread"&&i.point){const h=parseFloat(i.point)>0?`+${i.point}`:i.point;m=`${o} ${h}`}else c==="total"&&i.point&&(m=`${i.team==="over"?"Over":"Under"} ${i.point}`);this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          width: 100%;
