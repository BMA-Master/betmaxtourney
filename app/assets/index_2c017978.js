...
<pre data-n55-theme="warning" style="padding: 7px 0 4px 0; height: 34px; background-color: #161616"></pre>
    ...
    `,s.forEach(r=>{const o=this.shadowRoot.querySelector(`[data-item="${r.name}"]`);o&&(o.addEventListener("click",()=>this.handleItemClick(r.name,r.topic)),o.addEventListener("keypress",i=>{(i.key==="Enter"||i.key===" ")&&(i.preventDefault(),this.handleItemClick(r.name,r.topic))}))})}}customElements.define("bma-app-foot",tm);class sm extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render()}static get observedAttributes(){return["data-corebetslip"]}attributeChangedCallback(t,s,a){s!==a&&this.render()}get betSlipData(){const t=this.getAttribute("data-corebetslip");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-existing] Failed to parse data-corebetslip:",s),null}}render(){const t=this.betSlipData;if(!t){this.shadowRoot.innerHTML="<div>Invalid bet data</div>";return}const s=t.bet||[];if(s.length===0){this.shadowRoot.innerHTML="<div>No bet data</div>";return}const a=s[0],n=a.short_title||"Match Info N/A",o=Object.keys(a).filter(A=>A!=="short_title")[0]||"Unknown Team",i=a[o],l=i.odds||"N/A",c=i.type||"N/A",d=parseFloat(i.stake||0).toFixed(2),u=parseFloat(i.payout||0).toFixed(2),p=i.reconciled!==!1,f=p&&parseFloat(u)>parseFloat(d),g=p&&parseFloat(u)<=parseFloat(d);let b="";p?f?b="win":g&&(b="loss"):b="unreconciled";const k=t.status_time?Qs.formatDateLocal(t.status_time):"";let T=o;if(c==="spread"&&i.point){const A=parseFloat(i.point)>0?`+${i.point}`:i.point;T=`${o} ${A}`}else c==="total"&&i.point&&(T=`${i.team==="over"?"Over":"Under"} ${i.point}`);this.shadowRoot.innerHTML=`
      <style>
        ${ot()}
        :host {
          display: block;
    ...
    `}}customElements.define("bma-bet-existing",sm);Xs.init();window.BrandManager=Xs;const Bo=`
...
<section data-n55-carousel-page-name="SPLASH">
  ...
  <img src="https://machfive-bmacdev-rest.onrender.com/m5t/v5/coreAssets/image?token=bmt_white_primary_logo&format=png" alt="Bet Max Tourney" data-m5t-brand-src-swap="true" />
  </div>
</section>
