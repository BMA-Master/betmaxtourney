...
/*  Neodigm JuiceBar End  */ 
    ...
    `,this.setupEventListeners()}}customElements.define("bma-bet-entry",ef);class tf extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get observedAttributes(){return["data-match-id","data-match-guid","data-match-title","data-match-short-title","data-scheduled-at","data-home-team","data-away-team","data-odds-markets","data-is-disabled","data-sync-theme","data-scoreboard","data-sport-key","data-tournament-tags"]}get matchId(){return this.getAttribute("data-match-id")||""}get matchGuid(){return this.getAttribute("data-match-guid")||""}get matchTitle(){return this.getAttribute("data-match-title")||""}get shortTitle(){return this.getAttribute("data-match-short-title")||this.matchTitle}get scheduledAt(){return this.getAttribute("data-scheduled-at")||""}get homeTeam(){return this.getAttribute("data-home-team")||"Home"}get awayTeam(){return this.getAttribute("data-away-team")||"Away"}get oddsMarkets(){const t=this.getAttribute("data-odds-markets");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing odds_markets:",s),null}}get isDisabled(){return this.getAttribute("data-is-disabled")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}get scoreboard(){const t=this.getAttribute("data-scoreboard");if(!t)return null;try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing scoreboard:",s),null}}get sportKey(){return this.getAttribute("data-sport-key")||""}get sportGroup(){const t=this.sportKey;if(!t)return"";const s=qt.find(n=>n.key===t);return s?s.group:""}get tournamentTags(){const t=this.getAttribute("data-tournament-tags");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-bet-match-card] Error parsing tournament tags:",s),[]}}isMatchInProgressLocked(){if(!this.tournamentTags.some(o=>o.match_inprogress_lock===!0))return!1;const n=this.scoreboard;if(!n||!n.time_remaining)return!1;const a=n.time_remaining.toLowerCase();return a==="in progress"||a.includes("q")||a.includes("half")||a.includes("period")}connectedCallback(){this.render(),this.startCountdownTimer()}disconnectedCallback(){this.stopCountdownTimer()}startCountdownTimer(){this.stopCountdownTimer(),this.countdownInterval=setInterval(()=>{this.updateCountdown()},6e4)}stopCountdownTimer(){this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null)}updateCountdown(){const t=this.shadowRoot?.querySelector(".countdown");t&&(t.textContent=this.getCountdownText())}attributeChangedCallback(t,s,n){s!==n&&this.render()}formatDate(t){if(!t)return"TBD";let s=new Date(t);return!t.includes("Z")&&!t.match(/[+-]\d{2}:\d{2}$/)&&(s=new Date(t+"Z")),s.toLocaleString(void 0,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}getCountdownText(){if(!this.scheduledAt)return"";const t=new Date,s=new Date(this.scheduledAt);!this.scheduledAt.includes("Z")&&!this.scheduledAt.match(/[+-]\d{2}:\d{2}$/)&&s.setTime(new Date(this.scheduledAt+"Z").getTime());const n=s-t;if(n<=0)return"";const a=Math.floor(n/(1e3*60*60)),r=Math.floor(n%(1e3*60*60)/(1e3*60));return a>24?`${Math.floor(a/24)}d ${a%24}h`:a>0?`${a}h ${r}m`:r>5?`${r} min`:"Starting Soon"}getGameStatus(){const t=this.getCountdownText();if(t)return{type:"upcoming",text:t,color:"#F7C60D"};const s=this.scoreboard;if(s&&s.time_remaining){const n=s.time_remaining.toLowerCase();if(n==="final")return{type:"final",text:"Final",color:"#969696"};if(n==="in progress"||n.includes("q")||n.includes("half")||n.includes("period")){const a=s.home_score!==void 0&&s.home_score!==null||s.away_score!==void 0&&s.away_score!==null,r=s.period!==void 0&&s.period!==null&&s.period!==0;if(a||r)return{type:"live",text:s.time_remaining,color:"#00E676"}}}return{type:"scheduled",text:this.formatDate(this.scheduledAt),color:"#969696"}}handleButtonClick(t,s,n,a,r){t.stopPropagation();const o=t.currentTarget;if(o.classList.contains("btn--disabled")||o.dataset.disabled==="true")return;const i=o.classList.contains("btn--active"),l={matchId:this.matchId,matchGuid:this.matchGuid,matchTitle:this.matchTitle,abbreviatedTitle:this.shortTitle,homeTeam:this.homeTeam,awayTeam:this.awayTeam,scheduledAt:this.scheduledAt,type:s,team:n,price:a,point:r,timestamp:Date.now()};this.dispatchEvent(new CustomEvent("bet-button-click",{bubbles:!0,composed:!0,detail:{betData:l,isActive:i,button:o}}))}renderButton(t,s,n,a,r=!1){if(!n)return'<button class="btn btn--empty" disabled></button>';const o=n.price,i=n.point;let l="";return t==="spread"?l=`${i>0?`+${i}`:i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`:t==="money"?l=o:t==="total"&&(l=`${a}${i}<br><span class="btn-odds" data-price-value="${o}">${o}</span>`),`<button class="btn ${r?"btn--disabled":""}"
            ...
            ${l}
        </button>`}render(){const t=this.theme==="dark",s=this.oddsMarkets,n=s&&(s.h2h||s.totals||s.spreads),a=this.isMatchInProgressLocked(),r=this.isDisabled||a,o=n,i=this.getGameStatus(),l=this.scoreboard,c=l?.home_score||0,d=l?.away_score||0,u=i?.type==="live"||i?.type==="final",p=s?.spreads?.outcomes?.find(h=>h.name===this.homeTeam),f=s?.spreads?.outcomes?.find(h=>h.name===this.awayTeam),b=s?.h2h?.outcomes?.find(h=>h.name===this.homeTeam),v=s?.h2h?.outcomes?.find(h=>h.name===this.awayTeam),_=s?.totals?.outcomes?.find(h=>h.name==="Over"),m=s?.totals?.outcomes?.find(h=>h.name==="Under");this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: block;
                }
                    ...
                    <div class="row row--home">
                        ...
                        ${o?`
                            <div class="buttons">
                                ...
                                ${this.renderButton("total","over",_,"O",r)}
                            </div>
                        `:""}
                    </div>
                    ...
                    <div class="row row--away">
                        ...
                        ${o?`
                            <div class="buttons">
                                ...
                                ${this.renderButton("money","away",v,null,r)}
    ...
    `}}customElements.define("bma-match-status",sf);class Yt extends HTMLElement{static currentlyExpanded=null;constructor(){super(),this.attachShadow({mode:"open"}),this.expanded=!1}static get observedAttributes(){return["data-rank","data-username","data-user-guid","data-tournament-dollars","data-starting-dollars","data-total-betslips","data-total-payout","data-combined-betslips","data-user-matches","data-badge-class","data-is-current-user","data-sync-theme"]}get rank(){return this.getAttribute("data-rank")||""}get username(){return this.getAttribute("data-username")||"Unknown"}get userGuid(){return this.getAttribute("data-user-guid")||""}get tournamentDollars(){return this.getAttribute("data-tournament-dollars")||"0"}get startingDollars(){return this.getAttribute("data-starting-dollars")||"0"}get totalBetslips(){return this.getAttribute("data-total-betslips")||"0"}get totalPayout(){return this.getAttribute("data-total-payout")||"0"}get combinedBetslips(){const t=this.getAttribute("data-combined-betslips");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing combined_betslips:",s),[]}}get userMatches(){const t=this.getAttribute("data-user-matches");if(!t)return[];try{return JSON.parse(t)}catch(s){return console.error("[bma-leaderboard-card] Error parsing user_matches:",s),[]}}get badgeClass(){return this.getAttribute("data-badge-class")||""}get isCurrentUser(){return this.getAttribute("data-is-current-user")==="true"}get theme(){return this.getAttribute("data-sync-theme")||"dark"}connectedCallback(){this.render(),this.attachEventListeners()}attributeChangedCallback(t,s,n){s!==n&&(this.render(),this.attachEventListeners())}toggleExpanded(){Yt.currentlyExpanded&&Yt.currentlyExpanded!==this&&Yt.currentlyExpanded.collapse(),this.expanded=!this.expanded;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");this.expanded?(t.classList.add("open"),s.classList.add("expanded"),Yt.currentlyExpanded=this):(t.classList.remove("open"),s.classList.remove("expanded"),Yt.currentlyExpanded===this&&(Yt.currentlyExpanded=null))}collapse(){this.expanded=!1;const t=this.shadowRoot.querySelector(".leaderboard-card__details"),s=this.shadowRoot.querySelector(".expand-icon");t&&t.classList.remove("open"),s&&s.classList.remove("expanded")}attachEventListeners(){const t=this.shadowRoot.querySelector(".expand-icon");t&&t.addEventListener("click",s=>{s.stopPropagation(),this.toggleExpanded()})}getBetStats(){const t=this.combinedBetslips;let s=0,n=0,a=0,r=0,o=0;return t.forEach(i=>{const l=i.bet||[];if(l.length===0)return;const c=l[0];Object.keys(c).filter(u=>u!=="short_title").forEach(u=>{const p=c[u];!p||typeof p!="object"||(p.reconciled===!0?parseFloat(p.payout||0)>0?s++:n++:(a++,r+=parseFloat(p.stake||p.wager||0),o+=parseFloat(p.payout||0)))})}),{wins:s,losses:n,pending:a,total:s+n+a,pendingStakes:r,pendingMaxPayout:o}}renderBetsTable(){const t=this.combinedBetslips,s=this.userMatches,n=this.isCurrentUser;if(!t||t.length===0)return'<p class="no-bets">No bets available</p>';const a=t.filter(l=>{const c=l.bet||[];if(c.length===0)return!1;if(n)return!0;const d=c[0],u=Object.keys(d).filter(p=>p!=="short_title")[0];return d[u]?.reconciled===!0});if(a.length===0)return'<p class="no-bets">No reconciled bets</p>';const r={};s.forEach(l=>{const c=l.guid||l.id||l.odds_id;c&&(r[c]=l)});const o={};a.forEach(l=>{const c=l.coreMatches__guid||l.match_guid||l.odds_id||"unknown";o[c]||(o[c]={bets:[],matchTitle:"Unknown Match",sportKey:"default"});const d=r[c];if(d){o[c].matchTitle=d.short_title||d.title||"Unknown Match";const u=d.sport_id,p=qt.find(f=>f.key===u);o[c].sportKey=p?.group||"default"}o[c].bets.push(l)});let i="";return Object.keys(o).forEach(l=>{const c=o[l],d=c.sportKey||"default",u=c.matchTitle||"Unknown Match",p=c.bets.length;i+=`
            ...
            `,c.bets.forEach(f=>{const b=f.bet||[];if(b.length===0)return;const v=b[0];Object.keys(v).filter(m=>m!=="short_title").forEach(m=>{const h=v[m];if(!h||typeof h!="object")return;const g=(h.type||"").toUpperCase(),y=parseFloat(h.stake||h.wager||0).toFixed(2),T=h.odds||"-",A=T>0?`+${T}`:`${T}`,O=parseFloat(h.payout||0).toFixed(2),R=h.reconciled===!0;let E="pending",x="Pending";R&&Number(O)>0?(E="won",x="Won"):R&&Number(O)===0?(E="lost",x="Lost"):R&&(E="reconciled",x="Settled"),i+=`<div class="bet-item bet-item--${E}">
  ...
  background-image: var(--badge__icon--bronze);
        ...
        `;const c=document.getElementById("pop-play__caro-info-summary2--id");c&&(c.innerHTML="");const d=document.getElementById("pop-play__caro-info-list--id");d&&a.matches_expanded&&Array.isArray(a.matches_expanded)&&(d.innerHTML=`
            ...
            `+a.matches_expanded.map(u=>{const p=u.scoreboard_data?typeof u.scoreboard_data=="string"?u.scoreboard_data:JSON.stringify(u.scoreboard_data):"";return`
                ...
                width: 100%;
                margin: 6px;
            `,a.parentNode.insertBefore(r,a)),r.style.visibility="visible";try{const o=await Se.fetchLeaderboard(s),i=o?.data||o?.rows;if(i&&i.length>0){const c=e.coreTourn[e.coreTourn.length-1]?.data.find(b=>b.guid===s);a.innerHTML=i.map((b,v)=>{let _="";if(c?.tags&&Array.isArray(c.tags)){const g=c.tags.find(y=>!!(typeof y=="object"&&y!==null&&(y.entity_guid===b.user_guid&&y.badge&&y.badge.startsWith("--badge__ribbon--")||y[b.user_guid]&&y[b.user_guid].startsWith("--badge__ribbon--"))));if(g){const y=g.badge||g[b.user_guid];y==="--badge__ribbon--gold"?_="badge-trophy--gold":y==="--badge__ribbon--silver"?_="badge-trophy--silver":y==="--badge__ribbon--bronze"&&(_="badge-trophy--bronze")}}const m=b.combined_betslips?typeof b.combined_betslips=="string"?b.combined_betslips:JSON.stringify(b.combined_betslips):"[]";let h="[]";if(b.combined_betslips&&c?.matches_expanded){const g=typeof b.combined_betslips=="string"?JSON.parse(b.combined_betslips):b.combined_betslips,y=[...new Set(g.map(A=>A.coreMatches__guid||A.match_guid||A.odds_id).filter(Boolean))],T=c.matches_expanded.filter(A=>{const O=A.guid||A.id||A.odds_id;return y.includes(O)});h=JSON.stringify(T)}return`
            ...
            `,setTimeout(()=>{r&&(r.style.visibility="hidden")},1800)}},hydratePlayPopup(){const e=me(),s=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(!s||e.coreTourn.length===0)return;const a=e.coreTourn[e.coreTourn.length-1].data.find(r=>r.guid===s);a&&(console.log("[appEvents] Hydrating play popup for tournament:",s),this.populateInfoPage(),this.renderMatchCards(a),this.updateTournamentDashboard(a),this.updatePlayButton())},bindAppListeners(e){V.subscribe("APP",(_,m)=>{console.warn("~~ sub all APP | "+_+"|"+m)}),V.subscribe("WC",(_,m)=>{console.warn("~~ sub all WC  | "+_+"|"+m)}),V.subscribe("ROUTE",(_,m)=>{console.warn("~~ sub all ROUTE  | "+_+"|"+m)});let t="lobby",s="all";const n=()=>{const _=document.querySelector(".featured-carousel-section");_&&(t==="lobby"?_.style.display="block":_.style.display="none")},a=()=>{if(e.coreTourn.length===0)return;const m=e.coreTourn[e.coreTourn.length-1].data,h=e.appSession?.session_user?.guid;document.querySelectorAll(".tournament-item").forEach(T=>T.classList.remove("h-ds__none"));let y=0;if(m.forEach(T=>{const A=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${T.guid}"]`);if(!A)return;const O=A.parentElement;let R=!0;if(t==="my"?(T.entities?.guids||[]).includes(h)||(R=!1):t==="completed"&&(T.status||T.class)!=="COMPLETED"&&(R=!1),s!=="all"&&R){const E=T.sports_allowed||[];s==="multi"?E.length<=1&&(R=!1):E.some(L=>L.key===s)||(R=!1)}R?y++:O&&O.classList.add("h-ds__none")}),console.log(`[app_events] Filter applied - ${t}/${s} - showing ${y} of ${m.length} tournaments`),y===0){let T="No tournaments match the current filters";t==="my"&&(T="You haven't joined any tournaments yet"),t==="completed"&&(T="No completed tournaments yet"),typeof neodigmToast<"u"&&neodigmToast.q(T,"night")}};V.subscribe(e.hierTopics.COREBETSLIP,(_,m)=>{JSON.parse(m);let h=16;switch(_){case e.hierTopics.COREBETSLIP__BET:h=5,qs.shootConfetti(),console.log("~~~  |  "+_+" | ",m);const g=document.querySelector("[data-bets-valid]");g&&(g.dataset.betsValid="false");const y=document.querySelector("neodigm-sodapop");y&&y.setAttribute("data-wait","true");const T=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry");if(T.length===0){console.warn("[app_events] No bets to submit"),y&&y.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("No bets to submit","warning");break}const O=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,R=e.appSession?.session_user?.guid;if(!O||!R){console.error("[app_events] Missing tournament or user GUID"),y&&y.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Missing tournament or user data","danger");break}const E=Array.from(T).map(H=>{const $=H.getAttribute("data-content-team-points"),te=H.getAttribute("data-content-odds"),ne=H.getAttribute("data-stake")||"0",ie=H.getAttribute("data-content-type"),le=H.getAttribute("data-payout")||"0",ge=H.getAttribute("data-match-guid")||"",Ee={};return Ee[$]={type:ie,stake:parseFloat(ne),odds:parseFloat(te),payout:parseFloat(le),reconciled:!1},{acctEntity__guid:R,coreTournaments__guid:O,coreMatches__guid:ge,bet:[Ee],status:"PENDING"}});console.log("[app_events] Submitting bet slips:",E),Se.postBetSlips(E).then(H=>{console.log("[app_events] Bet slips posted successfully:",H);const $=E.reduce((te,ne)=>{const ie=ne.bet[0],le=Object.keys(ie)[0];return te+(ie[le]?.stake||0)},0);typeof neodigmToast<"u"&&neodigmToast.q(`Bets placed successfully!|Total Stake: TD$ ${$.toFixed(2)}`,"success"),Se.fetchBetSlips(R,O).then(te=>{if(te?.rows){const ne={timestamp:Date.now(),source:"API",data:te.rows};e.pushcoreBetSlip(ne),console.log("[app_events] Refreshed bet slips in store:",te.rows.length,"items")}ke.refreshPlayPopupUI(O,te),y&&y.setAttribute("data-wait","false")}).catch(te=>{console.error("[app_events] Error fetching fresh bet slips:",te);const ne=document.querySelector("[data-publish-betslip]");ne&&delete ne.dataset.processing,g&&(g.dataset.betsValid="true"),y&&y.setAttribute("data-wait","false"),typeof neodigmToast<"u"&&neodigmToast.q("Bets placed but failed to refresh|Please close and reopen","warning")})}).catch(H=>{console.error("[app_events] Error posting bet slips:",H);const $=document.querySelector("[data-publish-betslip]");$&&delete $.dataset.processing,g&&(g.dataset.betsValid="true"),typeof neodigmToast<"u"&&neodigmToast.q("Failed to place bets|Please try again","danger"),y&&y.setAttribute("data-wait","false")});break;case e.hierTopics.COREBETSLIP__SPREAD:break;case e.hierTopics.COREBETSLIP__MONEY:break;case e.hierTopics.COREBETSLIP__TOTAL:break;case e.hierTopics.COREBETSLIP__CLOSE:h=24;break;case e.hierTopics.COREBETSLIP__TDSYNC:if(document.querySelector("#summary-balance")&&m){m=JSON.parse(m);const $=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(e.coreBetSlip[0]?.data&&$){const te=e.coreBetSlip[0].data.filter(K=>K.coreTournaments__guid===$),ne=[],ie=[],le=[];te.forEach(K=>{const q=K.bet||[];if(q.length===0)return;const X=q[0];Object.keys(X).filter(S=>S!=="short_title").forEach(S=>{const w=X[S],k=w?.reconciled!==!1,D=parseFloat(w?.payout||0);k?D===0?ie.push(K):le.push(K):ne.push(K)})});const ge=nt.search(ne,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Ee=nt.search(ie,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Ie=ge+Ee,Te=nt.search(le,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,I=m.tournament_dollars-Ie+Te;ke.updateTDBalance("#summary-balance",I)}else ke.updateTDBalance("#summary-balance",m.tournament_dollars)}break;case e.hierTopics.COREBETSLIP__INTERSYNC:let L=0,P=0,z=0;if(e.coreBetSlip.length>0){const $=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if($){const ne=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(Te=>Te.coreTournaments__guid===$),ie=[],le=[],ge=[];ne.forEach(Te=>{const I=Te.bet||[];if(I.length===0)return;const K=I[0];Object.keys(K).filter(X=>X!=="short_title").forEach(X=>{const ce=K[X],S=ce?.reconciled!==!1,w=parseFloat(ce?.payout||0);S?w===0?le.push(Te):ge.push(Te):ie.push(Te)})}),L=nt.search(ie,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,P=nt.search(ge,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0;const Ee=nt.search(le,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Ie=nt.search(ge,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;z=Ee+Ie}}let Q=!0,oe=0;const G=document.querySelector("[data-bets-valid]");if(G&&m){m=JSON.parse(m),m.pending_stake_sum=0,m.pending_payout_sum=0,m.bets.length||(Q=!1),m.bets.forEach(ge=>{ge.stake=Number(ge.stake),ge.stake>0?m.pending_stake_sum+=ge.stake:Q=!1}),document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry").forEach(ge=>{const Ee=parseFloat(ge.getAttribute("data-payout")||"0");m.pending_payout_sum+=Ee});const $=L+m.pending_stake_sum,te=P+m.pending_payout_sum;oe=parseFloat((m.tournament_dollars-$-z+P).toFixed(2));const ie=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let le=null;ie&&e.coreTourn.length>0&&(le=e.coreTourn[e.coreTourn.length-1]?.data?.find(Ee=>Ee.guid===ie)),ke.updateSummaryAndDashboard(oe,$,te,le),(m.tournament_dollars<0||oe<0)&&(Q=!1),G.dataset.betsValid=Q}break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h,"QUITE").vibrate()});const r=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(T=>T.guid===m)?.tournament_dollars||0),V.publish(e.hierTopics.COREBETSLIP__TDSYNC,JSON.stringify({tournament_dollars:h,timestamp:Date.now()}))},o=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(x=>x.guid===m)?.tournament_dollars||0);let g=0,y=0,T=0;if(e.coreBetSlip.length>0&&m){const E=e.coreBetSlip[e.coreBetSlip.length-1].data.filter(G=>G.coreTournaments__guid===m);console.log("[app_events] Filtered bets for tournament:",m,"found:",E.length),console.log("[app_events] Using latest bet slip data index:",e.coreBetSlip.length-1),console.log("[app_events] First bet sample:",E[0]);const x=[],L=[],P=[];E.forEach(G=>{const H=G.bet||[];if(H.length===0)return;const $=H[0];Object.keys($).filter(ne=>ne!=="short_title").forEach(ne=>{const ie=$[ne],le=ie?.reconciled!==!1,ge=parseFloat(ie?.payout||0);le?ge===0?L.push(G):P.push(G):x.push(G)})}),console.log("[app_events] Bet categories:",{unreconciled:x.length,reconciledZero:L.length,reconciledNonZero:P.length});const z=nt.search(x,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,Q=nt.search(L,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0,oe=nt.search(P,"sum([].bet[].* | [].stake | map(&to_number(@), @)) || `0`")||0;T=Q+oe,y=nt.search(P,"sum([].bet[].* | [].payout | map(&to_number(@), @)) || `0`")||0,g=z,console.log("[app_events] Balance calculation:",{unreconciledStakes:z,lostBetStakes:Q,wonBetStakes:oe,allReconciledStakes:T,displayedStake:g,reconciledPayouts:y,tournamentDollars:h,calculatedBalance:h-g-T+y})}else console.warn("[app_events] No bet data found in Pinia store or missing tournamentGuid");const A=h-g-T+y;let O=null;m&&e.coreTourn.length>0&&(O=e.coreTourn[e.coreTourn.length-1].data.find(E=>E.guid===m)),ke.updateSummaryAndDashboard(A,g,y,O)};V.subscribe(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,(_,m)=>{const g=JSON.parse(m)?.rows||[],y=document.querySelector(".bet-grid__slip-MYBETS");if(!y)return;if(y.innerHTML="",g.length===0){y.innerHTML='<p style="color: var(--app-core-color--gray-6__dark--brand, #969696); text-align: center; padding: 20px;">No active bets</p>';const R=document.querySelector(".bet-grid__slip");R&&R.setAttribute("data-active-bet-tab","BETSLIP"),r();return}g.forEach(R=>{if((R.bet||[]).length===0)return;const x=document.createElement("bma-bet-existing");x.setAttribute("data-corebetslip",JSON.stringify(R)),y.appendChild(x)});const T=document.querySelector(".bet-grid__slip");T&&T.setAttribute("data-active-bet-tab","MYBETS"),me().appCLIFeatures?.features?.disable_existing_bet_btn?.state===!0&&ke.disableExistingBetButtons(),setTimeout(()=>{console.log("[app_events] Calling updateSummaryFromExistingBets after HYDRATE, bet count:",g.length),console.log("[app_events] Pinia store bet count:",e.coreBetSlip[0]?.data?.length||0),o()},200)}),document.addEventListener("click",_=>{const m=_.target;if(m&&m.classList.contains("bet-slip-tab")){const h=m.dataset.betTab,g=document.querySelector(".bet-grid__slip");h&&g&&(g.setAttribute("data-active-bet-tab",h),neodigmWired4Sound&&neodigmWired4Sound.sound(3),console.log("[app_events] Switched to bet tab:",h))}if(m&&m.dataset.publishBetslip){if(document.querySelector("[data-bets-valid]")?.dataset?.betsValid!=="true"){console.warn("[app_events] Cannot submit - bets invalid or button disabled");return}if(m.dataset.processing==="true"){console.warn("[app_events] Bet submission already in progress");return}m.dataset.processing="true";const g=m.dataset.publishBetslip;V.publish(g,JSON.stringify({timestamp:Date.now()}))}});const i=()=>{const m=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;let h=0;m&&e.coreTourn.length>0&&(h=e.coreTourn[e.coreTourn.length-1].data.find(O=>O.guid===m)?.tournament_dollars||0);const g=document.querySelectorAll(".bet-grid__slip-BETSLIP-content bma-bet-entry"),y=Array.from(g).map(T=>({teamPoints:T.getAttribute("data-content-team-points"),odds:T.getAttribute("data-content-odds"),stake:T.getAttribute("data-stake")||"0",type:T.getAttribute("data-content-type"),abbreviatedTitle:T.getAttribute("data-abbreviated-title"),scheduledAt:T.getAttribute("data-scheduled-at")}));V.publish(e.hierTopics.COREBETSLIP__INTERSYNC,JSON.stringify({tournament_dollars:h,bets:y,timestamp:Date.now()}))},l=(_,m)=>{const h=JSON.parse(m),g=document.querySelector(".bet-grid__slip-BETSLIP-content");if(!g){console.error("[app_events] Bet slip container not found");return}const y=g.querySelector(".bet-slip__empty");y&&y.remove();let T="";h.type==="total"?T=h.team==="over"?"Over":"Under":h.team==="home"?T=h.homeTeam:h.team==="away"&&(T=h.awayTeam);let A=T;if(h.point)if(h.type==="spread"){const P=parseFloat(h.point)>0?`+${h.point}`:h.point;A=`${T} ${P}`}else h.type==="total"&&(A=`${T} ${h.point}`);const O=document.createElement("bma-bet-entry");O.setAttribute("data-content-team-points",A),O.setAttribute("data-content-odds",h.price||"0"),O.setAttribute("data-content-stake-text","0"),O.setAttribute("data-content-type",h.type||""),O.setAttribute("data-abbreviated-title",h.abbreviatedTitle||`${h.homeTeam} vs ${h.awayTeam}`),O.setAttribute("data-scheduled-at",h.scheduledAt||""),O.setAttribute("data-match-guid",h.matchGuid||""),O.setAttribute("data-home-team",h.homeTeam||""),O.setAttribute("data-away-team",h.awayTeam||""),O.setAttribute("data-bet-team",T||"");const E=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(E&&e.coreTourn.length>0){const z=e.coreTourn[e.coreTourn.length-1].data.find(Q=>Q.guid===E);if(z?.sports_allowed&&z.sports_allowed.length>0){const Q=z.sports_allowed[0];O.setAttribute("data-sport-key",Q.key||"");const oe=qt.find(G=>G.key===Q.key);O.setAttribute("data-sport-group",oe?.group||"")}}g.appendChild(O),console.log("[app_events] Bet entry appended to container:",O,"Container children:",g.children.length),setTimeout(()=>{d(O)},0);const x=document.querySelector(".bet-grid__slip");x&&(x.setAttribute("data-active-bet-tab","BETSLIP"),console.log("[app_events] Switched to BETSLIP tab"));const L=g.querySelectorAll("bma-bet-entry").length;L>=3?requestAnimationFrame(()=>{setTimeout(()=>{const P=document.querySelector(".bet-grid__slip");P&&(console.log("[app_events] Scrolling parent to bottom - bet count:",L,"scrollHeight:",P.scrollHeight,"current scrollTop:",P.scrollTop),P.scrollTo({top:P.scrollHeight,behavior:"smooth"}),setTimeout(()=>{console.log("[app_events] After scroll - scrollTop:",P.scrollTop)},500))},150)}):console.log("[app_events] Skipping scroll - only",L,"bet cards (need 3+)"),setTimeout(()=>{i()},200),console.log("[app_events] Created bet entry:",{teamPoints:A,odds:h.price,type:h.type})};V.subscribe(e.hierTopics.COREBETSLIP__SPREAD,(_,m)=>{l(_,m),c()}),V.subscribe(e.hierTopics.COREBETSLIP__MONEY,(_,m)=>{l(_,m),c()}),V.subscribe(e.hierTopics.COREBETSLIP__TOTAL,(_,m)=>{l(_,m),c()});function c(){const _=window.matchMedia("(orientation: portrait)").matches,m=window.matchMedia("(max-width: 768px)").matches;if(_||m){const h=document.querySelector(".bet-grid__select"),g=document.querySelector(".bet-grid__slip"),y=document.querySelector(".bet-grid__toggle-text");h&&g&&y&&(h.classList.add("collapsed"),g.classList.remove("collapsed"),y.textContent="Close Bet Slip",console.log("[app_events] Bet slip expanded on portrait mode"))}}const d=_=>{const m={sport:{key:_.getAttribute("data-sport-key")||"",group:_.getAttribute("data-sport-group")||""},Match:{scheduled_at:_.getAttribute("data-scheduled-at")||"",home_team_id:_.getAttribute("data-home-team")||"",home_team_score:"",away_team_id:_.getAttribute("data-away-team")||"",away_team_score:""},Bet:{scope:{team_id:_.getAttribute("data-bet-team")||"",over:"",under:""},type:_.getAttribute("data-content-type")||"",odds:_.getAttribute("data-content-odds")||"0",stake:_.getAttribute("data-stake")||"0"}},h=ml.calcPayout(m);_.setAttribute("data-payout",h.toString()),console.log("[app_events] Payout calculated:",{stake:m.Bet.stake,odds:m.Bet.odds,payout:h})};new MutationObserver(_=>{_.forEach(m=>{m.type==="attributes"&&m.attributeName==="data-stake"&&m.target.tagName==="BMA-BET-ENTRY"&&(console.log("[app_events] Stake changed:",m.target.getAttribute("data-stake")),d(m.target),clearTimeout(window.betSlipStakeTimeout),window.betSlipStakeTimeout=setTimeout(()=>{i()},300))})}).observe(document.body,{attributes:!0,attributeFilter:["data-stake"],subtree:!0}),console.log("[app_events] MutationObserver for stake changes initialized"),document.addEventListener("bma-bet-entry-remove",_=>{console.log("[app_events] Removing bet entry:",_.detail);const m=_.target,h=_.detail;m.remove(),setTimeout(()=>{const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid;if(A){const O=me(),R=O.coreTourn[O.coreTourn.length-1];if(R&&R.data){const E=R.data.find(x=>x.guid===A);E&&ke.updateTournamentDashboard(E)}}},100),document.querySelectorAll("bma-bet-match-card").forEach(T=>{(T.shadowRoot?.querySelectorAll(".btn")||[]).forEach(O=>{const R=O.dataset.betType,E=O.dataset.price,x=R===h.type,L=E===h.odds;x&&L&&O.classList.contains("btn--active")&&(O.classList.remove("btn--active"),console.log("[app_events] Deactivated button via X removal:",{btnType:R,btnPrice:E}))})});const y=document.querySelector(".bet-grid__slip-BETSLIP-content");y&&y.children.length,setTimeout(()=>{i()},100)}),V.subscribe(e.hierTopics.CANVAS__HYDRATE__STATUS,(_,m)=>{const h=JSON.parse(m);h&&h.guid&&neodigmUtils&&neodigmUtils.shake(`[data-bma-tourn-guid="${h.guid}"]`)}),V.subscribe(e.hierTopics.WC__APP__FOOT,(_,m)=>{JSON.parse(m);let h=16;switch(_){case e.hierTopics.WC__APP__FOOT__ALL_SPORTS:V.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LOBBY,JSON.stringify({tab:"ALL_SPORTS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_TOURNEYS:V.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_MY,JSON.stringify({tab:"MY_TOURNEYS",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__LEADERBOARD:h=24,V.publish(e.hierTopics.WC__APP__HEAD_MID__HEAD_MID_LEADERBOARD,JSON.stringify({tab:"LEADERBOARD",timestamp:Date.now()}));break;case e.hierTopics.WC__APP__FOOT__MY_PROFILE:V.publish(e.hierTopics.WC__APP__HEAD_TOP__USER_PROFILE,JSON.stringify({tab:"USER_PROFILE",timestamp:Date.now()}));break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()}),V.subscribe(e.hierTopics.WC__APP__HEAD_SPORTS,(_,m)=>{const h=JSON.parse(m);let g=16;_=="WC.APP.HEAD_SPORTS.PREV"||_=="WC.APP.HEAD_SPORTS.NEXT"||_=="WC.APP.HEAD_SPORTS.HEAD_SPORTS_DRAG_START"?g=3:(s=h.key||"all",console.log(`[app_events] Sports filter changed to: ${s} (${h.group})`),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SPORT_FILTER_CHANGE"}))),g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),V.subscribe(e.hierTopics.WC__APP__HEAD_MID,(_,m)=>{JSON.parse(m);let h=16;const g=()=>{s="all";const y=document.querySelector("bma-app-head-sports");y&&y.setAttribute("data-selected-chip","all")};switch(_){case"WC.APP.HEAD_MID.HEAD_MID_LOBBY":t="lobby",g(),n(),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_MY":if(!e.appSession?.session_user?.guid){console.warn("[app_events] No user GUID found, cannot filter tournaments"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to view your tournaments","warning");return}t="my",g(),n(),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_COMPLETED":t="completed",g(),n(),V.publish(e.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"FILTER_CHANGE"}));break;case"WC.APP.HEAD_MID.HEAD_MID_LEADERBOARD":t="leaderboard",g(),n(),a(),h=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break;case"WC.APP.HEAD_MID.HEAD_MID_SQUAD":t="squad",g(),n(),a(),h=24,typeof neodigmToast<"u"&&neodigmToast.q("Feature not Available|in Beta","night");break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()}),V.subscribe(e.hierTopics.ROUTE__HOME__USER_PROFILE,async(_,m)=>{JSON.parse(m);let h=16;switch(_){case"ROUTE.HOME.USER_PROFILE.INSTALL_PWA":const g=window.deferredPWAPrompt;if(!g){console.log("No install prompt available"),typeof neodigmToast<"u"&&neodigmToast.q("PWA install not available","warning");return}try{g.prompt();const{outcome:T}=await g.userChoice;T==="accepted"?(console.log("PWA installed successfully"),window.deferredPWAPrompt=null,typeof neodigmToast<"u"&&neodigmToast.q("App installed successfully!","success"),h=8,neodigmSodaPop&&neodigmSodaPop.close()):(console.log("PWA install declined"),typeof neodigmToast<"u"&&neodigmToast.q("Install cancelled","night"))}catch(T){console.error("PWA install error:",T),typeof neodigmToast<"u"&&neodigmToast.q("Install failed","danger")}break;case"ROUTE.HOME.USER_PROFILE.TOGGLE_THEME":(()=>{const A=e.appCLIFeatures.features.theme.state==="dark"?"light":"dark";e.setFeaturePersistPub("theme",{state:A});const O=document.getElementById("app");O&&O.setAttribute("data-sync-theme",A),document.body.setAttribute("data-sync-theme",A)})(),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.RESET_PASSWORD":Fa.push({name:"resethash_route"}),neodigmSodaPop&&neodigmSodaPop.close();break;case"ROUTE.HOME.USER_PROFILE.SIGNOUT":Fa.push({name:"signout_route"}),neodigmSodaPop&&neodigmSodaPop.close();break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()});const p=(_,m)=>{const h=_.split(".").pop();console.log("[app_events] SODAPOP_PLAY subscriber - action:",h,"topic:",_),setTimeout(()=>{const g=document.querySelectorAll(".play-cntr .btn-info-sm"),T=document.querySelector(".play-cntr")?.closest("neodigm-sodapop");g.forEach(A=>{const O=A.dataset.publishRouteHome?.split(".").pop()||"";if(O===h){if(console.log("[app_events] Activating button:",O),A.classList.add("btn-info-sm--active"),A.classList.remove("btn-info-sm--inactive"),neodigmCarousel.nav({id:"caro-play",nav:O}),neodigmWired4Sound.sound(3),T){const R=h==="PLAY"?"hidden":"auto";T.style.overflow=R,console.log("[app_events] Set sodapop overflow to:",R)}}else A.classList.add("btn-info-sm--inactive"),A.classList.remove("btn-info-sm--active")})},100)};V.subscribe("ROUTE.HOME.SODAPOP_PLAY.INFO",p);let f=null;V.subscribe("ROUTE.HOME.SODAPOP_PLAY.LEADERBOARD",(_,m)=>{if(p(_),f){console.log("[appEvents] Leaderboard refresh debounced (cooling down)");return}f=setTimeout(()=>{f=null},2e3),console.log("[appEvents] Refreshing leaderboard data"),ke.populateLeaderboard()}),V.subscribe("ROUTE.HOME.SODAPOP_PLAY.PLAY",(_,m)=>{JSON.parse(m);const h=document.getElementById("btn-join__play--id");if(!h){console.warn("[app_events] PLAY button not found");return}const g=h.dataset.requiresJoin==="true",y=h.textContent.trim().toLowerCase();if(console.log("[app_events] PLAY button clicked - text:",y,"requiresJoin:",g),g&&y==="join"){const A=document.querySelector(".play-cntr")?.dataset?.currentTournGuid,O=me();if(!A){console.error("[app_events] No tournament GUID found");return}console.log("[app_events] Executing JOIN for tournament:",A),V.publish(O.hierTopics.WC__TOURN_ACTION,JSON.stringify({action:"JOIN",tournamentGuid:A,timestamp:Date.now()})),h.textContent="Play",h.dataset.requiresJoin="false",setTimeout(()=>{p(_),console.log("[app_events] JOIN triggered, advancing carousel to PLAY page")},500)}else p(_),console.log("[app_events] Advancing carousel to PLAY page")}),V.subscribe(e.hierTopics.WC__APP__HEAD_TOP,(_,m)=>{JSON.parse(m);let h=10;switch(_){case"WC.APP.HEAD_TOP.USER_PROFILE":h=10,neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_my_profile"),setTimeout(()=>{neodigmCarousel.nav({id:"caro-user_profile",nav:"caro-page__badges"})},1200);break;case"WC.APP.HEAD_TOP.LOGO":const g=e.appSession.session_app.version,y=new Date().getFullYear();g&&neodigmToast&&neodigmToast.q(`${g} 3/21/2026, 7:43:40 PM|© ${y} Bet Max Action`,"night");break}h&&neodigmWired4Sound&&neodigmWired4Sound.sound(h).vibrate()});let b=null,v=null;V.subscribe(e.hierTopics.WC__TOURN_ACTION,(_,m)=>{const h=JSON.parse(m);let g=0;switch(h?.action){case"JOIN":g=512;break;case"PLAY":g=8;break;case"INFO":g=8;break}if(g){b=h?.tournamentGuid,v=h?.action,console.log("[app_events] Storing pending tourn data:",b,v);const y=e.appSession?.session_user?.guid,T=h?.tournamentGuid;y&&T?Se.fetchBetSlips(y,T).then(A=>{if(A?.rows){const O={timestamp:Date.now(),source:"API",data:A.rows};e.pushcoreBetSlip(O),console.log("[app_events] Bet slips fetched:",A.rows.length,"items")}setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play"),setTimeout(()=>{V.publish(e.hierTopics.SODAPOP__HYDRATE__COREBETSLIP,JSON.stringify(A))},3e3)},g)}).catch(A=>{console.error("[app_events] Error fetching bet slips:",A),setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}):setTimeout(()=>{neodigmSodaPop&&neodigmSodaPop.autoOpen("sodapop_play")},g)}}),V.subscribe(e.hierTopics.WC__TOURN_ACTION,(_,m)=>{const h=JSON.parse(m);let g=16;switch(h?.action){case"FOCUS":g=3;break;case"SPORT_ICON":g=10;break;case"JOIN":g=5;break;case"PLAY":g=16;break}g&&neodigmWired4Sound&&neodigmWired4Sound.sound(g).vibrate()}),V.subscribe(e.hierTopics.WC__TOURN_ACTION,(_,m)=>{const h=JSON.parse(m);let g=0,y="night";switch(neodigmOpt&&neodigmOpt.EVENT_SOUNDS&&(neodigmOpt.EVENT_SOUNDS=!1),h?.tournamentStatus){case"UPCOMING":y="success";break;case"LOCKED":y="warning";break;case"COMPLETED":y="danger";break}switch(h?.action){case"SPORT_ICON":h?.sportTitle==h?.sportDescription?g=h?.sportTitle:g=h?.sportTitle+"|"+h?.sportDescription;break}g&&neodigmToast&&neodigmToast.q(g,y)}),V.subscribe(e.hierTopics.WC__TOURN_ACTION,async(_,m)=>{const h=JSON.parse(m);if(h?.action==="JOIN"){const y=me().appSession?.session_user?.guid;if(!y){console.error("No entity guid found in session"),typeof neodigmToast<"u"&&neodigmToast.q("Please sign in to join tournaments","warning");return}const T=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);T&&(T.setAttribute("data-bma-tourn-wait","true"),qs.shootConfetti());const A={acctEntityGuid:y,tournamentGuid:h.tournamentGuid};try{const O={method:"POST",body:JSON.stringify(A),headers:Se.genHeaders()};console.log("Posting to:",Se.API_baseURI+"/m5t/"+Se.API_ver+"/coreTournaments/join",A);const E=await(await fetch(Se.API_baseURI+"/m5t/"+Se.API_ver+"/coreTournaments/join",O)).json();if(console.log("Join response:",E),E.ok)typeof neodigmToast<"u"&&neodigmToast.q("Successfully joined tournament!","success"),setTimeout(()=>{const x=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);if(x){x.setAttribute("data-bma-tourn-wait","false");const L=x.getAttribute("data-bma-tourn-entities");if(L)try{const P=JSON.parse(L);P.guids.includes(y)||(P.guids.push(y),x.setAttribute("data-bma-tourn-entities",JSON.stringify(P)))}catch(P){console.error("Failed to parse entities:",P)}}},3e3);else{const x=E?.message||"Failed to join tournament";typeof neodigmToast<"u"&&neodigmToast.q(x,"danger");const L=document.querySelector(`bma-tournament-card[data-bma-tourn-guid="${h.tournamentGuid}"]`);L&&L.setAttribute("data-bma-tourn-wait","false")}}catch(O){console.error("Join tournament error:",O),typeof neodigmToast<"u"&&neodigmToast.q("Network error joining tournament","danger"),typeof neodigmUtils<"u"&&neodigmUtils.hardReload()}}}),V.subscribe(e.hierTopics.ROUTE__HOME_ONMOUNT,async(_,m)=>{console.log("[app_events] ROUTE__HOME_ONMOUNT event received");const h=me();try{const g=await Se.fetchTournaments();console.log("[app_events] Tournaments fetched:",g),g?.rows&&Array.isArray(g.rows)?(h.pushCoreTourn({timestamp:Date.now(),source:"API",data:g.rows}),console.log("[app_events] Pushed to coreTourn, length:",h.coreTourn.length),V.publish(h.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now()}))):console.warn("[app_events] Invalid API response format:",g)}catch(g){console.error("[app_events] Error fetching tournaments:",g),typeof neodigmToast<"u"&&neodigmToast.q("Please Sign In","success")}}),V.subscribe(e.hierTopics.PROMOTION__LOAD,async(_,m)=>{console.log("[app_events] PROMOTION__LOAD event received");try{const h=await Se.fetchPromotions();if(console.log("[app_events] Promotions fetched:",h),h?.ok&&h?.data&&Array.isArray(h.data)){const g=h.data.filter(T=>T.class==="banner"&&T.status==="LIVE");console.log("[app_events] Banner promotions:",g);const y=document.querySelector(".featured-swiper .swiper-wrapper");if(!y){console.warn("[app_events] Swiper wrapper not found");return}y.innerHTML="",g.forEach((T,A)=>{const O=document.createElement("div");O.className="swiper-slide";const R=document.createElement("div");R.className=`featured-card featured-card--${A+1}`;const E=T.hero_img?encodeURI(T.hero_img):"";R.style.backgroundImage=`url("${E}")`,R.setAttribute("data-promotion-hero-img",T.hero_img||""),R.setAttribute("data-promotion-caption",T.caption||""),R.setAttribute("data-promotion-tagline",T.tagline||""),R.setAttribute("data-promotion-toast",T.toast||""),R.setAttribute("data-promotion-topic",T.topic?.topic||""),R.setAttribute("data-promotion-topic-token",T.topic?.token||""),R.setAttribute("data-promotion-marquee",T.maquee||""),O.appendChild(R),y.appendChild(O)}),console.log("[app_events] Created",g.length,"promotion slides"),window.featuredSwiper&&window.featuredSwiper.update()}}catch(h){console.error("[app_events] Error fetching promotions:",h)}}),V.subscribe(e.hierTopics.PROMOTION__CLICK,(_,m)=>{console.log("[app_events] PROMOTION__CLICK event received");try{const h=JSON.parse(m);console.log("[app_events] Promotion clicked:",h),h.toast&&typeof neodigmToast<"u"&&neodigmToast.q(h.toast,"success"),typeof neodigmSodaPop<"u"&&(neodigmSodaPop.setOnAfterOpen(()=>{const g=document.querySelector(".l-promotion #promoHero");g&&h.heroImg&&(g.src=h.heroImg);const y=document.querySelector(".l-promotion #promCaption");y&&h.caption&&(y.textContent=h.caption);const T=document.querySelector(".l-promotion #promoTagline");return T&&h.tagline&&(/<[^>]+>/.test(h.tagline)?T.innerHTML=h.tagline:T.textContent=h.tagline),!0},"sodapop_promotion"),neodigmSodaPop.autoOpen("sodapop_promotion"))}catch(h){console.error("[app_events] Error handling promotion click:",h)}}),V.subscribe(e.hierTopics.ROUTE__HOME_HYDRATE,(_,m)=>{console.log("[app_events] ROUTE__HOME_HYDRATE event received");const h=me();if(h.coreTourn.length===0){console.warn("[app_events] No tournament data in coreTourn array");return}const g=h.coreTourn[h.coreTourn.length-1],y=h.coreTourn.length>1?h.coreTourn[h.coreTourn.length-2]:null;console.log("[app_events] Hydrating with latest data:",g);const T=[];if(y){const E=g.data,x=y.data;E.forEach(L=>{const P=x.find(z=>z.guid===L.guid);P&&P.status!==L.status&&(console.log(`[app_events] Status changed for tournament ${L.guid}: ${P.status} -> ${L.status}`),T.push(L.guid))})}const A=h.appSession?.session_user?.guid,O=[...g.data].sort((E,x)=>{const L=E.status||E.class,P=x.status||x.class,z=E.entities?.guids||[],Q=x.entities?.guids||[],oe=z.includes(A),G=Q.includes(A),H=(Te,I)=>{const K=new Date(Te.status_time||0).getTime();return new Date(I.status_time||0).getTime()-K},$=L==="LOCKED"&&oe,te=P==="LOCKED"&&G;if($&&!te)return-1;if(!$&&te)return 1;if($&&te)return H(E,x);const ne=L==="UPCOMING",ie=P==="UPCOMING";if(ne&&!ie)return-1;if(!ne&&ie)return 1;if(ne&&ie)return H(E,x);const le=L==="COMPLETED"&&oe,ge=P==="COMPLETED"&&G;if(le&&!ge)return-1;if(!le&&ge)return 1;if(le&&ge)return H(E,x);const Ee=L==="COMPLETED"&&!oe,Ie=P==="COMPLETED"&&!G;return Ee&&!Ie?1:!Ee&&Ie?-1:H(E,x)}),R=ke.categorizeTournaments(O,t,s,A);if(console.log(`[app_events] Categorized into ${R.length} categories for filter: ${t}/${s}`),window._homeRouteRenderCards&&typeof window._homeRouteRenderCards=="function"){if(window._homeRouteRenderCards(R,!0),setTimeout(()=>{a(),console.log("[app_events] Re-applied filter after hydration:",t)},100),T.length>0){const E=g.data;T.forEach(x=>{const L=E.find(P=>P.guid===x);if(L){const P=Math.floor(Math.random()*2e3)+1e3;setTimeout(()=>{V.publish(h.hierTopics.CANVAS__HYDRATE__STATUS,JSON.stringify({guid:x,status:L.status,timestamp:Date.now()}))},P)}})}}else console.warn("[app_events] window._homeRouteRenderCards not available")}),V.subscribe(e.hierTopics.SSE__CORE__TOURN_SYNC,(_,m)=>{console.log("[app_events] SSE__CORE__TOURN_SYNC event received:",m);const h=me();try{const g=JSON.parse(m);console.log("[app_events] SSE tournament sync data:",g);const y=JSON.parse(g.msg);if(console.log("[app_events] Parsed tournaments array:",y),!Array.isArray(y)){console.warn("[app_events] SSE msg is not an array:",y);return}h.pushCoreTourn({timestamp:Date.now(),source:"SSE",data:y}),console.log("[app_events] Pushed SSE data to coreTourn, length:",h.coreTourn.length),V.publish(h.hierTopics.ROUTE__HOME_HYDRATE,JSON.stringify({timestamp:Date.now(),source:"SSE"})),typeof neodigmSodaPop<"u"&&neodigmSodaPop.isOpen()==="sodapop_play"&&(console.log("[SSE] Play popup is open, triggering real-time update"),ke.hydratePlayPopup())}catch(g){console.error("[app_events] Error processing SSE tournament sync:",g)}}),setTimeout(function(){neodigmSodaPop&&neodigmMarquee&&neodigmEnchantedCTA&&(neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{},1e3),neodigmMetronome.init().pause(800),neodigmMarquee.init().pause(1200),neodigmEnchantedCTA.init(),neodigmCarousel.init()}),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{NeodigmClaire.showCanv("neodigm-sodapop",.5).setTheme(["warning","success"]).initCanvOn("neodigm-sodapop").waxOn("neodigm-sodapop");const _=document.querySelector("#caption__my-profile");_&&(_.textContent="My Profile");const m=e.appSession?.session_user?.guid;if(console.log("[app_events] User GUID:",m),console.log("[app_events] coreTourn length:",e.coreTourn.length),!m||e.coreTourn.length===0){console.warn("[app_events] Missing user GUID or tournament data");return}const g=e.coreTourn[e.coreTourn.length-1].data;console.log("[app_events] Processing tournaments:",g.length);let y=0,T=0,A=0;g.forEach((O,R)=>{if(!O.tags||!Array.isArray(O.tags)){console.log(`[app_events] Tournament ${R} has no tags or tags not an array`);return}O.tags.forEach((E,x)=>{if(typeof E=="object"&&E!==null){let L=null;E.entity_guid===m&&E.badge?L=E.badge:E[m]&&(L=E[m]),L==="--badge__ribbon--gold"?y++:L==="--badge__ribbon--silver"?T++:L==="--badge__ribbon--bronze"&&A++}})}),setTimeout(()=>{let O=document.querySelectorAll(".badge-counter");if(O.length===0){const R=document.querySelector("neodigm-sodapop");R&&(O=R.querySelectorAll(".badge-counter"))}O.length>=3?(O[0].textContent=y,O[1].textContent=T,O[2].textContent=A,console.log("[app_events] Updated UI counters successfully")):console.warn("[app_events] Not enough counter elements found")},500)},1e3)},"sodapop_my_profile"),neodigmSodaPop.setOnAfterOpen(function(){setTimeout(()=>{const _=me(),m=document.querySelector(".play-cntr");b&&(m.dataset.currentTournGuid=b),v&&(m.dataset.currentTournAction=v);const h=m?.dataset?.currentTournGuid,g=m?.dataset?.currentTournAction||"INFO";console.log("[app_events] sodapop_play opened - GUID:",h,"action:",g),window.initBetGridToggle&&window.initBetGridToggle(),ke.updatePlayButton(),ke.populateInfoPage(),setTimeout(()=>{ke.populateLeaderboard()},600);let y=g==="PLAY"||g==="JOIN"?"PLAY":g;if(!h||_.coreTourn.length===0||_.coreTourn[_.coreTourn.length-1].data.find(z=>z.guid===h)?.status==="COMPLETED"&&(y="LEADERBOARD",console.log("[app_events] Tournament is COMPLETED, defaulting to LEADERBOARD")),y){const L=`ROUTE.HOME.SODAPOP_PLAY.${y}`;console.log("[app_events] Publishing initial topic:",L),V.publish(L,JSON.stringify({tournamentGuid:h,timestamp:Date.now()}))}if(!h||_.coreTourn.length===0)return;const A=_.coreTourn[_.coreTourn.length-1].data.find(L=>L.guid===h);ke.renderMatchCards(A);const O=document.querySelector("#pop-play__tourn-caption--id"),R=document.querySelector("#pop-play__tourn-tagline--id");O&&(O.textContent=A.caption),R&&(R.textContent=A.tagline),console.log("[Modal Debug] Updating dashboard for tournament:",A?.caption);const E=document.querySelector(".head-caption");console.log("[Modal Debug] Modal header found:",!!E),E&&console.log("[Modal Debug] Modal classes:",E.className),ke.updateTournamentDashboard(A);const x=document.getElementById("dashboard-rank");if(x){const L=A.entities?.guids?.length||0;x.innerHTML=`
